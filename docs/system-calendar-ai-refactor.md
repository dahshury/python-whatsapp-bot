...
- Batch commands should target the new tool names (`system_get_reservations`, `system_move_reservations`, `system_batch_*`, etc.), which are documented in `app/services/system_tool_schemas.py` and covered by `SystemAgentBatchService`.
