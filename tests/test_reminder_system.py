...
class ReminderSystemTests(unittest.TestCase):
    ...
    async def test_send_reminders_with_mock_data(
        self, mock_append, mock_get_reservations
    ):
        ...
        mock_get_reservations.return_value = {
            "success": True,
            "data": {test_wa_id: [reservation.to_dict()]},
        }
...
async def test_whatsapp_templates(wa_id="201017419800"):
    ...
    time_slot = "11:00 AM"
    components = [{"type": "body", "parameters": [{"type": "text", "text": time_slot}]}]

    print(f"\nSending test template to {wa_id}...")
    try:
        response = await send_whatsapp_template(
            wa_id, "appointment_reminder", "ar", components
        )
...
async def run_tests(specific_wa_id=None):
    ...
    print(f"Docker environment: {'Yes' if Path('/.dockerenv').exists() else 'No'}")
