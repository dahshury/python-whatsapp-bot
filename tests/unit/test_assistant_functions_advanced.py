...
class TestModifyReservation(unittest.TestCase):
    ...
    def test_modify_reservation_success(
        ...
        mock_normalize.side_effect = (
            lambda time, to_24h: "16:00" if to_24h else "4:00 PM"
        )
    ...
    @mock.patch("app.services.assistant_functions.is_valid_number")
    @mock.patch("app.services.assistant_functions.get_customer_reservations")
    def test_modify_reservation_no_reservations(
        ...
        # Execute
        result = modify_reservation(wa_id="1234567890", new_date="2023-05-20")

        # Assert
        self.assertFalse(result["success"])
        self.assertEqual(
            result["message"],
            "No upcoming reservations found to modify. Please reserve a new appointment.",
        )

    @mock.patch("app.services.assistant_functions.is_valid_number")
    @mock.patch("app.services.assistant_functions.get_customer_reservations")
    def test_modify_reservation_multiple_reservations(
        self, mock_get_reservations, mock_is_valid
    ):
        # Setup
        mock_is_valid.return_value = True
        ...
        # Execute
        result = modify_reservation(wa_id="1234567890", new_date="2023-05-22")
...
class TestDeleteReservation(unittest.TestCase):
    ...
    def test_delete_specific_reservation(
        ...
        result = delete_reservation(
            wa_id="1234567890", date_str="2023-05-15", time_slot="2:30 PM"
        )
    ...
    def test_delete_all_reservations(
        ...
        # Verify database operations
        mock_cursor.execute.assert_called_with(
            "DELETE FROM reservations WHERE wa_id = ?", ("1234567890",)
        )
...
class TestGetCustomerReservations(unittest.TestCase):
    ...
    def test_get_customer_reservations_with_future(
        ...
        mock_datetime.datetime.strptime.return_value.time.return_value = (
            mock.MagicMock()
        )
        ...
        # Execute
        result = get_customer_reservations(wa_id="1234567890", include_past=True)
...
class TestSearchAvailableAppointments(unittest.TestCase):
    ...
    def test_search_all_slots(
        ...
        # Available time slots for each day
        mock_get_time_slots.return_value = {"10:00 AM": 0, "2:30 PM": 0, "4:00 PM": 0}
