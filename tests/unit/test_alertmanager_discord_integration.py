...
class TestAlertmanagerDiscordIntegration(unittest.TestCase):
    ...
    def test_retry_attempts_metric_triggers_alert(self):
        ...
        try:
            ...
            alert_payload = {
                ...
                "commonLabels": {
                    "alertname": "AssistantRetryOccurred",
            ...
            self.assertEqual(
                response.status_code, 200, "Discord adapter should accept the alert"
            )

            # Verify the metric was incremented
            sample_value = self._get_sample_value(
                "api_retry_attempts_total", {"exception_type": "HTTPError"}
            )
    ...
    def test_http_error_triggers_alert(self):
        ...
        try:
            ...
            http_counter.labels(
                method="POST", endpoint="/api/reservations", http_status="500"
            ).inc()
            ...
            alert_payload = {
                ...
                "commonLabels": {
                    "alertname": "AssistantFunctionException",
                    "http_status": "500",
            ...
            self.assertEqual(
                sample_value, 1.0, "HTTP error metric should be incremented"
            )
        finally:
            # Clean up
    ...
    def test_service_function_exception(self):
        ...
        try:
            ...
            http_counter.labels(
                method="DELETE", endpoint="/api/reservations", http_status="500"
            ).inc()
            ...
            self.assertEqual(
                sample_value,
                1.0,
                "HTTP error metric should be incremented when service function throws exception",
            )
        finally:
            # Clean up
            REGISTRY.unregister(http_counter)
    ...
    def test_delete_reservation_alert(self):
        ...
        try:
            ...
            http_counter.labels(
                method="DELETE", endpoint="/api/reservations/12345", http_status="500"
            ).inc()
            ...
            self.assertEqual(
                sample_value,
                1.0,
                "Delete reservation HTTP error metric should be incremented",
            )
        finally:
            # Clean up
            REGISTRY.unregister(http_counter)
            ...
            for sample in metric.samples:
                if sample.name == metric_name:
                    ...
                    for label_name, label_value in labels.items():
                        if (
                            label_name not in sample.labels
                            or sample.labels[label_name] != label_value
                        ):
                            match = False
                            break
                    if match:
