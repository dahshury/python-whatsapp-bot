...
class TestSendBusinessLocation(unittest.TestCase):
    ...
    @mock.patch(
        "app.services.assistant_functions.config",
        {
    ...
    def test_send_business_location_success(self, mock_send_location, mock_is_valid):
        ...
        mock_send_location.assert_called_once_with(
            wa_id, "1.234", "5.678", "Test Business", "123 Test St"
        )
...
class TestGetCurrentDatetime(unittest.TestCase):
    ...
    def test_get_current_datetime(self, mock_convert, mock_datetime):
        ...
        mock_now.strftime.side_effect = (
            lambda fmt: "2023-05-15" if fmt == "%Y-%m-%d" else "10:30 AM"
        )
...
class TestModifyId(unittest.TestCase):
    ...
    def test_modify_id_success(self, mock_get_connection, mock_is_valid):
        ...
        mock_cursor.execute.assert_any_call(
            "UPDATE threads SET wa_id = ? WHERE wa_id = ?", (new_wa_id, old_wa_id)
        )
        mock_cursor.execute.assert_any_call(
            "UPDATE conversation SET wa_id = ? WHERE wa_id = ?", (new_wa_id, old_wa_id)
        )
        mock_cursor.execute.assert_any_call(
            "UPDATE reservations SET wa_id = ? WHERE wa_id = ?", (new_wa_id, old_wa_id)
        )
        mock_cursor.execute.assert_any_call(
    ...
    def test_modify_id_same_ids(self, mock_is_valid):
        ...
        self.assertTrue(result["success"])
        self.assertEqual(
            result["message"], "The new wa_id is the same as the old wa_id."
        )
...
class TestReserveTimeSlot(unittest.TestCase):
    ...
    def test_reserve_time_slot_success_new_reservation(
        ...
        mock_normalize_time.side_effect = (
            lambda time, to_24h: "14:30" if to_24h else "2:30 PM"
        )

        # Datetime mocks
        mock_now = mock.MagicMock()
        mock_now.date.return_value = datetime.date(
            2023, 5, 1
        )  # Earlier than reservation date
        mock_datetime.datetime.now.return_value = mock_now
        mock_datetime.datetime.strptime.return_value.replace.return_value = (
        ...
        mock_cursor.fetchone.return_value = [
            0
        ]  # No existing reservations for this slot
        mock_conn.cursor.return_value = mock_cursor
        mock_get_connection.return_value = mock_conn
...
class TestGetAvailableTimeSlots(unittest.TestCase):
    ...
    def test_get_available_time_slots_success(
        ...
        mock_now.date.return_value = datetime.date(
            2023, 5, 1
        )  # Earlier than appointment date
        mock_datetime.datetime.now.return_value = mock_now
        mock_datetime.datetime.strptime.return_value.date.return_value = datetime.date(
            2023, 5, 15
        )
        ...
        mock_get_time_slots.return_value = {"10:00 AM": 0, "11:00 AM": 0, "2:00 PM": 0}
...
class TestCancelReservation(unittest.TestCase):
    ...
    def test_cancel_reservation_specific_date(
        self, mock_get_connection, mock_parse_date, mock_get_reservations, mock_is_valid
    ):
        # Setup
        mock_is_valid.return_value = True
        mock_parse_date.return_value = "2023-05-15"
    ...
    @mock.patch("app.services.assistant_functions.is_valid_number")
    @mock.patch("app.services.assistant_functions.get_customer_reservations")
    def test_cancel_reservation_no_reservations(
        self, mock_get_reservations, mock_is_valid
    ):
        # Setup
        mock_is_valid.return_value = True
