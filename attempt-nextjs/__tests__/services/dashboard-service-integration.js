...
const mockConversationData = {
  'customer001': [
    { date: '2024-01-15', time: '9:45 AM', message: 'I need an appointment', role: 'customer' },
    { date: '2024-01-15', time: '9:50 AM', message: 'Sure! What time works for you?', role: 'assistant' },
    { date: '2024-01-15', time: '9:52 AM', message: '10 AM would be perfect', role: 'customer' }
  ],
  'customer002': [
    { date: '2024-01-18', time: '8:30 AM', message: 'Hi there', role: 'customer' },
    { date: '2024-01-18', time: '8:35 AM', message: 'Hello! How can I help?', role: 'assistant' }
  ],
  'customer003': [
    { date: '2024-01-22', time: '10:45', message: 'Quick question about my appointment' },
    { date: '2024-01-22', time: '10:50', message: 'Of course, what can I help with?' }
  ]
}
...
function compareObjects(obj1, obj2, path = '', tolerance = 0.001) {
  ...
  function compare(a, b, currentPath) {
    if (typeof a !== typeof b) {
      errors.push(`Type mismatch at ${currentPath}: ${typeof a} vs ${typeof b}`)
      return
    }
    ...
    if (typeof a === 'number') {
      if (Math.abs(a - b) > tolerance) {
        errors.push(`Number mismatch at ${currentPath}: ${a} vs ${b} (diff: ${Math.abs(a - b)})`)
      }
      return
    }
    ...
    if (Array.isArray(a) && Array.isArray(b)) {
      if (a.length !== b.length) {
        errors.push(`Array length mismatch at ${currentPath}: ${a.length} vs ${b.length}`)
        return
      }
    ...
    if (typeof a === 'object' && typeof b === 'object') {
      ...
      if (keysA.length !== keysB.length) {
        errors.push(`Object key count mismatch at ${currentPath}: ${keysA.length} vs ${keysB.length}`)
      }
...
async function testProcessDashboardData() {
  ...
  try {
    ...
    assert(typeof result.stats.totalReservations === 'number', 'Should have total reservations')
    assert(typeof result.stats.uniqueCustomers === 'number', 'Should have unique customers')
    assert(Array.isArray(result.dailyTrends), 'Should have daily trends array')
    assert(Array.isArray(result.topCustomers), 'Should have top customers array')
    
    // Verify data makes sense with our test data
    assert(result.stats.totalReservations > 0, 'Should have processed reservations')
    assert(result.stats.uniqueCustomers > 0, 'Should have identified customers')
    ...
    console.log(`  Conversion Rate: ${result.stats.conversionRate.toFixed(2)}%`)
...
async function testWithEmptyData() {
  ...
  try {
    ...
    assert(result.stats, 'Result should have stats object')
    assert(result.stats.totalReservations === 0, 'Empty data should have zero reservations')
    assert(result.stats.uniqueCustomers === 0, 'Empty data should have zero customers')
    assert(result.stats.conversionRate === 0, 'Empty data should have zero conversion rate')
    assert(Array.isArray(result.dailyTrends), 'Should have daily trends array')
...
async function testWithFilteredData() {
  ...
  try {
    ...
    assert(filteredResult.stats.totalReservations <= unfilteredResult.stats.totalReservations, 
           'Filtering should not increase reservation count')
    assert(filteredResult.stats.uniqueCustomers <= unfilteredResult.stats.uniqueCustomers,
           'Filtering should not increase customer count')
    assert(Array.isArray(filteredResult.dailyTrends), 'Should have daily trends')
    
    console.log('✅ Date filtering works correctly')
    console.log(`  Unfiltered: ${unfilteredResult.stats.totalReservations} reservations`)
    console.log(`  Filtered: ${filteredResult.stats.totalReservations} reservations`)
