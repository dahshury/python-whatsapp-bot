...
function ConversationMetric({ title, value, unit, icon, description, isRTL }: ConversationMetricProps) {
  return (
    <motion.div
...
export function ConversationLengthAnalysis({ conversationAnalysis, isRTL }: ConversationLengthAnalysisProps) {
  ...
  const getEngagementLevel = (avgMessages: number) => {
    if (avgMessages >= 20) return { level: 'high', color: 'text-green-600', label: i18n.getMessage('engagement_high', isRTL) }
    if (avgMessages >= 10) return { level: 'medium', color: 'text-yellow-600', label: i18n.getMessage('engagement_medium', isRTL) }
    return { level: 'low', color: 'text-red-600', label: i18n.getMessage('engagement_low', isRTL) }
  }
