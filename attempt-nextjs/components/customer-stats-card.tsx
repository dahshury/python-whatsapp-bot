...
export const CustomerStatsCard = React.memo(function CustomerStatsCard({
  ...
  const firstMessage = conversation.length > 0 
  ...
  return (
    <div className={isHoverCard ? "" : "mb-2"}>
      <Card className={`bg-background/90 ${isHoverCard ? 'shadow-none border-0' : ''}`}>
              ...
              {firstMessage && (
                <div className={`flex items-center justify-between text-xs ${isHoverCard ? 'py-0' : 'py-0.5'}`}>
                  <div className="flex items-center gap-1">
                    ...
                    <span>{isRTL ? "عميل منذ" : "Customer since"}</span>
                  </div>
              ...
              {lastMessage && (
                <div className={`flex items-center justify-between text-xs ${isHoverCard ? 'py-0' : 'py-0.5'}`}>
                  <div className="flex items-center gap-1">
                    ...
                    <span>{isRTL ? "آخر رسالة" : "Last message"}</span>
                  </div>
                  <span className="text-muted-foreground text-[10px]">
                    {formatDate(lastMessage.date)} {formatTime(lastMessage.time)}
                  </span>
                </div>
              )}
            </div>
          ...
          {customerReservations.length > 0 && (
            ...
            >
              <AccordionItem value="reservations" className="border-b-0">
                <AccordionTrigger className={`${isHoverCard ? 'py-0.5 px-0.5' : 'py-1 px-1'} text-xs font-medium hover:no-underline`}>
                  <div className="flex items-center gap-1">
                    <Calendar className={isHoverCard ? "h-2.5 w-2.5" : "h-3 w-3"} />
                    <span>{isRTL ? "الحجوزات" : "Reservations"}</span>
