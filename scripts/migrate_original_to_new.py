...
def extract_data_from_original() -> tuple[list[dict], list[dict], list[dict]]:
    ...
    try:
        ...
        for row in cursor.fetchall():
            customers_data.append({"wa_id": row["wa_id"], "customer_name": row["customer_name"]})
...
def create_new_database_with_data(
    customers_data: list[dict], conversations_data: list[dict], reservations_data: list[dict]
) -> str:
    ...
    try:
        ...
        cursor.executescript("""
            ...
            -- Removed legacy customer_documents table (no longer used)
        """)
...
def validate_sqlite_direct(db_path: str) -> bool:
    ...
    try:
        ...
        # Legacy customer documents removed
        documents_count = 0
        ...
        print(f"  - Users: {users_count:,}")
        print(f"  - Legacy customer documents (removed): {documents_count:,}")
...
def validate_sqlalchemy_compatibility(db_path: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            # Skip legacy customer_documents validation (table does not exist)
