...
import { CalendarEvent, ReservationData } from "@/types/data-table-editor"
...
export function useDataTableDataSource(
  ...
  const dataSource = useMemo(() => {
    if (!selectedDateRange || events.length === 0) {
      ...
      return mapper.mapToDataSource([], columns, {
        getValue: () => null
    ...
    // Update event map
    const filteredEvents = events.filter(mappingConfig.filter).sort(mappingConfig.sort)
