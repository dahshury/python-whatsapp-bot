...
export function useVacationDateChecker(vacationPeriods: VacationPeriod[]): VacationDateChecker {
  return useMemo(() => {
    console.log('🏖️ VacationDateChecker: Creating checker with periods:', vacationPeriods.map(p => ({
      ...
      for (const period of vacationPeriods) {
        ...
        if (dateStr === '2025-01-15' || dateStr.includes('2025-01-1')) {
          console.log('🏖️ VacationDateChecker: Checking date', dateStr, {
            ...
            originalPeriod: { start: period.start.toString(), end: period.end.toString() },
            comparison: { 
              dateStr, 
              vacationStart, 
              vacationEnd,
