		...
		return (
			<BlurFade duration={0.3} blur="8px" direction="down" className="relative">
				<div className="relative">
					...
					<div className="filter blur-sm opacity-70 pointer-events-none select-none">{children}</div>
				...
				<div className="p-4">
					...
					<div className="grid grid-cols-7 gap-2 mb-4">
						{["mon", "tue", "wed", "thu", "fri", "sat", "sun"].map((day) => (
							<div key={`skeleton-header-${day}`} className="h-8 bg-muted animate-pulse rounded text-center" />
						))}
					</div>

					{/* Calendar days skeleton */}
					<div className="grid grid-cols-7 gap-2">
						{Array.from({ length: 35 }, (_, i) => `day-${i + 1}`).map((dayId, index) => (
							<div key={`skeleton-${dayId}`} className="h-20 bg-muted animate-pulse rounded relative">
