...
import type { DateSelectArg, EventApi, EventChangeArg } from "@fullcalendar/core";
import type { DateClickInfo } from "@shared/libs/calendar/calendar-callbacks";
...
import { handleEventChange as handleEventChangeService } from "@shared/libs/calendar/calendar-event-handlers";
...
import { CalendarErrorFallback, ErrorBoundary } from "@/shared/ui/error-boundary";
...
>(
	(
		...
		const resolvedInitialDate = (initialDate ?? new Date().toISOString().split("T")[0]) as string;
		...
		const processedAllEvents = useMemo(() => {
			...
			return base.map((event: CalendarEvent) => {
				...
				if ((event.extendedProps as { type?: number })?.type !== 2 && eventStartDate < today) {
					return {
						...event,
						editable: false,
		...
		const leftGetCalendarApi = useCallback(
			(): FullCalendarApi | undefined => leftCalendarRef.current?.getApi?.() as FullCalendarApi | undefined,
			[]
		);

		const rightGetCalendarApi = useCallback(
			(): FullCalendarApi | undefined => rightCalendarRef.current?.getApi?.() as FullCalendarApi | undefined,
			[]
		);

		// No-op updater to avoid re-triggering FullCalendar eventChange loop
		const updateEventNoop = useCallback((_id: string, _event: CalendarEventData) => {}, []);
		...
		const handleLeftEventChange = useCallback(
			async (info: EventChangeArg | { event: EventApi; draggedEl: HTMLElement }) => {
				...
				try {
					...
					globalWithGuard.__dualHandlersGuard = globalWithGuard.__dualHandlersGuard || new Map<string, number>();
					const ev = info?.event as { id?: unknown; startStr?: string; start?: Date } | undefined;
					const key =
				...
				try {
					const start: Date | null | undefined = (info as { event?: { start?: Date | null } })?.event?.start;
				...
				const convertedInfo: FullCalendarEventChangeInfo = {
					...
					oldEvent:
						"oldEvent" in info && info.oldEvent
							...
							: undefined,
					revert: "revert" in info && typeof info.revert === "function" ? info.revert : undefined,
				};
				...
				await handleEventChangeService({
					...
					...(leftGetCalendarApi ? { getCalendarApi: () => leftGetCalendarApi() || undefined } : {}),
		...
		const handleRightEventChange = useCallback(
			async (info: EventChangeArg | { event: EventApi; draggedEl: HTMLElement }) => {
				...
				await handleEventChangeService({
					...
					onRefresh: handleRefreshWithBlur,
					...(rightGetCalendarApi ? { getCalendarApi: () => rightGetCalendarApi() || undefined } : {}),
					updateEvent: updateEventNoop,
		...
		const wrapLeftSelect = useCallback(
			(callback: (info: DateSelectArg) => void) => {
				return (info: { start: Date; end: Date; startStr: string; endStr: string; allDay: boolean }) => {
					const wrappedInfo = {
						...info,
						view: {
