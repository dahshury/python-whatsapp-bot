...
import { orchestrateCalendarDrag } from "@processes/calendar";
...
type CancelReservationArgs = Parameters<typeof handleCancelReservationService>[0];
...
function getWindowProperty<T>(property: keyof Window, defaultValue: T): T {
	...
	return ((window as unknown as Record<string, unknown>)[property] as T) ?? defaultValue;
}
...
export function useCalendarEventHandlers({
		...
		const reflowSlot = (dateStr: string, timeSlotRaw: string) => {
			try {
				...
				for (const ev of inSlot) {
					...
					end.setMinutes(end.getMinutes() + Math.floor(offset + minutesPerReservation));
					try {
				...
				if (type === "reservation_created") {
					...
					if (!existing) {
						...
						api.addEvent({
							id: String(data.id),
							title: String(
								(data as { customer_name?: unknown; wa_id?: unknown } | undefined)?.customer_name ||
									(data as { customer_name?: unknown; wa_id?: unknown } | undefined)?.wa_id ||
									""
							),
							start,
							end: endDate,
					...
					reflowSlot(String(data.date), String(String(data.time_slot || "00:00").slice(0, 5)));
					...
					try {
						...
						if (ts && Date.now() - ts < 1000) {
							console.log("🚫 Suppressing recent local move for event", data.id);
							return;
						}
					} catch {}
					const evObj = api.getEventById(String(data.id));
					console.log("🔍 Found calendar event for update:", !!evObj, "eventId:", data.id);
					const start = `${data.date}T${String(data.time_slot || "00:00").slice(0, 5)}:00`;
					if (evObj) {
						evObj.setProp(
							"title",
						...
						evObj.setExtendedProp(
							"waId",
							data.wa_id || (evObj as EventApi)?.extendedProps?.waId || (evObj as EventApi)?.extendedProps?.wa_id
						);
						evObj.setExtendedProp(
							"wa_id",
							data.wa_id || (evObj as EventApi)?.extendedProps?.wa_id || (evObj as EventApi)?.extendedProps?.waId
						);
						evObj.setExtendedProp("slotDate", data.date);
						evObj.setExtendedProp("slotTime", to24h(String(data.time_slot || "00:00").slice(0, 5)));
						const startDate = new Date(start);
						const endDate = new Date(startDate.getTime() + 20 * 60 * 1000);
						try {
							const currentDepth = getWindowProperty("__suppressEventChangeDepth", 0);
	...
	const handleEventChange = useCallback(
			...
			await orchestrateCalendarDrag({
				...
				resolveEvent: (id: string) => {
					...
					try {
						...
						return ev ? { extendedProps: (ev as EventApi).extendedProps || {} } : undefined;
					} catch {
						return undefined;
					}
				},
		...
		[isVacationDate, _isLocalized, currentView, calendarRef, updateEvent, events.find]
	);
	...
	const handleOpenConversation = useCallback(
		async (eventId: string) => {
			...
			try {
				...
				const wa = (ev as EventApi)?.extendedProps?.waId || (ev as EventApi)?.extendedProps?.wa_id;
	...
	const handleCancelReservation = useCallback(
		async (eventId: string) => {
			...
			const getCalendarApi = calendarRef?.current ? () => calendarRef.current?.getApi?.() || undefined : undefined;
			await handleCancelReservationService({
				...
				...(getCalendarApi
					? {
							getCalendarApi: () =>
								getCalendarApi() as unknown as ReturnType<NonNullable<CancelReservationArgs["getCalendarApi"]>>,
						}
					: {}),
	...
	const handleEventAdded = useCallback(
		(event: unknown) => {
			if (event && typeof event === "object") {
				const calendarEvent = convertDataTableEventToCalendarEvent(event as Record<string, unknown>);
				addEvent({
					id: calendarEvent.id,
					title: calendarEvent.title,
					start: typeof calendarEvent.start === "string" ? calendarEvent.start : "",
					end:
					...
					extendedProps: {
						...
						reservationId:
							typeof calendarEvent.extendedProps?.reservationId === "number"
								? calendarEvent.extendedProps.reservationId
								: typeof calendarEvent.extendedProps?.reservationId === "string"
									? Number(calendarEvent.extendedProps.reservationId)
									: undefined,
					},
				});
			}
