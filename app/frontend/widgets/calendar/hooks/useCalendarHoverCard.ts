...
export function useCalendarHoverCard({ isDragging }: UseCalendarHoverCardProps) {
	...
	const handleEventMouseEnter = useCallback(
		(info: { event?: unknown; el: HTMLElement; jsEvent?: MouseEvent }) => {
			const event = info.event as { id?: string; extendedProps?: Record<string, unknown> } | undefined;
			const el = info.el;
			...
			const incomingWaId = String(event?.extendedProps?.waId || event?.extendedProps?.wa_id || event?.id || "");
			...
			if (!hoveredEventId) {
				const timer = setTimeout(() => {
					...
					const waId = String(event?.extendedProps?.waId || event?.extendedProps?.wa_id || event?.id || "");
	...
	const handleEventMouseLeave = useCallback(
		(info: { event?: unknown }) => {
			...
			const currentWaId = String(event?.extendedProps?.waId || event?.extendedProps?.wa_id || event?.id || "");
	...
	useEffect(() => {
		...
		const handleGlobalMouseMove = (e: MouseEvent) => {
			...
			const cardTop = hoverCardPosition.preferBottom ? hoverCardPosition.y : hoverCardPosition.y - cardHeight;
			const cardBottom = hoverCardPosition.preferBottom ? hoverCardPosition.y + cardHeight : hoverCardPosition.y;
			...
			if (eventRect) {
				...
				if (hoverCardPosition.preferBottom) {
					...
					isInBridgeArea =
						e.clientX >= bridgeLeft && e.clientX <= bridgeRight && e.clientY >= bridgeTop && e.clientY <= bridgeBottom;
				} else {
			...
			if ((!isNearCard && !isNearEvent && !isInBridgeArea) || isMovingAwayFromCard) {
				setIsMovingToCard(false);
