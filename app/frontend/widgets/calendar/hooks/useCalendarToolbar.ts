...
export function useCalendarToolbar({ calendarRef, currentView }: UseCalendarToolbarProps): UseCalendarToolbarReturn {
	...
	const updateButtonStates = useCallback(() => {
		...
		try {
			...
			try {
				...
				const isTodayInRange = currentStart && currentEnd && today >= currentStart && today < currentEnd;
				...
				try {
					...
					const api = (calendarRef?.current as unknown as { getApi?: () => unknown })?.getApi?.() as
						| {
								currentDataManager?: {
									data?: {
					...
					if (gen && state?.dateProfile && state?.currentDate) {
						const prevInfo = gen.buildPrev(state.dateProfile, state.currentDate, false);
				...
				try {
					...
					const state = dm?.state;
					if (gen && state?.dateProfile && state?.currentDate) {
						const nextInfo = gen.buildNext(state.dateProfile, state.currentDate, false);
				...
				if (currentStart && currentEnd) {
					...
					setVisibleEventCount((prev) => (prev === visibleCount ? prev : visibleCount));
				}
			} catch (err) {
	...
	const burstUpdateButtonStates = useCallback(() => {
		try {
			...
			typeof requestAnimationFrame === "function" && requestAnimationFrame(() => updateButtonStates());
