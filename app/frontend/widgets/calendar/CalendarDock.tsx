	...
	useKeyboardRepeatNavigation({
		...
		onCtrlUp: () => {
			try {
				...
				const currentIndex = calendarViewOptions.findIndex(
					(o) => o.value === currentView
				)
				const nextIndex =
					(currentIndex - 1 + calendarViewOptions.length) %
					calendarViewOptions.length
				const view = calendarViewOptions[nextIndex]?.value || 'multiMonthYear'
		...
		onCtrlDown: () => {
			try {
				...
				const nextIndex = (currentIndex + 1) % calendarViewOptions.length
				const view = calendarViewOptions[nextIndex]?.value || 'multiMonthYear'
				// Prevent view switching in simple dock when using list-only contexts
				if (currentView === 'listMonth') {
					return
