...
export function useHomeCalendarController(): HomeCalendarControllerResult {
	...
	// Sync default view from config - only when the config value changes
	// Do NOT run when user manually changes view (don't include currentView in deps)
	useEffect(() => {
		...
		// Only update if the default view config actually changed
		if (previousDefaultViewRef.current === defaultCalendarView) {
			return
		}
		previousDefaultViewRef.current = defaultCalendarView
		// Apply the new default view (user will see their config change take effect)
