...
import type { CalendarEvent as DataTableCalendarEvent, DataTableEditorProps } from "@widgets/data-table-editor/types";
...
export function DataTableEditor(props: DataTableEditorProps) {
	...
	const [sourceEvents, setSourceEvents] = useState<DataTableCalendarEvent[]>(events);
	...
	const { validateAllCells, checkEditingState, hasUnsavedChanges } = useDataTableValidation(dataProviderRef);
	...
	const previousValidationErrors = useRef<Array<{ row: number; col: number; message: string; fieldName?: string }>>([]);
	...
	const setValidationErrorsIfChanged = useCallback(
		(
			...
			if (!areValidationErrorsEqual(previousValidationErrors.current, newErrors)) {
	...
	const handleCheckEditingState = useCallback(() => {
		...
		try {
			if (canEnable && dataProviderRef.current) {
				...
				const isRequiredCellMissing = (cell: unknown, colDef: { isRequired?: boolean }): boolean => {
	...
	}, [checkEditingState, gridRowToEventMapRef?.current, setValidationErrorsIfChanged, validateAllCells]);
	...
	useEffect(() => {
		return () => {
			const provider = dataProviderRef.current as (DataProvider & { unsubscribe?: () => void }) | null;
	...
	React.useLayoutEffect(() => {
		try {
			...
			try {
				const mapRef = gridRowToEventMapRef?.current as Map<number, DataTableCalendarEvent> | undefined;
				if (mapRef && mapRef.size > 0) {
			...
			for (const [k, oldEv] of prevMap.entries()) {
				if (blockedKeys.has(k)) {
					const stillExists = (events || []).some((ev) => getReservationKeyRef.current(ev) === k);
						...
						>
							<div className="px-4 py-1.5 border-b flex flex-row items-center justify-between">
								<div className="flex flex-col space-y-1.5">
									<h2 className={"text-xl font-semibold leading-none tracking-tight py-2 text-left"}>
										{i18n.getMessage("data_editor_title", _isLocalized)} - {formattedDateRange}
									</h2>
									<p
										id={`data-editor-description-${typeof window !== "undefined" ? "client" : "ssr"}`}
										className="sr-only"
									>
										{i18n.getMessage("data_editor_description", _isLocalized)}
									</p>
								</div>
								<button
									type="button"
							...
							<div className="overflow-visible w-full flex-1 min-h-0">
								<div className="overflow-visible relative w-full h-full">
									{!isGridReady && <div className="absolute inset-0 z-10 bg-background/0" />}
									<div>
										<FullscreenProvider>
											{Grid && (
												<Grid
													...
													dataEditorRef={dataEditorRef as React.RefObject<DataEditorRef>}
													loading={!isGridReady}
							...
							<div className="px-4 py-1.5 border-t flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2 gap-2">
								<div className="flex items-center gap-2 relative ms-auto">
									<Button onClick={handleSaveChanges} className="gap-2" disabled={!canSave || isSaving}>
										{isSaving ? (
											<>
									...
									{validationErrors?.length > 0 && (
										<div className="absolute -top-1 -left-1">
											<ValidationErrorsPopover errors={validationErrors} triggerClassName="" />
										</div>
									)}
								</div>
							</div>
