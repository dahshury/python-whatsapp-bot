...
export function ValidationErrorsPopover(props: ValidationErrorsPopoverProps) {
	...
	const rowLabel = (rowIdx: number) => `${i18n.getMessage("row", isLocalized)} ${rowIdx + 1}`;
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent align={isLocalized ? "end" : "start"} sideOffset={8} className="w-96 p-0">
							...
							<AccordionItem key={row} value={`row-${row}`} className="px-2">
								<AccordionTrigger className="justify-start gap-3 px-2 [&>svg]:-order-1">
									<span className="flex items-center gap-2">
										<CircleDashedIcon size={16} className="shrink-0 text-destructive" aria-hidden="true" />
								...
								<AccordionContent className="p-0">
									{list.map((err) => {
										...
										return (
											<Collapsible key={key} className="border-t py-2 ps-6 pe-3" defaultOpen={false}>
												<CollapsibleTrigger className="flex w-full gap-2 text-[0.8125rem] leading-6 font-semibold [&[data-state=open]>svg]:rotate-180">
													...
													<span className="flex items-center gap-2">
														...
														<span className="truncate">{formatFieldLabel(err.fieldName, isLocalized)}</span>
													</span>
												</CollapsibleTrigger>
