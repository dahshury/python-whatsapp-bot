...
const UncontrolledPhoneNumberSelectorContent: React.FC<
...
> = ({
	...
	const statusStats = phoneStats?.status ?? phoneStats?.registration
	...
	const statusFilterHook = useStatusFilter()
	...
	const allContactsFilters = React.useMemo(() => {
		...
		if (statusFilterHook.statusFilter) {
			filters.status = statusFilterHook.statusFilter
		}
	...
	const isLoadingContacts = !isActiveSearch && (isLoadingRecent || isLoadingAll)
	...
	const phoneGroups = React.useMemo(() => {
		...
		const hasActiveFilters =
			...
			Boolean(statusFilterHook.statusFilter) ||
		...
		// Separate recent, favorites from all
		const recentGroup = groups.find((g) => g.key === 'recent')
		const favoritesGroup = groups.find((g) => g.key === 'favorites')
		...
		// Favorites group - show on page 1 and when no filters are active
		if (
			!hasActiveFilters &&
			allContactsPage === 1 &&
			favoritesGroup &&
			favoritesGroup.items.length > 0
		) {
			finalGroups.push(favoritesGroup)
		}
			...
			{isLoadingStats ? (
				...
				<div className="flex items-center gap-1 border-b px-1.5 py-[3.072px]">
					<div className="flex flex-1 flex-wrap items-center gap-1">
						...
						{statusFilterHook.statusFilter && (
							<StatusFilter
								getStatusLabel={statusFilterHook.getStatusLabel}
								handleRemoveStatusFilter={
									statusFilterHook.handleRemoveStatusFilter
								}
								handleStatusFilterSelect={
									statusFilterHook.handleStatusFilterSelect
								}
								isLocalized={isLocalized}
								isStatusOpen={isStatusOpen}
								statusFilter={statusFilterHook.statusFilter}
							...
							{hasFilteredItems && (
								<PhoneGroupsList
									...
									{...(pagination?.total
										...
										: {})}
									favoritesHeading={messages.favoritesHeading}
...
export const PhoneNumberSelectorContent = UncontrolledPhoneNumberSelectorContent
