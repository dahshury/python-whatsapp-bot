...
> = ({
	...
	const allContactsFilters = React.useMemo(() => {
		...
		if (messagesFilter?.range?.from && messagesFilter.range.to) {
			filters.dateRange = {
				type: 'messages',
				range: messagesFilter.range,
			}
		} else if (reservationsFilter?.range?.from && reservationsFilter.range.to) {
			filters.dateRange = {
				type: 'reservations',
	...
	const phoneGroups = React.useMemo(() => {
		...
		// Recent group - only show on page 1
		if (allContactsPage === 1 && recentGroup && recentGroup.items.length > 0) {
