const en = {
  ...
  kpi_avg_response_time_tooltip: "Average time for assistant to reply to customer (customer→assistant) vs previous period",
...
Object.assign(ar, {
  ...
  backend_connection_error_description: "لم نتمكن من الوصول إلى خادم بايثون. يرجى التأكد من تشغيله وإمكانية الوصول إليه.",
...
export const messages = {
  validation: {
    ...
    required: (field: string) => (isRTL() ? `${field} مطلوب` : `${field} is required`),
    // When field name is not known
    thisFieldIsRequired: () => (isRTL() ? "هذا الحقل مطلوب" : "This field is required"),
    // Formats / validity
    ...
    invalidPhone: () => (isRTL() ? "رقم هاتف غير صالح" : "Invalid phone number"),
    ...
    nameInvalidCharacters: () =>
      isRTL() ? "الاسم يحتوي على أحرف غير صالحة" : "Name contains invalid characters",
    nameWordsTooShort: () =>
      isRTL() ? "كل كلمة يجب أن تحتوي على حرفين على الأقل" : "Each word must be at least 2 characters",
  },
  grid: {
