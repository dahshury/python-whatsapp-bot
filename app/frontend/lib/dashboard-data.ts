...
export function useDashboardDataFrom(state: {
  ...
  const dashboardData = React.useMemo<DashboardData | null>(() => {
    try {
      ...
      const totalReservations = reservationEntriesFiltered.reduce((sum, [, items]) => sum + (Array.isArray(items) ? items.length : 0), 0);
      const totalMessages = conversationEntriesFiltered.reduce((sum, [, msgs]) => sum + (Array.isArray(msgs) ? msgs.length : 0), 0);
      ...
      const dashboard: DashboardData = {
        ...
        monthlyTrends: [],
        funnelData: [
          { stage: "Conversations", count: conversationEntriesFiltered.filter(([, msgs]) => (Array.isArray(msgs) ? msgs.length : 0) > 0).length },
          { stage: "Made reservation", count: reservationEntriesFiltered.filter(([, items]) => (Array.isArray(items) ? items.length : 0) > 0).length },
  ...
  }, [conversations, reservations, activeRange, vacations]);
