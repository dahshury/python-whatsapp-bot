export class VacationEventsService {
  static updateVacationEvents(api: any, vacationPeriods: Array<{ start: Date; end: Date }>) {
    if (!api) return;
    try {
      // Remove existing background events tagged as vacation
      const existing = api.getEvents().filter((e: any) => e.extendedProps?.__vacation);
      ...
      vacationPeriods.forEach((vp) => {
        ...
        const endExclusive = new Date(new Date(vp.end).getTime() + 24 * 60 * 60 * 1000);
        api.addEvent({
          start,
          end: endExclusive,
          display: "background",
