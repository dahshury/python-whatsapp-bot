import { cancelReservation, modifyReservation, reserveTimeSlot } from "@/lib/api";
...
export class DataTableOperationsService {
  ...
  private formatHHmmInZone(value: any, timeZone: string = "Asia/Riyadh"): string | null {
    try {
      if (!value) return null;
  ...
  async processCancellations(
  ...
  ): Promise<{ hasErrors: boolean; successfulOperations: SuccessfulOperation[] }> {
    let hasErrors = false;
    ...
    for (const rowIdx of deletedRows || []) {
      ...
      const waId: string = (mapped.extendedProps?.waId || mapped.extendedProps?.wa_id || mapped.id || "").toString();
      ...
      try {
        ...
        if (evObj) {
          try { evObj.setExtendedProp("cancelled", true); } catch {}
        }
        // Backend cancel
        const resp = await cancelReservation({ id: waId, date, isRTL: this.isRTL });
        if (!resp?.success) throw new Error(resp?.message || resp?.error || "Cancel failed");

        // Remove visually from calendar (optional: keep and grey out). Use remove for explicit deletion UX.
        try { evObj?.remove?.(); } catch {}
        ...
        hasErrors = true;
        toastService.error(this.isRTL ? "فشل الإلغاء" : "Cancel Failed", (e as Error)?.message || (this.isRTL ? "خطأ بالنظام، حاول لاحقًا" : "System error, try later"), 3000);
      }
    }
  ...
  async processModifications(
    ...
    for (const idxStr of indices) {
      ...
      const waId: string = (original.extendedProps?.waId || original.extendedProps?.wa_id || original.id || "").toString();
      const prevStartStr: string = original.start;
      const prevDate = prevStartStr.split("T")[0];
      const timeStrNew = this.to24h(change.time || prevStartStr.split("T")[1]?.slice(0,5) || "00:00");
      const dateStrNew = (change.date || prevDate) as string;
      const typeNew = this.parseType(change.type ?? original.extendedProps?.type);
      const titleNew = change.name || original.title || original.extendedProps?.customerName || waId;

      try {
        // Optimistic UI using FullCalendar Event API
        const evObj = this.calendarApi?.getEventById?.(evId);
        if (evObj) {
          try { evObj.setProp("title", titleNew); } catch {}
          try { evObj.setExtendedProp("type", Number(typeNew)); } catch {}
          try { evObj.setExtendedProp("cancelled", false); } catch {}
          ...
          } catch { try { (evObj as any).setDates(new Date(startIso), null); } catch {} }
        }

        // Backend modify
        const resp = await modifyReservation(waId, {
          date: dateStrNew,
          time: timeStrNew,
          title: titleNew,
          type: Number(typeNew),
          reservationId: original.extendedProps?.reservationId,
          approximate: !(typeof window !== "undefined" && (this.calendarApi?.view?.type || "").toLowerCase().includes("timegrid")),
        });
        if (!resp?.success) throw new Error(resp?.message || resp?.error || "Modify failed");
        ...
        onEventModified?.(evId, {
          ...
          extendedProps: { type: Number(typeNew), cancelled: false, reservationId: original.extendedProps?.reservationId },
        });
        successful.push({ type: "modify", id: evId, data: { waId, date: dateStrNew, time: timeStrNew, type: Number(typeNew) } });
        ...
        hasErrors = true;
        toastService.error(this.isRTL ? "فشل التعديل" : "Update Failed", (e as Error)?.message || (this.isRTL ? "خطأ بالنظام، حاول لاحقًا" : "System error, try later"), 3000);
      }
    }
  ...
  async processAdditions(
    ...
    for (const row of addedRows || []) {
      ...
      const tStr = this.formatHHmmInZone(row.time, "Asia/Riyadh")
        || this.formatHHmm(row.time)
        || this.to24h((row.time || "").toString())
        || "";
      const phoneRaw = (row.phone || "").toString();
      const waId = phoneRaw.startsWith("+") ? phoneRaw : (phoneRaw ? `+${phoneRaw}` : phoneRaw);
      ...
      try {
        ...
        // Backend create
        const resp = await reserveTimeSlot({ id: waId, title: name || waId, date: dStr, time: tStr, type: Number(type), ar: this.isRTL });
        if (!resp?.success) {
          const reason =
            (typeof resp?.message === "string" && resp.message) ||
            (typeof resp?.error === "string" && resp.error) ||
            (typeof resp?.detail === "string" && resp.detail) ||
            (resp && typeof resp === "object" ? JSON.stringify(resp) : "") || "Unknown";
          const desc = [
          ...
          toastService.error(this.isRTL ? "فشل الإنشاء" : "Create Failed", desc, 6000);
          hasErrors = true;
          continue;
        }
        ...
        const newEvent = this.calendarApi?.addEvent?.({
          id: String(resp?.id || resp?.reservationId || `${waId}-${dStr}-${tStr}`),
          title: name || waId,
          start: startIso,
          end: startIso,
          extendedProps: {
        ...
        if (newEvent && typeof newEvent.setExtendedProp === "function") {
          try { newEvent.setExtendedProp("waId", waId); } catch {}
          try { newEvent.setExtendedProp("wa_id", waId); } catch {}
        }
        ...
        onEventAdded?.({
          ...
          extendedProps: { type: Number(type), cancelled: false, waId, wa_id: waId, reservationId: resp?.id || resp?.reservationId },
        });

        successful.push({ type: "create", id: waId, data: { date: dStr, time: tStr } });
