...
export class CalendarDnDService {
	...
	async handleEventChange(args: {
		...
		const { info, isVacationDate, currentView, updateEvent, resolveEvent } = args;
		...
		try {
			(
				...
				).__calendarLocalMoves as Map<string, number>) || new Map<string, number>();
			(
				(
					globalThis as unknown as {
						__calendarLocalMoves?: Map<string, number>;
			...
			if (evObj) {
				this.localEchoManager.withSuppressedEventChange(() => {
					...
					try {
						...
						if (typeof rid === "number") evObj.setExtendedProp?.("reservationId", rid);
					} catch {}
				});
			}
		} catch {}
