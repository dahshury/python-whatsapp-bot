...
import type { ApiResponse, CalendarEvent, OperationResult, SuccessfulOperation } from "@/entities/event";
...
export class ReservationCancelService {
	...
	async processCancellations(
		...
		for (const rowIdx of deletedRows || []) {
			...
			try {
				...
				const baseTime = ((): string | null => {
					try {
						...
						if (mapped?.extendedProps?.slotTime) {
							const slotTime = String(mapped.extendedProps.slotTime).slice(0, 5);
							console.log(`🔄 Using existing slotTime: ${slotTime} for event ${eventId}`);
							return slotTime;
						}
						// Fallback: use the raw time from the event (don't normalize to avoid slot jumping)
						if (time) {
							console.log(`🔄 Using raw time: ${time} for event ${eventId} on ${date}`);
							return time;
						}
						return null;
					} catch (e) {
						console.error(`❌ Error computing baseTime for event ${eventId}:`, e);
						return null;
					}
				})();
				if (date && baseTime) {
					console.log(`🔄 Reflowing slot ${date} ${baseTime} after cancelling ${eventId}`);
					try {
				...
				if (date && time) {
					this.markLocalEchoForCancellation(resp, mapped, eventId, date, time, waId);
				}
			} catch (e) {
				hasErrors = true;
	...
	private extractWaId(mapped: CalendarEvent): string {
		return (mapped.extendedProps?.waId || mapped.extendedProps?.wa_id || mapped.id || "").toString();
	}
