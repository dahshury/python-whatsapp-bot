...
class ReservationModifyService {
	...
	async processModifications(
		...
		for (const idxStr of indices) {
			...
			try {
				...
				const resp = await this.webSocketService.modifyReservation(modificationData.waId, modifyParams);
				...
				if (!resp?.success) {
					...
					const errorMessage = resp?.message || resp?.error || i18n.getMessage("update_failed", this.isLocalized);
	...
	private prepareModificationData(change: RowChange, original: CalendarEvent) {
		...
		const waId = (original.extendedProps?.waId || original.extendedProps?.wa_id || original.id || "").toString();
		...
		} else if (typeof change.scheduled_time === "string" && change.scheduled_time.includes("T")) {
			const [dPart, tPart] = change.scheduled_time.split("T");
			dateStrNew = dPart as string;
			timeStrNew = this.formattingService.to24h(tPart || prevStartStr.split("T")[1]?.slice(0, 5) || "00:00");
		} else {
			// Backward compatibility
			timeStrNew = this.formattingService.to24h(
				(change as unknown as { time?: string }).time || prevStartStr.split("T")[1]?.slice(0, 5) || "00:00"
			);
			dateStrNew = ((change as unknown as { date?: string }).date || prevDate) as string;
		}
		const typeNew = this.formattingService.parseType(change.type ?? original.extendedProps?.type);
	...
	private applyOptimisticUpdates(modificationData: ReturnType<typeof this.prepareModificationData>): void {
		...
		this.calendarIntegration.updateEventTiming(modificationData.evId, modificationData.prevStartStr, startIso);
	}
	...
	private markLocalEchoForModification(
		...
		const keys = generateLocalOpKeys("reservation_updated", {
			id: resp?.id || original.extendedProps?.reservationId || modificationData.evId,
			wa_id: modificationData.waId,
			date: modificationData.dateStrNew,
			time: modificationData.timeStrNew,
		});
	...
	private handleModificationError(error: Error): void {
		toastService.error(
			...
			error?.message || i18n.getMessage("system_error_try_later", this.isLocalized),
			3000
		);
	}
}
