...
const fromEventDurationSettingsDto = (
	dto: EventDurationSettingsDto | null | undefined
): EventDurationSettings | null => {
	if (!dto) {
		return null
	}
	return {
		strategy: dto.strategy,
		defaultMinutes: dto.default_minutes,
		perTypeMinutes: { ...(dto.per_type_minutes ?? {}) },
	}
}

const toEventDurationSettingsDto = (
	config: EventDurationSettings | null | undefined
): EventDurationSettingsDto | null => {
	if (!config) {
		return null
	}
	return {
		strategy: config.strategy,
		default_minutes: config.defaultMinutes,
		per_type_minutes: { ...config.perTypeMinutes },
	}
}

const fromSlotCapacitySettingsDto = (
	dto: SlotCapacitySettingsDto | null | undefined
): SlotCapacitySettings | null => {
	if (!dto) {
		return null
	}
	const normalizeRole = (role: SlotCapacityRoleDto | undefined) => ({
		totalMax: role?.total_max ?? 0,
		perTypeMax: role?.per_type_max ? { ...role.per_type_max } : {},
	})
	return {
		agent: normalizeRole(dto.agent),
		secretary: normalizeRole(dto.secretary),
	}
}

const toSlotCapacitySettingsDto = (
	config: SlotCapacitySettings | null | undefined
): SlotCapacitySettingsDto | null => {
	if (!config) {
		return null
	}
	return {
		agent: {
			total_max: config.agent.totalMax,
			per_type_max: { ...config.agent.perTypeMax },
		},
		secretary: {
			total_max: config.secretary.totalMax,
			per_type_max: { ...config.secretary.perTypeMax },
		},
	}
}
...
export function appConfigToDomain(dto: AppConfigDto): AppConfig {
	const snapshot: AppConfigSnapshot = {
		...
		availableThemes: ensureList(dto.available_themes, VALID_THEME_VALUES),
		availableCalendarViews: ensureList(
			dto.available_calendar_views,
		...
		eventDurationSettings: fromEventDurationSettingsDto(
			dto.event_duration_settings
		),
		slotCapacitySettings: fromSlotCapacitySettingsDto(
			dto.slot_capacity_settings
		),
...
export function appConfigToDto(config: AppConfig): AppConfigDto {
	...
	return {
		...
		available_themes: snapshot.availableThemes,
		available_calendar_views: snapshot.availableCalendarViews,
		...
		event_duration_settings: toEventDurationSettingsDto(
			snapshot.eventDurationSettings
		),
		slot_capacity_settings: toSlotCapacitySettingsDto(
			snapshot.slotCapacitySettings
		),
...
export function appConfigToUpdateDto(
	...
	if (input.availableThemes) {
		dto.available_themes = input.availableThemes
	}
	if (input.availableCalendarViews) {
		dto.available_calendar_views = input.availableCalendarViews
	}
	if (input.timezone) {
	...
	if (input.eventDurationSettings !== undefined) {
		dto.event_duration_settings = toEventDurationSettingsDto(
			input.eventDurationSettings
		)
	}
	if (input.slotCapacitySettings !== undefined) {
		dto.slot_capacity_settings = toSlotCapacitySettingsDto(
			input.slotCapacitySettings
		)
	}
	return dto
}
