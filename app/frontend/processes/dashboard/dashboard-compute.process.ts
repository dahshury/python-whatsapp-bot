...
export function computeDashboardData(
	...
	try {
		...
		const chattedIdsPeriod = new Set<string>(
			filteredConversationEntries.filter(([, msgs]) => (Array.isArray(msgs) ? msgs.length : 0) > 0).map(([id]) => id)
		);
		const reservedIdsPeriod = new Set<string>(
			filteredReservationEntries.filter(([, items]) => (Array.isArray(items) ? items.length : 0) > 0).map(([id]) => id)
		);
		...
		const prevChattedIds = new Set<string>(
			prevConversationEntries.filter(([, msgs]) => (Array.isArray(msgs) ? msgs.length : 0) > 0).map(([id]) => id)
		);
		const prevReservedIds = new Set<string>(
			prevReservationEntries.filter(([, items]) => (Array.isArray(items) ? items.length : 0) > 0).map(([id]) => id)
		);
		...
		const prom = (globalThis as { __prom_metrics__?: Record<string, unknown> }).__prom_metrics__ || {};
		...
		reservationEntries.forEach(([id, items]) => {
			const hasUpcoming = (Array.isArray(items) ? items : []).some((r) => {
				...
				return d && d > now && (r as Reservation & { cancelled?: boolean }).cancelled !== true;
			});
		...
		const totalCancellations = filteredReservationEntries.reduce(
			(sum, [, items]) =>
				sum +
				(Array.isArray(items)
					? items.filter((r) => (r as unknown as Reservation & { cancelled?: boolean }).cancelled === true).length
					: 0),
			0
		);
		...
		const dashboard: DashboardData = {
			...
			prometheusMetrics: {
				cpu_percent: typeof prom.process_cpu_percent === "number" ? prom.process_cpu_percent : 0,
				memory_bytes: typeof prom.process_memory_bytes === "number" ? prom.process_memory_bytes : 0,
				...
				reservations_cancellation_requested_total: prom.reservations_cancellation_requested_total as number,
				reservations_cancellation_successful_total: prom.reservations_cancellation_successful_total as number,
				reservations_cancellation_failed_total: prom.reservations_cancellation_failed_total as number,
				reservations_modification_requested_total: prom.reservations_modification_requested_total as number,
				reservations_modification_successful_total: prom.reservations_modification_successful_total as number,
				reservations_modification_failed_total: prom.reservations_modification_failed_total as number,
			},
			dailyTrends: [],
			typeDistribution: [],
			timeSlots: [],
			...
			funnelData: [
				{
					stage: "Conversations",
					count: filteredConversationEntries.filter(([, msgs]) => (Array.isArray(msgs) ? msgs.length : 0) > 0).length,
				},
				{
					stage: "Made reservation",
					count: filteredReservationEntries.filter(([, items]) => (Array.isArray(items) ? items.length : 0) > 0).length,
				},
