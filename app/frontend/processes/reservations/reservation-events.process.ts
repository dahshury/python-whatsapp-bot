...
export function getReservationEventProcessor() {
	return {
		generateCalendarEvents(
			...
			const groupMap: Record<string, Array<{ waId: string; r: ReservationItem }>> = {};
			...
			orderedGroups.forEach(([key, arr]) => {
				...
				arr.forEach(({ waId, r }) => {
					try {
						...
						const endTime = addMinutesToClock(baseTime, Math.floor(offsetMinutes + minutesPerReservation));
			...
			if (options.freeRoam && conversationsByUser) {
				Object.entries(conversationsByUser).forEach(([waId, conv]) => {
					...
					const convArr: ConversationItem[] = Array.isArray(convMaybe) ? (convMaybe as ConversationItem[]) : [];
					const convNameFromConv = (() => {
						try {
							const found = convArr.find(
								(m: ConversationItem) => typeof m?.customer_name === "string" && m.customer_name.trim()
							);
					...
					const resArr: ReservationItem[] = Array.isArray(resMaybe) ? (resMaybe as ReservationItem[]) : [];
					const convNameFromRes = (() => {
						try {
							const found = resArr.find(
								(r: ReservationItem) => typeof r?.customer_name === "string" && r.customer_name.trim()
							);
							return (found?.customer_name || "").trim();
						} catch {
							return "";
...
function toSlotBase(dateStr: string, timeStr: string, freeRoam: boolean): string {
	try {
