...
export async function POST(request: Request) {
	try {
		...
		if (!old_id || !new_id) {
			return NextResponse.json({ success: false, message: "Missing required fields: old_id, new_id" }, { status: 400 });
		}
		...
		const backendResponse = await callPythonBackend(`/reservations/${old_id}/modify_id`, {
			method: "POST",
			body: JSON.stringify({
