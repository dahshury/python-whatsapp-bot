...
export async function POST(request: Request) {
	try {
		...
		const pythonResponse = await callPythonBackend<UndoVacationUpdateResponse>("/undo-vacation-update", {
			method: "POST",
			body: JSON.stringify({
		...
		return NextResponse.json({ success: false, error: "Failed to undo vacation update" }, { status: 500 });
	}
}
