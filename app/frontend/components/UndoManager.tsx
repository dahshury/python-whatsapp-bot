        ...
        if (canUndo()) {
          ...
          if (operationToUndo) {
            toast.promise(operationToUndo.execute(), {
              loading: isRTL ? `جاري التراجع: ${operationToUndo.description}...` : `Undoing: ${operationToUndo.description}...`,
              success: (data) => {
                // Assuming execute() returns some success message or data
                return isRTL ? `تم التراجع بنجاح: ${operationToUndo.description}` : `Successfully undid: ${operationToUndo.description}`;
              },
              error: (err) => {
