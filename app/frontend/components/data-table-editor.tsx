...
export function DataTableEditor(props: DataTableEditorProps) {
  ...
  const { refreshData: refreshCustomerData } = useCustomerData()
  ...
  const formatDateRange = () => {
    ...
    if (isRTL) {
      ...
      if (hasTimeInfo && computedEnd &&
        ...
        const dateStr = formatDateRangeWithHijri(startDate, isRTL, undefined, {
          includeTime: false,
        ...
        >
          <div className="px-4 py-1.5 border-b flex flex-row items-center justify-between">
            <div className="flex flex-col space-y-1.5">
              <h2 className={`text-xl font-semibold leading-none tracking-tight py-2 ${isRTL ? "text-right" : "text-left"}`}>
                  ...
                  onDataProviderReady={(provider: any) => {
                    ...
                    configureCustomerAutoFill(
                      (rowIndex: number, customerName: string) => {
                        ...
                        let dataSource = provider.getDataSource?.() || provider.dataSource || provider.getData?.()
                        
                        if (dataSource) {
                          // Try different methods to get columns
                          let columns = dataSource.getColumns?.() || dataSource.columns
                          if (columns) {
                            const nameColumnIndex = columns.findIndex((col: any) => col.id === 'name')
                            // Use provider.columnDefinitions for display order since displayColumns is out of scope here
                            const nameDisplayIndex = provider.columnDefinitions.findIndex((col: any) => col.id === 'name')
                            
                            if (nameColumnIndex >= 0) {
                              // Update the underlying data via provider.setCell
                              const columnDef = columns[nameColumnIndex]
                              const columnType = provider.columnTypeRegistry.get(columnDef.dataType)
                              const newCell = columnType.createCell(customerName, columnDef, provider.theme, provider.isDarkTheme, { row: rowIndex, getRowCellData: () => undefined })
                              provider.setCell(nameColumnIndex, rowIndex, newCell)
                              
                              // Trigger re-render helpers
                              if (provider.editingState?.triggerOnChange) provider.editingState.triggerOnChange()
                              if (provider.refresh) provider.refresh()
                              if (provider.cellCache?.invalidateCell) provider.cellCache.invalidateCell(rowIndex, nameColumnIndex)
                              
                              // Update addedRows cache if applicable
                              const isNewRow = provider.editingState?.addedRows && provider.editingState.addedRows.length > rowIndex
                              if (isNewRow && provider.editingState?.addedRows[rowIndex]) {
                                provider.editingState.addedRows[rowIndex][nameColumnIndex] = customerName
                              }
                              
                              // Repaint cell
                              if (dataEditorRef.current?.updateCells) {
                                dataEditorRef.current.updateCells([{ cell: [nameDisplayIndex >= 0 ? nameDisplayIndex : nameColumnIndex, rowIndex] }])
                              }
                            }
                          }
                        }
                    ...
                    console.log('CustomerAutoFillService configuration complete')
