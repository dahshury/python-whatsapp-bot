				...
				<CardHeader className="text-center space-y-4">
					...
					<div className="space-y-2">
						<CardTitle className="text-lg font-semibold text-foreground">
							{getMessage("backend_connection_error_title", isRTL)}
						</CardTitle>
						<CardDescription className="text-sm text-muted-foreground">
							{getMessage("backend_connection_error_description", isRTL)}
						</CardDescription>
					</div>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="bg-muted/50 p-4 rounded-lg border border-border">
						<p className="text-sm font-medium text-foreground mb-2">
							{getMessage("backend_connection_error_instructions", isRTL)}
						</p>
					...
					>
						{isRetrying ? (
							<>
								...
								{getMessage("backend_connection_checking", isRTL)}
							</>
						) : (
							<>
								<RefreshCw className="w-4 h-4 mr-2" />
								{getMessage("backend_connection_error_retry", isRTL)}
							</>
						)}
					</Button>
				</CardContent>
