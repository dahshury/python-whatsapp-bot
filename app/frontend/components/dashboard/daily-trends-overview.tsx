...
export function DailyTrendsOverview({ dailyTrends, isRTL }: DailyTrendsOverviewProps) {
  ...
  const description = useMemo(() => {
    if (filteredData.length === 0) return i18n.getMessage('chart_no_data', isRTL)
    
    if (timeRange === "all") {
      return i18n.getMessage('chart_showing_all_data', isRTL, { count: filteredData.length })
    } else {
        ...
        <CardContent>
          <div className="h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart 
                key={`${filteredData.length}-${timeRange}`}
                data={filteredData}
              >
                <defs>
                  ...
                  <linearGradient id="fillCancellations" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={THEME_COLORS.secondary} stopOpacity={0.8}/>
                    <stop offset="95%" stopColor={THEME_COLORS.secondary} stopOpacity={0.1}/>
                  </linearGradient>
                </defs>
                ...
                <Tooltip 
                  ...
                  labelStyle={{ fontWeight: 'bold', color: THEME_COLORS.foreground }}
                />
                <Area
                  type="monotone"
