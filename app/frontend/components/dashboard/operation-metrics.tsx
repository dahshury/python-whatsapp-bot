...
function OperationMetricCard({ title, icon, attempts, success, failures, isRTL }: OperationMetricCardProps) {
        ...
        <CardContent className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>{successRate.toFixed(1)}% {i18n.getMessage('operation_success', isRTL)}</span>
              <span>{failureRate.toFixed(1)}% {i18n.getMessage('operation_failed', isRTL)}</span>
            </div>
          </div>
...
export function OperationMetrics({ prometheusMetrics, isRTL }: OperationMetricsProps) {
