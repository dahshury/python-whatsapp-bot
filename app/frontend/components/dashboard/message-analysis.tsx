...
const wordFrequencyChartConfig = {
  customerCount: {
    label: "Customers",
...
export function MessageAnalysis({ 
  ...
  const enhancedWordFrequency = React.useMemo(() => {
    console.log('WordFrequency Debug:', {
      conversationsAvailable: conversations ? Object.keys(conversations).length : 0,
    ...
    if (conversations && Object.keys(conversations).length > 0) {
      ...
      console.log('Enhanced word frequency from conversations:', result.slice(0, 5))
      return result
    }
    ...
    const mockData = [
      ...
      { word: "appointment", customerCount: 32, assistantCount: 28, totalCount: 60 },
      { word: "time", customerCount: 28, assistantCount: 15, totalCount: 43 },
      { word: "available", customerCount: 15, assistantCount: 25, totalCount: 40 },
      { word: "booking", customerCount: 22, assistantCount: 8, totalCount: 30 },
      { word: "schedule", customerCount: 18, assistantCount: 12, totalCount: 30 },
      { word: "confirm", customerCount: 12, assistantCount: 16, totalCount: 28 },
          ...
          <Card>
            ...
            <CardContent>
              <div className="mb-4 text-xs text-muted-foreground bg-muted p-2 rounded">
                Debug: {enhancedWordFrequency.length} words available, showing first 10
                {enhancedWordFrequency.length > 0 && (
                  <div>
                    Sample: {enhancedWordFrequency[0]?.word} ({enhancedWordFrequency[0]?.totalCount})
                    <br />
                    Customer: {enhancedWordFrequency[0]?.customerCount}, Assistant: {enhancedWordFrequency[0]?.assistantCount}
                    <br />
                    Chart data: {JSON.stringify(enhancedWordFrequency.slice(0, 3), null, 2)}
                          </div>
                        )}
                      </div>
              <div 
                    ...
                    <Tooltip 
                      content={({ active, payload, label }) => {
                        if (active && payload && payload.length) {
                          const customerCount = payload.find(p => p.dataKey === 'customerCount')?.value || 0
                          const assistantCount = payload.find(p => p.dataKey === 'assistantCount')?.value || 0
                              ...
                              <div className="space-y-1 text-sm">
                                <div className="flex items-center justify-between gap-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-muted-foreground">{i18n.getMessage('msg_customers', isRTL)}:</span>
                                  </div>
                                ...
                                <div className="flex items-center justify-between gap-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-muted-foreground">{i18n.getMessage('msg_assistant', isRTL)}:</span>
                                  </div>
