...
export function useDialogOverlayPortal() {
  useEffect(() => {
    ...
    const moveToPortal = () => {
      ...
      const forceElementAboveDialog = (element: HTMLElement, reason: string) => {
        // First try to move to portal
        if (element.parentElement !== portalContainer) {
          try {
            console.log('Moving overlay to portal:', reason, element.className || element.tagName)
      ...
      selectors.forEach(selector => {
        ...
        elements.forEach(element => {
          if (element instanceof HTMLElement) {
            ...
            const isInDialog = element.closest('[role="dialog"], [data-radix-dialog-content]')
            const isDialogRelated = element.getAttribute('data-radix-portal') !== null ||
      ...
      absoluteElements.forEach(element => {
        if (element instanceof HTMLElement && element.parentElement !== portalContainer) {
    ...
    observer.observe(document.body, { 
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ['style', 'data-state', 'data-radix-portal', 'data-radix-popper-content-wrapper']
    })
