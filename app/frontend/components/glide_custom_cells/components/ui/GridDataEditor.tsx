...
const useColumnPinning = (
  ...
  const isPinnedColumnsWidthTooLarge = useMemo(() => {
    const pinnedColumnsWidth = columns
      ...
      .reduce((acc, col) => acc + ((col as any).width || minColumnWidth * 2), 0);
...
const useGridSizer = (
  ...
  const contentWidth = displayColumns.reduce((sum, col) => sum + ((col as any).width || 150), 60);
...
export const GridDataEditor: React.FC<GridDataEditorProps> = ({
  ...
  const [internalColumnConfig, setInternalColumnConfig] = useState<Map<string, ColumnConfig>>(
    new Map()
  );
  ...
  }, [width, height, isFullscreen, fullWidth, containerWidth]);
    ...
    (args, draw) => {
      ...
      if ((cell as any).isMissingValue) {
        ...
        const hasContent = (() => {
          ...
          if (cell.kind === GridCellKind.Number) return (cell as any).data !== null && (cell as any).data !== undefined;
    ...
    >
      <Resizable
        ...
        style={{
          ...
          borderRadius: isFullscreen ? "calc(var(--radius) + 4px)" : "var(--radius)",
          overflow: "hidden",
