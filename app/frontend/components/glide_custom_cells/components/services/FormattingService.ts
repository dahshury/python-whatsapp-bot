export class FormattingService {
  ...
  private static formatTime(value: string | Date, format: string): string {
    ...
    if (isNaN(date.getTime())) {
      ...
      return this.parseAndFormatTimeString(String(value), format);
    }
   ...
   */
  private static parseAndFormatTimeString(time: string, format?: string): string {
    if (!time) return '';
   ...
   */
  static parseFormattedTime(formattedTime: string): { hours: number; minutes: number } {
    let hours = 0;
    let minutes = 0;
