export class FormattingService {
	static formatValue(value: any, columnType: string, format: string): string {
		...
		switch (columnType) {
			case "number":
			case "progress":
				return FormattingService.formatNumber(value, format);
			case "date":
				return FormattingService.formatDate(value, format);
			case "time":
				return FormattingService.formatTime(value, format);
			case "datetime":
				return FormattingService.formatDateTime(value, format);
			default:
				return String(value);
		}
	}
	...
	private static formatDate(value: string | Date, format: string): string {
		...
		switch (format) {
			...
			case "distance":
				return FormattingService.formatRelativeTime(date);
			default:
	...
	private static formatTime(value: string | Date, format: string): string {
		const date =
			value instanceof Date ? value : new Date(`2000-01-01T${value}`);
		if (Number.isNaN(date.getTime())) {
			// Try parsing with enhanced format handling
			return FormattingService.parseAndFormatTimeString(String(value), format);
		}
	 ...
	 */
	private static parseAndFormatTimeString(
		time: string,
		format?: string,
	): string {
		if (!time) return "";
	 ...
	 */
	static parseFormattedTime(formattedTime: string): {
		hours: number;
		minutes: number;
	} {
		let hours = 0;
	...
	private static formatDateTime(value: string | Date, format: string): string {
		...
		switch (format) {
			...
			case "distance":
				return FormattingService.formatRelativeTime(date);
			case "calendar":
				return FormattingService.formatCalendarTime(date);
			default:
