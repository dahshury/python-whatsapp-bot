...
export class CustomerAutoFillService {
  ...
  createCustomerSelectHandler(rowIndex: number) {
    return (phone: string, customerName?: string) => {
      console.log('CustomerAutoFillService createCustomerSelectHandler called with:', phone, customerName, 'rowIndex:', rowIndex);
      console.log('CustomerAutoFillService config:', this.config);
      
      if (customerName && this.config.onNameUpdate) {
        console.log('Calling onNameUpdate with rowIndex:', rowIndex, 'customerName:', customerName);
        this.config.onNameUpdate(rowIndex, customerName)
      } else {
        console.log('Not calling onNameUpdate. customerName:', customerName, 'onNameUpdate available:', !!this.config.onNameUpdate);
      }
    }
  }
