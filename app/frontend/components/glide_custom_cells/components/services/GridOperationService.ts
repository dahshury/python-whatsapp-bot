...
export type OperationHandler<T> = (data: T) => Promise<{ success: boolean; result?: any; error?: string }>;
export type UndoHandler<T> = (operation: SuccessfulOperation<T>) => Promise<{ success: boolean; error?: string }>;
...
export class GridOperationService {
   ...
   * Undo an operation
   */
  async undo<T>(operation: SuccessfulOperation<T>): Promise<{ success: boolean; error?: string }> {
    const undoHandler = this.undoHandlers.get(operation.type);
    if (!undoHandler) {
      return { success: false, error: 'No undo handler available for this operation' };
    }
  ...
  async validateBeforeOperation(
    ...
    validator: (cell: GridCell, position: { row: number; col: number }) => { isValid: boolean; error?: string }
  ...
  private showUndoableToast<T>(operation: SuccessfulOperation<T>, duration: number) {
