...
export class TimeRestrictionService {
  ...
  constructor(selectedDate: Date) {
    ...
    console.log('TimeRestrictionService created:', {
      ...
      dayName: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][this.dayOfWeek],
  ...
  validateHour = (hour: number): boolean => {
    console.log(`TimeRestrictionService: Checking hour ${hour} for day ${this.dayOfWeek}`);
    
    if (this.isRamadanPeriod) {
      // During Ramadan: Only 10:00-13:59 is enabled (10 AM to 1:59 PM)
      const isEnabled = hour >= 10 && hour < 14;
      console.log(`Ramadan validation: hour ${hour} -> ${isEnabled ? 'ENABLED' : 'DISABLED'}`);
      return isEnabled;
    }
    ...
    switch (this.dayOfWeek) {
      case 6: // Saturday
        ...
        console.log(`Saturday validation: hour ${hour} -> ${isEnabled ? 'ENABLED' : 'DISABLED'} (16-20 enabled, so 8PM is enabled but 9PM+ disabled)`);
        return isEnabled;
        
      case 5: // Friday
        // No restrictions - all hours enabled
        console.log(`Friday validation: hour ${hour} -> ENABLED (no restrictions)`);
        return true;
        
      case 0: // Sunday
        // Enable ONLY 11:00-15:59 (11 AM to 3:59 PM) - includes 11 AM itself
        const isEnabledSunday = hour >= 11 && hour < 16;
        console.log(`Sunday validation: hour ${hour} -> ${isEnabledSunday ? 'ENABLED' : 'DISABLED'} (11-15 enabled, so 11AM is enabled)`);
      ...
      default: // Monday-Thursday
        ...
        console.log(`Weekday validation: hour ${hour} -> ${isEnabledWeekday ? 'ENABLED' : 'DISABLED'} (12-15 enabled)`);
  ...
  validateMinute = (hour: number, minute: number): boolean => {
    ...
    console.log(`TimeRestrictionService: Checking minute ${minute} in hour ${hour} -> hour is ${hourValid ? 'ENABLED' : 'DISABLED'}`);
    ...
    if (this.isRamadanPeriod) {
      ...
      if (hour === 10 || hour === 13) {
        ...
        console.log(`Ramadan edge hour ${hour}: minute ${minute} -> ${isValidMinute ? 'ENABLED' : 'DISABLED'} (only :00 allowed)`);
      ...
      switch (this.dayOfWeek) {
        case 6: // Saturday: 16:00-20:59 enabled
          ...
          if (hour === 16 || hour === 20) {
            const isValidMinute = minute === 0;
            console.log(`Saturday edge hour ${hour}: minute ${minute} -> ${isValidMinute ? 'ENABLED' : 'DISABLED'} (only :00 allowed)`);
            return isValidMinute;
          }
          break;
        ...
        case 0: // Sunday: 11:00-15:59 enabled
          ...
          if (hour === 11 || hour === 15) {
            const isValidMinute = minute === 0;
            console.log(`Sunday edge hour ${hour}: minute ${minute} -> ${isValidMinute ? 'ENABLED' : 'DISABLED'} (only :00 allowed)`);
            return isValidMinute;
          }
          break;
        ...
        default: // Monday-Thursday: 12:00-15:59 enabled
          ...
          if (hour === 12 || hour === 15) {
            const isValidMinute = minute === 0;
            console.log(`Weekday edge hour ${hour}: minute ${minute} -> ${isValidMinute ? 'ENABLED' : 'DISABLED'} (only :00 allowed)`);
            return isValidMinute;
          }
          break;
      }
...
export const applyTimeRestrictionMonkeyPatch = (customValidator: TimeRestrictionService): void => {
  console.log('Applying time restriction monkey patch for date:', customValidator.selectedDate);
  ...
  try {
    ...
    if (!(globalThis as any).OriginalDisabledTimeRange) {
      ...
      if (!DisabledTimeRangeClass) {
        try {
          ...
          DisabledTimeRangeClass = disableTimeModule.default || disableTimeModule.DisabledTimeRange;
        } catch (importError) {
          console.warn('Could not import DisabledTimeRange directly, will use global patching');
        }
      }
    ...
    const MonkeyPatchedDisabledTimeRange = class {
      ...
      constructor(from: string, to: string) {
        console.log('MonkeyPatchedDisabledTimeRange created with:', { from, to });
        console.log('Using custom validator for date:', customValidator.selectedDate);
        console.log('Validator dayOfWeek:', customValidator.selectedDate.getDay());
...
export const cleanupTimeRestrictionMonkeyPatch = (): void => {
  try {
    if ((globalThis as any).OriginalDisabledTimeRange) {
      ...
      (globalThis as any).DisabledTimeRange = (globalThis as any).OriginalDisabledTimeRange;
      
      // Also try to restore the module export if possible
      try {
        const disableTimeModule = require('react-timekeeper/lib/helpers/disable-time');
        if (disableTimeModule) {
          disableTimeModule.default = (globalThis as any).OriginalDisabledTimeRange;
          if (disableTimeModule.DisabledTimeRange) {
            disableTimeModule.DisabledTimeRange = (globalThis as any).OriginalDisabledTimeRange;
          }
        }
      } catch (moduleError) {
