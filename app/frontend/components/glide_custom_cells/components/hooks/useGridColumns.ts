...
export function useGridColumns(
  ...
  React.useEffect(() => {
    if (!dataSource) {
      ...
      const defaultColumns: GridColumn[] = [
        ...
        { id: "amount", title: "Amount", width: 100, icon: GridColumnIcon.HeaderNumber, themeOverride: { textDark: "#009CA6", textLight: "#009CA6" }, isRequired: true, isEditable: true, hasMenu: true, dataType: "number" } as any,
        { id: "date", title: "Date", width: 120, icon: GridColumnIcon.HeaderDate, isRequired: true, isEditable: true, hasMenu: true, dataType: "date" } as any,
        { id: "time", title: "Time", width: 120, icon: GridColumnIcon.HeaderTime, isRequired: true, isEditable: true, hasMenu: true, dataType: "time" } as any,
      ];
    ...
    const gridColumns: GridColumn[] = columnDefinitions.map((colDef: IColumnDefinition, index: number) => ({
      id: colDef.id,
  ...
  const displayColumns = React.useMemo(() => {
    ...
    visibleColumns.forEach((col, idx) => {
      ...
      if (pinnedColumns.includes(originalIndex)) {
        ...
        const hasExplicitSize = originalCol && 'grow' in originalCol && originalCol.grow === 0;
  ...
  }, [visibleColumns, pinnedColumns, visibleColumnIndices, fullWidth, columnsState]);

  const onColumnResize = React.useCallback((column: GridColumn, newSize: number) => {
    setColumnsState((prev) =>
      prev.map(c => c.id === column.id ? { ...c, width: newSize, grow: 0 } : c)
    );
  }, []);
  ...
  const togglePin = React.useCallback((columnIndex: number) => {
    ...
    console.warn('togglePin is deprecated, use column configuration system instead');
  }, []);
