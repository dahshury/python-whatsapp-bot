...
import { GridSelection } from "../core/types";
...
export function useAdvancedSelection() {
  ...
  const selectRow = useCallback((rowIndex: number, multiSelect: boolean = false) => {
  ...
  const selectColumn = useCallback((columnId: string, multiSelect: boolean = false) => {
    setSelectionState(prev => {
      if (!multiSelect) {
  ...
  const invertSelection = useCallback((totalRows: number, columnIds: string[]) => {
    setSelectionState(prev => {
  ...
  const isColumnSelected = useCallback((columnId: string): boolean => {
  ...
  const handleKeyboardSelection = useCallback((
    ...
    if (event.shiftKey && selectionState.anchorRow !== undefined) {
      if (event.key === "ArrowUp" || event.key === "ArrowDown") {
        event.preventDefault();
        const newRow = event.key === "ArrowUp" 
  ...
  return {
    ...
    gridSelection,
    hasSelection: selectionState.selectedRows.size > 0 || selectionState.selectedColumns.size > 0,
