...
export function useDataLoader(
  ...
  const { debouncedCallback: debouncedLoadRange } = useDebouncedCallback(
    ...
    performanceMode === PerformanceMode.HIGH_PERFORMANCE ? 200 : 100
  );
  ...
  const loadPage = useCallback(async (page: number): Promise<void> => {
    if (loadingRequests.current.has(page) || loaderState.loadedPages.has(page)) {
      return;
    }
    ...
    try {
      ...
      setLoaderState(prev => {
        ...
        return {
          ...
          isLoading: false,
          totalRows: Math.max(prev.totalRows, (page + 1) * fullConfig.pageSize),
        };
      });
    } catch (error) {
  ...
  }, [loadDataPage, fullConfig.pageSize, fullConfig.cacheSize, loaderState.loadedPages]);

  const loadDataRange = useCallback(async (range: DataRange) => {
    const pagesToLoad: number[] = [];
    
    for (let page = range.startPage; page <= range.endPage; page++) {
      if (!loaderState.loadedPages.has(page) && !loadingRequests.current.has(page)) {
        pagesToLoad.push(page);
      }
    }

    // Add prefetch pages
    for (let i = 1; i <= fullConfig.prefetchPages; i++) {
    ...
    const concurrencyLimit = performanceMode === PerformanceMode.HIGH_PERFORMANCE ? 2 : 4;
  ...
  const getRowData = useCallback((rowIndex: number): any | null => {
  ...
  const getRowsInRange = useCallback((startRow: number, endRow: number): (any | null)[] => {
    const rows: (any | null)[] = [];
  ...
  const prefetchAroundRow = useCallback((rowIndex: number) => {
    const page = Math.floor(rowIndex / fullConfig.pageSize);
    const range: DataRange = {
      startRow: Math.max(0, rowIndex - fullConfig.pageSize),
      endRow: Math.min(loaderState.totalRows - 1, rowIndex + fullConfig.pageSize),
  ...
  const reloadPage = useCallback(async (page: number) => {
    setLoaderState(prev => {
      const newCache = new Map(prev.cache);
  ...
  const getLoadedRowCount = useCallback((): number => {
    return loaderState.loadedPages.size * fullConfig.pageSize;
  ...
  const getCacheStats = useCallback(() => {
    return {
      ...
      cacheHitRatio: loaderState.loadedPages.size > 0 
