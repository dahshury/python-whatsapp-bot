...
export interface ValidationOptions {
  ...
  customValidators?: Array<{
    columnId: string;
    validator: (value: any, cell: GridCell, row: number) => { isValid: boolean; error?: string };
  }>;
}
...
export function useGridValidation(
    ...
    if (customValidators.length > 0 && columns.length > 0) {
      ...
      for (let row = 0; row < numRows; row++) {
        for (const column of columns) {
          ...
          if (customValidator) {
            const cellValue = (cell as any).data || (cell as any).displayData || null;
  ...
  }, [dataProviderRef, columns, translateMessage, validateOnlyChanged, customValidatorMap]);
