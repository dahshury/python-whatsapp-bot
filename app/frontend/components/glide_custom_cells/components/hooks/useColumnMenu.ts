...
export function useColumnMenu() {
  ...
  const changeFormat = useCallback((columnId: string, format: string) => {
  ...
  const getPinnedSide = useCallback((columnId: string): "left" | "right" | false => {
  ...
  const getColumnWidth = useCallback((columnId: string): number | undefined => {
    return columnWidths[columnId];
  }, [columnWidths]);

  const getColumnFormat = useCallback((columnId: string): string | undefined => {
