...
import { getDateRestrictions, formatForTempusDominus } from "../../../lib/date-restrictions";
    ...
    editor: (props) => {
      ...
      const handleIconClick = React.useCallback(async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
          ...
          const disabledDateTimes = tempusRestrictions.disabledDates?.map(date => new DateTime(date)) || [];
          ...
          const options = {
            ...
            restrictions: {
              minDate: tempusRestrictions.minDate ? new DateTime(tempusRestrictions.minDate) : undefined,
              maxDate: tempusRestrictions.maxDate ? new DateTime(tempusRestrictions.maxDate) : undefined,
            ...
            localization: {
              locale: "en-GB",
              format: data.format === "time" ? "hh:mm A" : data.format === "date" ? "dd/MM/yyyy" : "dd/MM/yyyy hh:mm A",
          ...
          try {
            showSubscription = instance.subscribe(Namespace.events.show, handleShow);
            changeSubscription = instance.subscribe(Namespace.events.change, handleDateChange);
            hideSubscription = instance.subscribe(Namespace.events.hide, handleHide);
          } catch (error) {
            console.warn("Failed to subscribe to Tempus Dominus events:", error);
          }
