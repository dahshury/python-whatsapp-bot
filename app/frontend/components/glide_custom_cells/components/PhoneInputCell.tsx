...
const PhoneInputEditor: React.FC<import("@glideapps/glide-data-grid").ItemEditorProps<PhoneInputCell>> = (props) => {
  ...
  const parsePhoneToE164ForSelection = React.useCallback((phone: string): string => {
  ...
  const updateCell = React.useCallback((newPhone: string, shouldFinish = false) => {
    console.log('🔥 PhoneInputCell updateCell called with:', newPhone, 'shouldFinish:', shouldFinish);
  ...
  const handleCustomerSelect = React.useCallback((phone: string, customerName?: string) => {
    console.log('PhoneInputCell handleCustomerSelect called with:', phone, customerName);
    ...
    if (data.onCustomerSelect) {
      console.log('Calling data.onCustomerSelect with:', parsedPhone, customerName);
  ...
  const handleChange = React.useCallback((value: string | undefined) => {
    console.log('🔥 PhoneInputCell handleChange called with:', value, 'current phoneValue:', phoneValue);
    ...
    if (value === undefined) {
      ...
      requestAnimationFrame(() => {
        ...
        if (inputs && inputs.length > 0) {
          ...
          console.log('🔥 handleChange: found DOM input value:', currentInputValue);
          ...
          if (newPhone !== phoneValue) {
            console.log('🔥 handleChange: updating phoneValue from', phoneValue, 'to', newPhone);
    ...
    if (newPhone !== phoneValue) {
      console.log('🔥 handleChange: phone value changed from', phoneValue, 'to', newPhone);
      setPhoneValue(newPhone);
      lastKnownValue.current = newPhone;
