...
import { ChevronsUpDown, User, Plus } from 'lucide-react'
...
const PhoneConversationComboboxBase = React.forwardRef<HTMLButtonElement, PhoneConversationComboboxProps>(({
  value = "",
  onChange,
  onBlur,
  onKeyDown,
  ...
  const conversationOptions = React.useMemo(() => {
    return customers.map((customer: any): CustomerOption => {
      return {
        value: customer.phone,
        label: customer.name ? `${customer.name} (${customer.formattedPhone || customer.phone})` : (customer.formattedPhone || customer.phone),
  ...
  const filteredOptions = React.useMemo(() => {
    if (!searchValue) {
      console.log('🔍 No search value, returning all options:', conversationOptions.length);
      return conversationOptions;
    }
    
    const searchLower = searchValue.toLowerCase().trim()
    const normalizedSearch = normalizePhone(searchValue)
    console.log('🔍 Filtering with searchValue:', searchValue, 'normalized:', normalizedSearch);
    ...
    const filtered = conversationOptions.filter((option: CustomerOption) => {
      ...
      if (option.customerName && option.customerName.toLowerCase().includes(searchLower)) {
        return true
      }
    ...
    console.log('🔍 Filtered results:', filtered.length, 'from', conversationOptions.length, 'total options');
    return filtered;
  ...
  const canAddNewCustomer = searchValue.trim() && 
                           !searchedOption && 
  ...
  const handleCustomerSelectByValue = (phoneValue: string) => {
    const option = conversationOptions.find((opt: CustomerOption) => opt.value === phoneValue || opt.formattedPhone === phoneValue)
    if (option && onChange) {
                ...
                <CommandGroup>
                  {filteredOptions.map((option) => (
                    <CommandItem
                      ...
                      className={cn(
                        ...
                        (selectedOption?.value === option.value) && "ring-1 ring-primary ring-offset-1"
                      )}
                    >
                  ...
                  {canAddNewCustomer && (
                    <CommandItem
                      ...
                      onPointerDown={(e) => {
                        console.log('🔍 New customer onPointerDown triggered');
                        e.preventDefault();
                        handleAddNewCustomer();
                      }}
                      onClick={(e) => {
...
export const PhoneConversationCombobox = React.memo(PhoneConversationComboboxBase, (prevProps, nextProps) => {
  return (
