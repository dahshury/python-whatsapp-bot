import { IDataSource, IColumnDefinition, ColumnDataType } from "../interfaces/IDataSource";
...
export class InMemoryDataSource implements IDataSource {
  ...
  private initializeDefaultData(): void {
    const sampleNames = [
      ...
      "Chen Wei", "Anna Kowalski", "David Brown", "Fatima Al-Zahra"
    ];
  ...
  private generateSampleData(row: number, col: number, sampleNames: string[]): any {
    ...
    if (row === 2) {
      switch (col) {
        case 0: return ""; // Empty name
        case 1: return ""; // Empty dropdown
        case 2: return null; // Empty amount
      }
    }
    ...
    switch (col) {
      ...
      case 2: return Math.round((random() * 10000 + 100) * 100) / 100;
  ...
  async getCellData(col: number, row: number): Promise<any> {
    if (row >= 0 && row < this.rowCount && col >= 0 && col < this.columnCount && !this.deletedRows.has(row)) {
  ...
  async addRow(): Promise<number> {
    ...
    for (let col = 0; col < this.columnCount; col++) {
      ...
      newRowData.push(column.defaultValue || this.generateSampleData(newRowIndex, col, ["New User"]));
    }
