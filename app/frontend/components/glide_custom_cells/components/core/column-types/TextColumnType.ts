...
export class TextColumnType implements IColumnType {
  ...
  validateValue(value: any, column: IColumnDefinition): { isValid: boolean; error?: string } {
    ...
    if (column.isRequired && !text.trim()) {
      return { isValid: false, error: messages.validation.thisFieldIsRequired() };
    }
        ...
        switch (rule.type) {
          case "pattern":
            if (rule.value && !new RegExp(rule.value).test(text)) {
              return { isValid: false, error: rule.message || messages.validation.invalidFormat() };
            }
            break;
          case "min":
   ...
   */
  private validateNameWithDetails(text: string): { isValid: boolean; errorMessage?: string } {
    if (!text || text.trim() === "") {
    ...
    const hasInvalidCharacters = words.some(word => !/^[\p{L}-]+$/u.test(word));
    if (hasInvalidCharacters) {
      return { isValid: false, errorMessage: messages.validation.nameInvalidCharacters() };
    }
