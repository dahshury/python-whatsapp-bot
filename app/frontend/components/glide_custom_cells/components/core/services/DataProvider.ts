...
import { IRowContext } from "../interfaces/IColumnType";
...
export class DataProvider implements IDataProvider {
  ...
  public getCell(col: number, row: number): GridCell {
    ...
    if (cachedCell) {
      ...
      const columnType = this.columnTypeRegistry.get(this.columnDefinitions[col]?.dataType);
      if (columnType) {
    ...
    let cell = columnType.createCell(defaultValue, formattedColumn, this.theme, this.isDarkTheme, rowContext);
  ...
  public setCell(col: number, row: number, value: GridCell): void {
    ...
    } else if (!validation.isValid || (column.isRequired && (cellValue === null || cellValue === undefined || cellValue === ""))) {
