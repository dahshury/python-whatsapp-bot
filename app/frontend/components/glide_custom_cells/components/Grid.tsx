...
export default function Grid({ 
  ...
  const [columnConfigMapping, setColumnConfigMapping] = React.useState<Map<string, ColumnConfig>>(
    new Map()
  );
  ...
  const actualIconColor = isUsingExternalTheme 
    ? (isDarkMode ? "#e8e8e8" : "#000000") // Use black in light theme, white in dark theme
    : iconColor;
  ...
  }, [fullWidth]); // Keep fullWidth dependency for consistency
  ...
  }, [columnMenu.columnFormats, isInitializing, isDataReady, displayColumns, filteredRowCount, refreshCells]);
  ...
  const getCellContent = React.useCallback(
    ...
    [filteredRows, baseGetCellContent, displayColumns, theme, darkTheme, dataProvider, columnMenu.columnFormats]
  );
  ...
  const { tooltip, clearTooltip, onItemHovered: onTooltipHover } = useGridTooltips(
    getCellContent,
    displayColumns
  );
  ...
  const calculatedWidth = displayColumns.reduce((sum, col) => sum + ((col as any).width || 150), 60);
  ...
  if (!isDataReady || columnsState.length === 0) {
    ...
    return (
      <FullscreenWrapper theme={theme} darkTheme={darkTheme}>
        <div className={`glide-grid-container ${theme === darkTheme ? 'glide-grid-container-dark' : 'glide-grid-container-light'}`}>
          <div className="flex flex-col items-center gap-4">
            <div className={`glide-loading-spinner ${theme === darkTheme ? 'glide-loading-spinner-dark' : 'glide-loading-spinner-light'}`} />
            <div className={`glide-loading-text ${theme === darkTheme ? 'glide-loading-text-dark' : 'glide-loading-text-light'}`}>
              Loading grid...
            </div>
          </div>
        </div>
    ...
    >
      <GridPortalProvider>
      <div className={`glide-grid-wrapper ${isFullscreen ? 'glide-grid-wrapper-fullscreen' : 'glide-grid-wrapper-centered'}`}>
