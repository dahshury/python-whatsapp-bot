...
import { TimekeeperCell, TimekeeperCellProps } from "./models/TimekeeperCellTypes";
import { useTimekeeperEditor } from "./hooks/useTimekeeperEditor";
import { useClickOutsideIgnore } from "./hooks/useClickOutsideIgnore";
import { formatTimeForDisplay, parseTimeFromPicker, TIME_REGEX_24, TIME_REGEX_12 } from "./utils/timeUtils";
import {
  timekeeperClassNames,
  getTimePickerPortalStyle,
...
const TimeKeeperRenderer = ({ 
  ...
  try {
    ...
    console.log('TimeKeeper restrictions:', { 
      ...
      dateString: data.selectedDate ? data.selectedDate.toDateString() : 'No date',
    ...
    if (timeRestrictionService) {
      ...
      if (dayOfWeek === 6) {
        console.log('Using time restrictions for Saturday - 4PM-8PM enabled, 9PM+ disabled');
      } else if (dayOfWeek === 0) {
...
const renderer: CustomRenderer<TimekeeperCell> = {
  ...
  isMatch: (c): c is TimekeeperCell => (c.data as any).kind === "timekeeper-cell",
