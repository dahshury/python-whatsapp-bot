...
export function createGlideTheme(mode: "light" | "dark"): Partial<Theme> {
  ...
  const background = isServer ? defaults.background : hslToHex('--background', defaults.background);
  ...
  const cardForeground = isServer ? defaults.cardForeground : hslToHex('--card-foreground', defaults.cardForeground);
  const primary = isServer ? defaults.primary : hslToHex('--primary', defaults.primary);
  const primaryForeground = isServer ? defaults.primaryForeground : hslToHex('--primary-foreground', defaults.primaryForeground);
  const secondary = isServer ? defaults.secondary : hslToHex('--secondary', defaults.secondary);
  const secondaryForeground = isServer ? defaults.secondaryForeground : hslToHex('--secondary-foreground', defaults.secondaryForeground);
  const accent = isServer ? defaults.accent : hslToHex('--accent', defaults.accent);
  const accentForeground = isServer ? defaults.accentForeground : hslToHex('--accent-foreground', defaults.accentForeground);
  const muted = isServer ? defaults.muted : hslToHex('--muted', defaults.muted);
  const mutedForeground = isServer ? defaults.mutedForeground : hslToHex('--muted-foreground', defaults.mutedForeground);
  ...
  const popoverForeground = isServer ? defaults.popoverForeground : hslToHex('--popover-foreground', defaults.popoverForeground);
