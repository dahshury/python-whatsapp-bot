...
export const isValidDate = (date: any): date is Date => {
  ...
  if (isNaN(hours) || isNaN(minutes) || isNaN(seconds) || isNaN(milliseconds)) return false;
...
export const formatTimeForPicker = (date: Date, use24Hour: boolean = false): string => {
  try {
    if (!isValidDate(date)) {
...
export const formatTimeForDisplay = (date: Date | undefined, use24Hour: boolean = false): string => {
