...
export function reduceOnMessage(
	...
	switch (type) {
		case 'customer_document_updated': {
			// Document updates are now handled via REST API, not WebSocket
			// Ignore websocket events for documents to prevent duplicate loading
				...
				if (Array.isArray(raw)) {
					const filtered = raw.filter((r) => {
						try {
							...
							if (t === 'conversation_new_message') {
								const role = String(
									(r?.data as { role?: string } | undefined)?.role || ''
								).toLowerCase()
					...
					setTimeout(() => {
						try {
							...
							// Event dispatch failed - notifications history not delivered
						}
					}, 0)
				}
			} catch {
				// Notifications history processing failed - skip this update
			}
			next.lastUpdate = timestamp
			return next
		}
				...
				if (wa) {
					setTimeout(() => {
						try {
							...
							// Event dispatch failed - typing indicator not updated
						}
					}, 0)
				}
			} catch {
				// Typing event processing failed - skip this update
			}
		...
		case 'snapshot': {
			...
			// Reservations and conversations are no longer sent in snapshot
			// They are loaded on-demand via TanStack Query
			// Keep existing state for these (don't overwrite with empty)
				...
				if (wa) {
					setTimeout(() => {
						try {
							...
							// Event dispatch failed - customer profile update not delivered
						}
					}, 0)
				}
			} catch {
				// Customer update processing failed - skip this update
			}
			next.lastUpdate = timestamp
			return next
		}

		case 'customer_block_updated':
		case 'customer_favorite_updated': {
			// Broadcast customer metadata changes (block/favorite status)
			try {
				const d = (data || {}) as {
					wa_id?: string
					is_blocked?: boolean
				...
				if (wa) {
					setTimeout(() => {
						try {
							const evt = new CustomEvent('customers:metadata_updated', {
								detail: {
									wa_id: wa,
							...
							// Event dispatch failed - metadata update not delivered
						}
					}, 0)
				}
			} catch {
				// Customer metadata update processing failed - skip this update
			}
			next.lastUpdate = timestamp
			return next
		}
