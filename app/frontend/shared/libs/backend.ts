...
export type CallPythonBackendOptions = {
	cache?: RequestCache
	timeoutMs?: number
	signal?: AbortSignal
}
...
async function fetchWithTimeout(
	...
	try {
		// Use provided signal if available, otherwise use timeout controller signal
...
export function callPythonBackendCached<T = unknown>(
	...
	const cacheOptions: { revalidate: number; tags?: string[] } = { revalidate }
	if (options?.tags) {
