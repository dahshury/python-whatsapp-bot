class LocalEchoManager {
	 ...
	 */
	storeModificationContext(eventId: string, context: Record<string, unknown>): void {
		try {
			const globalScope = globalThis as GlobalThis;
			globalScope.__calendarLastModifyContext =
				globalScope.__calendarLastModifyContext || new Map<string, Record<string, unknown>>();
	...
	withSuppressedEventChange<T>(callback: () => T): T {
		try {
			const globalScope = globalThis as GlobalThis;
			globalScope.__suppressEventChangeDepth = (globalScope.__suppressEventChangeDepth || 0) + 1;
