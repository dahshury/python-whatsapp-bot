...
import { buildBaseCustomers, mergeCustomerOverlays, sortCustomersByLastMessage } from "@processes/customers";

export interface CustomerDataState {
	customers: Array<{ id: string; name: string; phone?: string }>;
	conversations: Record<string, Array<{ id?: string; text?: string; ts?: string }>>;
	reservations: Record<
		string,
		Array<{ id?: string | number; title?: string; start?: string; end?: string; customer_name?: string }>
	>;
	loading: boolean;
...
export const CustomerDataProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
	...
	const [customerOverrides, setCustomerOverrides] = React.useState<Map<string, { name?: string; phone?: string }>>(
		new Map()
	);
	const [deletedCustomers, setDeletedCustomers] = React.useState<Set<string>>(new Set());
	...
	React.useEffect(() => {
		const handler = (ev: Event) => {
			try {
				...
				const wa = String((d?.wa_id as string) || (d?.waId as string) || (d?.id as string) || "");
				if (!wa) return;
				...
				if (t === "customer_updated" || t === "customer_created") {
					...
					setCustomerOverrides((prev) => {
						...
						const nameAny = (d?.name as string) || (d?.customer_name as string) || undefined;
	...
	const customers = React.useMemo<CustomerDataState["customers"]>(() => {
		const merged = mergeCustomerOverlays(baseCustomers, customerOverrides, deletedCustomers);
	...
	const transformedConversations = React.useMemo(() => {
		const result: Record<string, Array<{ id?: string; text?: string; ts?: string }>> = {};
	...
	const value = React.useMemo(
		() => ({ customers, conversations: transformedConversations, reservations, loading, refresh }),
