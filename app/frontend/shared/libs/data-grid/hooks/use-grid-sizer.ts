...
const GRID_WIDTH_MULTIPLIER = 1.2;
const GRID_WIDTH_EXTRA = 200;
...
export const useGridSizer = ({
...
}: UseGridSizerOptions): GridSizerResult => {
	...
	if (isFullscreen && containerWidth > 0) {
		...
		maxWidth = Math.max(
			gridWidth * GRID_WIDTH_MULTIPLIER,
			gridWidth + GRID_WIDTH_EXTRA
		);
		...
		maxWidth = Math.max(
			contentWidth * GRID_WIDTH_MULTIPLIER,
			contentWidth + GRID_WIDTH_EXTRA
		);
	}
