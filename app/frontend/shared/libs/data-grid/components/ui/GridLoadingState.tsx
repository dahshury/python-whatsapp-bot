	...
	if (showSkeleton) {
		return (
			...
			>
				...
				>
					{Array.from({ length: skeletonColumns }, (_, i) => `header-${i}`).map((key) => (
						<div
							key={`skeleton-${key}`}
							style={{
								height: "32px",
				...
				{/* Body skeleton */}
				{Array.from({ length: skeletonRows }, (_, rowIndex) => `row-${rowIndex}`).map((key) => (
					<div
						key={`skeleton-${key}`}
						style={{
							display: "grid",
						...
						{Array.from({ length: skeletonColumns }, (_, colIndex) => ({
							...
							<div
								...
								style={{
									...
									borderRadius: "4px",
									animation: "pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite",
									animationDelay: `${(Number.parseInt(key.split("-")[1] || "0", 10) + colIndex) * 0.05}s`,
								}}
							/>
						))}
					</div>
...
if (typeof document !== "undefined" && !document.getElementById("grid-loading-animations")) {
