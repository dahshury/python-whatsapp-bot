...
interface GridDataEditorProps {
	...
	onHeaderMenuClick: (column: GridColumn, bounds: { x: number; y: number; width: number; height: number }) => void;
	searchValue: string;
	...
	rowMarkers?:
		...
		| "selection"
		| {
				kind: "checkbox" | "number" | "clickable-number" | "checkbox-visible" | "both" | "none" | "selection";
	...
	const isPinnedColumnsWidthTooLarge = useMemo(() => {
		const pinnedColumnsWidth = columns
			.filter((col, idx) => {
				...
				return columnConfigMapping.get(id)?.pinned === true || columnConfigMapping.get(`col_${idx}`)?.pinned === true;
			})
			.reduce((acc, col) => acc + ((col as { width?: number }).width || minColumnWidth * 2), 0);
...
const useGridSizer = (
	...
	const contentWidth = displayColumns.reduce((sum, col) => sum + ((col as { width?: number }).width || 150), 60);
	...
	const calculatedHeight = isFullscreen ? initialHeight : Math.min(initialHeight, defaultTableHeight);
...
export const GridDataEditor: React.FC<GridDataEditorProps> = ({
	...
	const { isFullscreen, width: fullscreenWidth, height: fullscreenHeight } = useFullscreen();
	const borderWidth = hideOuterFrame ? 0 : 2;

	// Use fullscreen dimensions in fullscreen mode, otherwise use measured dimensions
	const containerWidth = isFullscreen ? fullscreenWidth : measuredContainerWidth || 0;
	const containerHeight = isFullscreen ? fullscreenHeight : measuredContainerHeight || 0;
	...
	const { width, height, minWidth, minHeight, maxWidth, maxHeight, rowHeight, headerHeight } = useGridSizer(
		displayColumns,
		filteredRowCount,
		containerWidth,
		containerHeight,
	...
	const [internalColumnConfig, setInternalColumnConfig] = useState<Map<string, ColumnConfig>>(new Map());

	const effectiveColumnConfig = columnConfigMapping.size > 0 ? columnConfigMapping : internalColumnConfig;
	const setEffectiveColumnConfig = onColumnConfigChange || setInternalColumnConfig;

	// Column pinning management
	const { freezeColumns, pinColumn, unpinColumn, isPinnedColumnsWidthTooLarge } = useColumnPinning(
		displayColumns,
		containerWidth,
		50, // minColumnWidth
		clearSelection,
		...
		(columnIndex: number) => {
			if (dataEditorRef?.current) {
				...
				import("@glideapps/glide-data-grid").then(({ CompactSelection }) => {
					dataEditorRef.current?.remeasureColumns(CompactSelection.fromSingleSelection(columnIndex));
				});
			}
		},
		[dataEditorRef]
	...
	useEffect(() => {
		if (onAutosize) {
			...
			(window as unknown as { gridAutosize?: () => void }).gridAutosize = () => handleAutosize(0);
		}
		return () => {
	...
	const orderedColumns = useMemo(() => {
		...
		displayColumns.forEach((column, index) => {
			...
			const isPinned =
				effectiveColumnConfig.get(id)?.pinned === true || effectiveColumnConfig.get(`col_${index}`)?.pinned === true;
	...
	}, [freezeColumns, isPinnedColumnsWidthTooLarge, orderedColumns, effectiveColumnConfig]);
			...
			if ((cell as { isMissingValue?: boolean }).isMissingValue) {
				...
				const hasContent = (() => {
					if (cell.kind === GridCellKind.Custom) {
						...
						if (data?.kind === "timekeeper-cell") return !!(data as { time?: Date }).time;
					...
					if (cell.kind === GridCellKind.Text) return !!(cell as { data?: unknown }).data;
					if (cell.kind === GridCellKind.Number)
						return (cell as { data?: unknown }).data !== null && (cell as { data?: unknown }).data !== undefined;

					return false;
				})();

				if (
					(column as { isRequired?: boolean; isEditable?: boolean })?.isRequired &&
		...
		(row) => {
			if (row === hoverRow) {
				return {
					...
					bgCellMedium: (theme as Theme & { bgHeaderHovered?: string }).bgHeaderHovered,
				};
			}

			if (row === filteredRowCount) {
				return {
					bgCell: (theme as Theme & { bgHeader?: string }).bgHeader ?? (theme as Theme).bgCell,
					bgCellMedium: (theme as Theme & { bgHeader?: string }).bgHeader ?? (theme as Theme).bgCellMedium,
	...
	const containerClass = fullWidth ? "glide-grid-fullwidth glide-grid-inner-full" : "";
	...
	return (
		...
		>
			<Resizable
				...
				style={{
					border: borderWidth
						...
						: "none",
					borderRadius: hideOuterFrame ? 0 : isFullscreen ? "calc(var(--radius) + 4px)" : "var(--radius)",
					overflow: "hidden",
			...
			>
				<DataEditor
					getCellContent={(cell) => getCellContent(cell) as unknown as GDGGridCell}
					...
					fillHandle={!readOnly}
					{...(!disableTrailingRow ? { trailingRowOptions: { sticky: false, tint: true } } : {})}
