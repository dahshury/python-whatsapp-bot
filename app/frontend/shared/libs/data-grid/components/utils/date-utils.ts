...
export const formatDisplayDate = (date: Date, format?: TempusFormat): string => {
	if (!date) return "";
	switch (format) {
		case "time":
...
export const parseDisplayToDate = (display?: string, format?: TempusFormat): Date | undefined => {
	...
	try {
		...
		if (format === "time") {
			...
			const d = new Date(today.getFullYear(), today.getMonth(), today.getDate(), hours, minutes);
			return Number.isNaN(d.getTime()) ? undefined : d;
		}
		// datetime: expect dd/MM/yyyy hh:mm AM/PM
		const m = s.match(/^(\d{1,2})[/.-](\d{1,2})[/.-](\d{2,4})\s+(\d{1,2}):(\d{2})\s*([AaPp][Mm])$/);
		if (!m || !m[1] || !m[2] || !m[3] || !m[4] || !m[5] || !m[6]) return undefined;
		const day = Number.parseInt(m[1], 10);
		const month = Number.parseInt(m[2], 10) - 1;
		let year = Number.parseInt(m[3], 10);
		if (year < 100) year += 2000;
...
export const getInputType = (format?: TempusFormat): "text" | "time" | "datetime-local" => {
	switch (format) {
		case "time":
			return "time";
		case "datetime":
...
export const getInputValue = (date?: Date, format?: TempusFormat, displayDate?: string): string => {
	let baseDate = date;
...
export const useSetInputFromDate = (ref: React.RefObject<HTMLInputElement>, format?: TempusFormat) =>
	React.useCallback((date: Date) => setInputFromDate(ref.current, date, format), [ref, format]);
