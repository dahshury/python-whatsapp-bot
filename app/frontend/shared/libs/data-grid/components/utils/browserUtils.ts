...
function detectCustomScrollbars(): boolean {
	return (
		(window.navigator.userAgent.includes("Mac OS") && window.navigator.userAgent.includes("Safari")) ||
...
export function setCookie(name: string, value?: string, expiration?: Date): void {
	...
	const w = window as unknown as {
		cookieStore?: {
			set: (opts: { name: string; value: string; expires?: number; path?: string }) => Promise<void>;
		};
	};
	if (w.cookieStore) {
		try {
