...
export function createCSVRow(rowValues: unknown[]): string {
	return rowValues.map((cell) => escapeCSVValue(cell)).join(CSV_DELIMITER) + CSV_ROW_DELIMITER;
}
...
export class FileExporter {
	...
	async exportToCSV(data: unknown[][], headers: string[], filename?: string): Promise<void> {
		...
		try {
			...
			console.warn("Failed to export data as CSV with FileSystem API, trying fallback method", error);
			await this.fallbackCSVExport(data, headers, suggestedName);
		}
	}

	private async fallbackCSVExport(data: unknown[][], headers: string[], filename: string): Promise<void> {
...
export class FileUploader {
	...
	constructor(allowedTypes: string[] = [], maxFileSize: number = 10 * 1024 * 1024) {
