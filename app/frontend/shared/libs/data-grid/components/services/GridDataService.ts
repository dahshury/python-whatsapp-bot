...
export type FilterAndSortRowsParams = {
  searchValue: string;
...
export class GridDataService {
  ...
  private compareValues(
    ...
    const numParse = (v: unknown) => {
      if (typeof v === "string" && PHONE_CHAR_PATTERN.test(v)) {
  ...
  filterAndSortRows({
    ...
    getRawCellContent,
    sortState,
  }: FilterAndSortRowsParams): FilteredRowsResult {
    ...
    for (let row = 0; row < numRows; row += 1) {
      ...
      for (let c = 0; c < displayColumns.length; c += 1) {
