import { type GridCell, GridCellKind, type GridColumn } from "@glideapps/glide-data-grid";
...
export class GridDataService {
	private extractCellValue(cell: GridCell): unknown {
		if (cell.kind === GridCellKind.Number) return (cell as { data?: unknown }).data ?? 0;

		if (cell.kind === GridCellKind.Custom) {
			const data = (cell as { data?: unknown }).data as { kind?: string; value?: unknown; date?: Date } | undefined;
			switch (data?.kind) {
				case "dropdown-cell":
					return data.value ?? "";
				...
				case "phone-cell":
					...
					return data.value ?? "";
				default:
					return (cell as { displayData?: unknown }).displayData ?? (cell as { data?: unknown }).data ?? "";
			}
		}

		return (cell as { data?: unknown }).data ?? (cell as { displayData?: unknown }).displayData ?? "";
	}
