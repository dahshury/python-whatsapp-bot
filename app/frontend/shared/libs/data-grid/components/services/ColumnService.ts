...
export class ColumnService {
	...
	public calculateAutoSize(
		...
		return Math.max(this.MIN_COLUMN_WIDTH, Math.min(finalWidth, this.MAX_COLUMN_WIDTH));
	}

	public updateColumnWidth(columns: GridColumn[], columnIndex: number, newWidth: number): GridColumn[] {
		return columns.map((c, idx) => (idx === columnIndex ? { ...c, width: newWidth } : c));
	}
