...
export const applyTimeRestrictionMonkeyPatch = (customValidator: TimeRestrictionService): void => {
	...
	try {
		...
		if (!(globalThis as { OriginalDisabledTimeRange?: unknown }).OriginalDisabledTimeRange) {
			// Try to access the DisabledTimeRange from react-timekeeper
			let DisabledTimeRangeClass = (globalThis as { DisabledTimeRange?: unknown }).DisabledTimeRange;
			...
			if (DisabledTimeRangeClass) {
				(globalThis as Record<string, unknown>).OriginalDisabledTimeRange = DisabledTimeRangeClass;
			}
		}
		...
		(globalThis as Record<string, unknown>).DisabledTimeRange = MonkeyPatchedDisabledTimeRange;
	...
	try {
		if ((globalThis as Record<string, unknown>).OriginalDisabledTimeRange) {
			...
			try {
				const disableTimeModule = require("react-timekeeper/lib/helpers/disable-time");
				if (disableTimeModule) {
					disableTimeModule.default = (globalThis as Record<string, unknown>).OriginalDisabledTimeRange;
					if (disableTimeModule.DisabledTimeRange) {
						disableTimeModule.DisabledTimeRange = (globalThis as Record<string, unknown>).OriginalDisabledTimeRange;
					}
				}
			} catch (_moduleError) {
