...
export class DataSourceMapper<T = unknown> {
	 ...
	 */
	createObjectMapper<TObject extends Record<string, unknown>>(
		fieldMap: Record<string, keyof TObject | ((item: TObject) => unknown)>
	): MappingConfig<TObject>['getValue'] {
		return (item: TObject, columnId: string) => {
