...
import { ColumnTypeRegistry } from "../core/services/column-type-registry";
...
export type DataSourceMapperOptions = {
	/**
...
export class DataSourceMapper<T = unknown> {
	private readonly columnTypeRegistry = ColumnTypeRegistry.getInstance();
	 ...
	 */
	createObjectMapper<TItem extends Record<string, unknown>>(
		fieldMap: Record<string, keyof TItem | ((item: TItem) => unknown)>
	): MappingConfig<TItem>["getValue"] {
		return (item: TItem, columnId: string) => {
	 ...
	 */
	createDateRangeFilter<TItem>(
		getDate: (item: TItem) => Date | string,
		startDate: Date | string,
		endDate: Date | string
	): MappingConfig<TItem>["filter"] {
	 ...
	 */
	createMultiFieldSorter<TItem>(
		...sortFields: Array<{
			field: (item: TItem) => unknown;
			direction?: "asc" | "desc";
		}>
	): MappingConfig<TItem>["sort"] {
		...
		return (a: TItem, b: TItem) => {
			for (const { field, direction = "asc" } of sortFields) {
				...
				const result = compareFieldValues(aStr, bStr, direction);
				if (result !== undefined) {
					return result;
				}
			}
	 ...
	 */
	createRowIndexMap<TItem>(items: TItem[]): Map<number, TItem> {
