...
import { useClickOutsideIgnore } from "./hooks/use-click-outside-ignore";
import { useTimekeeperEditor } from "./hooks/use-timekeeper-editor";
import type {
	TimekeeperCell,
	TimekeeperCellProps,
...
} from "./styles/timekeeper-styles";
import { TIME_REGEX_12, TIME_REGEX_24 } from "./utils/time-utils";
...
// Time constants for 12-hour/24-hour conversion
const NOON_HOUR = 12;
const PM_HOUR_OFFSET = 12;
const MIDNIGHT_HOUR = 0;

// Helper function to parse 24-hour format time
const parseTime24Hour = (timeStr: string): Date | null => {
...
// Helper function to parse 12-hour format time
const parseTime12Hour = (timeStr: string): Date | null => {
	const match = timeStr.match(TIME_REGEX_12);
...
const TimeKeeperRenderer = ({
	...
	try {
		...
		if (timeRestrictionService) {
			// TODO: Implement time restrictions for weekends (Saturday: 6, Sunday: 0) if needed
		}
		...
		return (
			<TimeKeeper
				...
				onChange={() => {
					// Change is handled by done button click
				}}
...
const renderer: CustomRenderer<TimekeeperCell> = {
	...
	onPaste: (val, data) => {
		...
		const date = TIME_REGEX_24.test(val)
			? parseTime24Hour(val)
			: parseTime12Hour(val);
