import { type CustomCell, type CustomRenderer, drawTextCell, GridCellKind } from "@glideapps/glide-data-grid";
...
const renderer: CustomRenderer<TempusDateCell> = {
	...
	isMatch: (c): c is TempusDateCell => (c.data as { kind?: string }).kind === "tempus-date-cell",
	...
	provideEditor: () => ({
		editor: (props) => {
			...
			const { ensureStyleLoaded, handleIconClick, handleChange, handleBlur, handleKeyDown } = useTempusDominusWidget({
				inputRef,
				wrapperRef,
			...
			return (
				<div ref={wrapperRef} style={wrapperStyle}>
					<input
						ref={inputRef}
						style={editorStyle}
						type={getInputType(data.format as unknown as "date" | "datetime" | "time" | undefined)}
