import { GridCellKind, type GridColumn, type GridSelection } from "@glideapps/glide-data-grid";
				...
				for (let c = 0; c < columns.length; c++) {
					...
					switch (cell.kind) {
						...
						case GridCellKind.Custom: {
							...
							rowVals.push(escapeValue(extendedCell.copyData ?? extendedCell.displayData ?? extendedCell.data ?? ""));
							break;
						}
						default:
	...
	const handleAutosizeColumns = React.useCallback(() => {
		const newCols = columnsState.map((col, idx) => {
			...
			for (const r of visibleRows) {
				...
				const text =
					val.kind === GridCellKind.Text || (val.kind as unknown) === (GridCellKind as { Uri?: string }).Uri
