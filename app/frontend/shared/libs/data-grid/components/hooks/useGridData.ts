...
const generateSampleData = (row: number, col: number): unknown => {
	...
	switch (col) {
		...
		case 3:
			return new Date(2020 + Math.floor(random() * 4), Math.floor(random() * 12), Math.floor(random() * 28) + 1);
		case 4:
			return new Date(1970, 0, 1, Math.floor(random() * 24), Math.floor(random() * 60));
		default:
			return `Cell ${row},${col}`;
	}
};
...
function getInitialCell(
	...
	switch (col) {
		...
		case 4: {
			...
			const use24Hour = Boolean((columnFormats as { use24Hour?: boolean })?.use24Hour) || false;
			const formattedTime =
...
export function useGridData(
	...
	const editingState = React.useRef(new EditingState(initialNumRows, theme, theme === darkTheme));
		...
		(col: number, row: number): GridCell => {
			...
			if (storedCell) {
				...
				if (storedCell.kind === GridCellKind.Number) {
					...
					const format = String(columnFormats?.[key] || columnFormats?.number || "number");
					const formatted = raw !== undefined && raw !== null ? formatNumber(Number(raw), format) : "";
					// @ts-expect-error
				...
				if (storedCell.kind === GridCellKind.Custom && cellWithDateData.data?.kind === "tempus-date-cell") {
					...
					if (date instanceof Date) {
						...
						if (format) {
							...
							if ((cellData as { format?: string }).format === "date") {
								(cellData as { displayDate?: string }).displayDate = date.toLocaleDateString("en-GB");
							} else if ((cellData as { format?: string }).format === "time") {
								(cellData as { displayDate?: string }).displayDate = date.toLocaleTimeString([], {
									hour: "2-digit",
									minute: "2-digit",
								});
							} else {
								(cellData as { displayDate?: string }).displayDate = date.toLocaleString();
							}
							(storedCell as GridCell & { copyData?: string }).copyData =
								(cellData as { displayDate?: string }).displayDate ?? "";
						}
				...
				if (storedCell.kind === GridCellKind.Custom && cellWithTimekeeperData.data?.kind === "timekeeper-cell") {
					...
					if (time instanceof Date) {
						...
						if (format) {
							const formattedValue = FormattingService.formatValue(time, "time", format);
				...
				if (col === 0 && storedCell.kind === GridCellKind.Text) {
					...
					if (!validation.isValid) {
						...
						cellWithValidation.validationError = validation.errorMessage || "Validation error";
					} else {
			...
			if (cell.kind === GridCellKind.Number) {
				...
				const format = String(columnFormats?.[key] || columnFormats?.number || "number");
				// @ts-expect-error
				cell.displayData = raw !== undefined && raw !== null ? formatNumber(Number(raw), String(format)) : "";
			}
			...
			if (col === 0 && cell.kind === GridCellKind.Text) {
				...
				if (!validation.isValid) {
					...
					(cell as { isMissingValue?: boolean; validationError?: string }).validationError =
					...
					if (validation.correctedValue && validation.correctedValue !== data) {
						...
						(cell as { displayData?: unknown }).displayData = validation.correctedValue;
					}
				}
			}
			return cell;
			...
			switch (cell.kind) {
				case GridCellKind.Text: {
					...
					if (col === 0) {
						...
						if (!validation.isValid) {
							hasError = true;
							errorMessage = validation.errorMessage || messages.validation.invalidName();
	...
	const onCellEdited = React.useCallback(
		(visibleRows: readonly number[]) => (cell: Item, newValue: EditableGridCell) => {
