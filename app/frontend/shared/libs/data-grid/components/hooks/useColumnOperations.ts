...
interface UseColumnOperationsProps {
	...
	setColumns: (columns: GridColumn[] | ((prev: GridColumn[]) => GridColumn[])) => void;
...
export const useColumnOperations = ({
	...
	const handleAutosize = React.useCallback(
		(columnId: string) => {
			...
			const calculatedWidth = Math.max(MIN_COLUMN_WIDTH, Math.min(finalWidth, MAX_COLUMN_WIDTH));

			// Set the calculated width
			setColumns((prev) =>
				Array.isArray(prev)
					? prev.map((c, idx) => (idx === actualColIndex ? { ...c, width: calculatedWidth, grow: 0 } : c))
					: prev
			);
		},
		[displayColumns, visibleColumnIndices, filteredRows, getCellContent, setColumns, dataEditorRef?.current]
	);
	...
	const handlePin = React.useCallback(
		(columnId: string, _side: "left" | "right") => {
			...
			const existingConfig = (newMapping.get(configKey) as Record<string, unknown>) || {};
		...
		[displayColumns, columnConfigMapping, setColumnConfigMapping, clearSelection]
	);
