...
function getSortableValue(cell: GridCell): string | number | Date {
	switch (cell.kind) {
		...
		case GridCellKind.Text:
			return String(
				(cell as { displayData?: unknown; data?: unknown }).displayData ?? (cell as { data?: unknown }).data ?? ""
			);
 ...
 */
export function useGridSort(visibleRows: number[], getRawCell: (col: number, row: number) => GridCell) {
