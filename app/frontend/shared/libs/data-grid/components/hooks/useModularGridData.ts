...
type UseModularGridDataOptions = {
...
export function useModularGridData(params: UseModularGridDataOptions) {
		...
		try {
			...
			if (prevEditing) {
				...
				try {
					...
					for (const r of deleted) {
						if (typeof r === 'number' && r >= 0 && r < baseRows) {
							try {
								nextProvider.deleteRow?.(r).catch(() => {
									// Row deletion failed; continue
								})
							} catch {
								// deleteRow call failed; continue
							}
						}
					}
				} catch {
					// getDeletedRows failed; continue
				}
						...
						for (let c = 0; c < colCount; c += 1) {
							...
							if (cell !== undefined) {
								try {
									...
									// setCell failed; continue
								}
							}
						}
					}
				} catch {
					// Cell migration failed; continue
				}
				...
				try {
					...
					for (let r = prevBaseRows; r < prevTotal; r += 1) {
						...
						Promise.resolve(nextProvider.addRow())
							...
							.catch(() => {
								// addRow failed; continue
							})
					}
				} catch {
					// Added row migration failed; continue
				}
			}
		} catch {
			// Editing state migration failed; continue
		}
