import { type EditableGridCell, type GridCell, GridCellKind, type Item, type Theme } from "@glideapps/glide-data-grid";
					...
					for (let r = prevBaseRows; r < prevTotal; r++) {
						...
						Promise.resolve(nextProvider.addRow())
							.then((newIndex: unknown) => {
								const newRowIndex =
									typeof newIndex === "number" ? (newIndex as number) : nextProvider.getRowCount() - 1;
								for (let c = 0; c < colCount; c++) {
									const cell = prevEditing.getCell?.(c, r);
									if (cell !== undefined) {
										try {
