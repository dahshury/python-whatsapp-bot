...
export function useAdvancedTheming() {
	...
	const effectiveTheme = useMemo((): GridThemeConfig => {
		...
		} else if (currentTheme === "dark" || (currentTheme === "auto" && preferences.prefersDarkMode)) {
	...
	useEffect(() => {
		...
		const savedTheme = (typeof savedThemeRaw === "string" ? savedThemeRaw : null) as "light" | "dark" | "auto" | null;
	...
	return {
		...
		updateCustomTheme,
		resetTheme,
		isDarkMode: currentTheme === "dark" || (currentTheme === "auto" && preferences.prefersDarkMode),
...
export function useRowHoverEffect(theme: GridThemeConfig) {
	...
	const onItemHovered = useCallback((args: { kind: string; location: [number, number] }) => {
		if (args.kind === "cell") {
