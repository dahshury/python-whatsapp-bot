...
export function useAdvancedSelection() {
	...
	const { debouncedCallback: debouncedSelectionChange } = useDebouncedCallback((selection: GridSelection) => {
	...
	const handleKeyboardSelection = useCallback(
		(event: KeyboardEvent, currentRow: number, _currentColumn: string, totalRows: number, columnIds: string[]) => {
			...
			if (event.shiftKey && selectionState.anchorRow !== undefined) {
				if (event.key === "ArrowUp" || event.key === "ArrowDown") {
					event.preventDefault();
					const newRow =
						event.key === "ArrowUp" ? Math.max(0, currentRow - 1) : Math.min(totalRows - 1, currentRow + 1);
