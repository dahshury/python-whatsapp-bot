...
function hasTooltip(cell: unknown): cell is { tooltip: string } {
	return (
		!!cell && typeof cell === "object" && cell !== null && typeof (cell as { tooltip?: unknown }).tooltip === "string"
	);
}
...
export function useGridTooltips(
	...
	getBoundsForCell?: (col: number, row: number) => { x: number; y: number; width: number; height: number } | undefined
) {
	...
	const onItemHovered = useCallback(
			...
			if ((args.kind === "header" || args.kind === "cell") && !!args.location) {
				...
				if (args.kind === "header" && column) {
					...
					try {
						...
						if (!tooltipContent && column && !!column.isRequired && !!column.isEditable && isMissingValueCell(cell)) {
		...
		[columns, getCellContent, validationErrors, formatFieldLabel, getBoundsForCell]
	);
