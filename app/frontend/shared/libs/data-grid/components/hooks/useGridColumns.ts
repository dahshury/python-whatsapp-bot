...
export function useGridColumns(
	...
	React.useEffect(() => {
		...
		const gridColumns: GridColumn[] = columnDefinitions.map((colDef: IColumnDefinition, index: number) => {
			...
			const themeOverride = (colDef as { themeOverride?: Record<string, unknown> }).themeOverride;
			return {
				id: colDef.id,
	...
	const displayColumns = React.useMemo(() => {
		...
		visibleColumns.forEach((col, idx) => {
			...
			if (originalIndex !== undefined && pinnedColumns.includes(originalIndex)) {
				pinnedCols.push({
					...col,
					hasMenu: true,
				...
				const originalCol = originalIndex !== undefined ? columnsState[originalIndex] : undefined;
	...
	const onColumnResize = React.useCallback((column: GridColumn, newSize: number) => {
		setColumnsState((prev) => prev.map((c) => (c.id === column.id ? { ...c, width: newSize, grow: 0 } : c)));
	}, []);
