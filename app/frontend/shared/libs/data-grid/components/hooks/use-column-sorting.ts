...
import { toSafeString } from "../utils/general-utils";
...
export function useColumnSorting(
	...
	const getSortDirection = useCallback(
		(columnId: string): SortDirection | null => {
			const sortRule = sortState.rules.find((r) => r.columnId === columnId);
			return sortRule?.direction ?? null;
		},
		[sortState.rules]
	);
	...
	const getSortIndex = useCallback(
		(columnId: string): number =>
			sortState.rules.findIndex((rule) => rule.columnId === columnId),
		[sortState.rules]
	);
