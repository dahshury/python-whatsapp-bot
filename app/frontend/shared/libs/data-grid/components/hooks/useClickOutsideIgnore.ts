...
export const useClickOutsideIgnore = ({
	...
	React.useEffect(() => {
		...
		return () => {
			...
			if (portalRef.current) {
				const observer = (portalRef.current as { _glideOutsideObserver?: MutationObserver })._glideOutsideObserver;
				if (observer) {
					observer.disconnect();
				}
				(
			...
			if (wrapperRef.current) {
				const observer = (wrapperRef.current as { _glideOutsideObserver?: MutationObserver })._glideOutsideObserver;
				if (observer) {
					observer.disconnect();
				}
				(
			...
			if (iconButtonRef.current) {
				const observer = (iconButtonRef.current as { _glideOutsideObserver?: MutationObserver })._glideOutsideObserver;
				if (observer) {
					observer.disconnect();
				}
				(
