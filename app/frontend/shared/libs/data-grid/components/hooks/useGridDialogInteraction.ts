...
interface UseGridDialogInteractionOptions {
	onClose: () => void;
 ...
 */
export function useGridDialogInteraction({ onClose, isFullscreen = false }: UseGridDialogInteractionOptions) {
	...
	const handleEscapeKeyDown = useCallback(
		(e: KeyboardEvent) => {
			...
			const overlayEditor = document.querySelector(".glide-data-grid-overlay-editor");
			if (overlayEditor) {
				e.preventDefault();
				return;
			}
			...
			const phoneCellEditor = document.querySelector(".glide-data-grid-overlay-editor");
			if (phoneCellEditor) {
				e.preventDefault();
				return;
			}
