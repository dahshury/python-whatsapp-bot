...
export function useAdvancedTooltips(config: Partial<TooltipConfig> = {}) {
	const fullConfig = React.useMemo(() => ({ ...DEFAULT_CONFIG, ...config }), [config]);
	...
	const { debouncedCallback: debouncedHide } = useDebouncedCallback(() => setTooltipData(null), fullConfig.hideDelay);

	const calculatePosition = useCallback(
		(mouseX: number, mouseY: number, content: string): { x: number; y: number } => {
	...
	const updateTooltipPosition = useCallback(
		(mouseX: number, mouseY: number) => {
			if (tooltipData?.visible) {
				const newPosition = calculatePosition(mouseX, mouseY, tooltipData.content);
				setTooltipData((prev) => (prev ? { ...prev, position: newPosition } : null));
			}
		},
...
export function useGridTooltips(data: (string | number | undefined)[][], columnNames: string[]) {
	...
	const handleCellHover = useCallback(
		(col: number, row: number, mouseX: number, mouseY: number) => {
			...
			if (cellValue !== undefined && cellValue !== null && cellValue !== "" && columnName) {
...
function formatTooltipContent(value: string | number, columnName: string, row: number, col: number): string {
