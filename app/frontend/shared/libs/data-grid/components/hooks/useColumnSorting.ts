...
export function useColumnSorting(columns: BaseColumnProps[], data: unknown[][], maxSortRules = 3) {
	...
	const toggleSort = useCallback(
		(columnId: string) => {
			setSortState((prev) => {
				const existingRuleIndex = prev.rules.findIndex((rule) => rule.columnId === columnId);
	...
	const compareValues = useCallback((a: unknown, b: unknown, direction: SortDirection): number => {
	...
	const sortedRowIndices = useMemo(() => {
		...
		return Array.from({ length: data.length }, (_, i) => i).sort((indexA, indexB) => {
			for (const rule of sortState.rules) {
				const columnIndex = columnIndices.get(rule.columnId);
				if (columnIndex === undefined || !data[indexA] || !data[indexB]) continue;
