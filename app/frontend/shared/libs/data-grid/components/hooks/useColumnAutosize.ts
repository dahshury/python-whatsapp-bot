...
export function useColumnAutosize(options: ColumnAutosizeOptions = {}) {
	...
	const { minWidth = 50, maxWidth = 400, padding = 16, headerPadding = 20, sampleSize = 100 } = options;

	const getTextWidth = useCallback((text: string, font = "14px Arial"): number => {
	...
	const calculateColumnWidth = useCallback(
		(column: BaseColumnProps, data: unknown[], headerFont = "14px Arial", cellFont = "14px Arial"): number => {
			// Calculate header width
			const headerWidth = getTextWidth(column.name, headerFont) + headerPadding;

			// Get sample data for the column
			const sampleData = data.slice(0, sampleSize);
			const columnData = sampleData.map((row) => (row as Record<string, unknown>)[column.id]);
	...
	const autosizeColumn = useCallback(
		(column: BaseColumnProps, data: unknown[], onWidthChange: (columnId: string, width: number) => void) => {
	...
	const autosizeColumns = useCallback(
		(columns: BaseColumnProps[], data: unknown[], onWidthChange: (columnId: string, width: number) => void) => {
