...
export class TimeColumnType implements IColumnType {
	...
	createCell(
		...
		if (rowContext?.getRowCellData) {
			try {
				...
				console.warn("Error getting date column data for time restrictions:", error);
			}
		}
	...
	getCellValue(cell: GridCell): unknown {
		if (cell.kind === GridCellKind.Custom && (cell as { data?: { kind?: string } }).data?.kind === "timekeeper-cell") {
	...
	validateValue(value: unknown, column: IColumnDefinition): { isValid: boolean; error?: string } {
		if (column.isRequired && !value) {
			return {
				isValid: false,
				error: messages.validation.required(column.title || column.name || "Time"),
			};
		}

		if (value && !(value instanceof Date) && !this.isValidTimeString(String(value))) {
	...
	formatValue(value: unknown, formatting?: IColumnFormatting): string {
		...
		const date = value instanceof Date ? value : this.parseTimeString(String(value));
	...
	getDefaultValue(column: IColumnDefinition): unknown {
		...
		return column.defaultValue ? this.parseTimeString(String(column.defaultValue)) : null;
	}
