...
const validatePhoneNumberLength = (phoneNumber: string): ValidatePhoneNumberLengthResult => {
...
export class PhoneColumnType implements IColumnType {
	...
	getCellValue(cell: GridCell): unknown {
		if (
			...
			(cell as { data?: { kind?: string; value?: unknown } }).data?.kind === "phone-cell"
		) {
		...
		if (phoneNumber.trim()) {
			...
			try {
				...
				if (parsedForLength?.country && parsedForLength.nationalNumber) {
					const lengthValidation = validatePhoneNumberLength(String(parsedForLength.nationalNumber));
	...
	coercePasteValue(value: string, _cell: GridCell, column: IColumnDefinition): GridCell | undefined {
