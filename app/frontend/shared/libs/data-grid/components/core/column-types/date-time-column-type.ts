...
export class DateTimeColumnType implements IColumnType {
	...
	formatValue(value: unknown, formatting?: IColumnFormatting): string {
		...
		const MINUTES_PAD = 2;
		const dd = String(date.getDate()).padStart(MINUTES_PAD, "0");
		const mm = String(date.getMonth() + 1).padStart(MINUTES_PAD, "0");
		const yyyy = date.getFullYear();
		const minutes = String(date.getMinutes()).padStart(MINUTES_PAD, "0");
		const isPM = date.getHours() >= HOURS_IN_HALF_DAY;
		const hour12 = date.getHours() % HOURS_IN_HALF_DAY || HOURS_IN_HALF_DAY;
