...
export class DateColumnType implements IColumnType {
	...
	validateValue(value: unknown, column: IColumnDefinition): { isValid: boolean; error?: string } {
		if (column.isRequired && !value) {
			return {
				isValid: false,
				error: messages.validation.required(column.title || column.name || "Date"),
			};
		}

		if (value && !(value instanceof Date) && (typeof value !== "string" || Number.isNaN(Date.parse(String(value))))) {
				...
				switch (rule.type) {
					case "min":
						if (rule.value !== undefined && rule.value !== null) {
							...
							const minDate =
								rv instanceof Date ? rv : typeof rv === "string" || typeof rv === "number" ? new Date(rv) : undefined;
							if (minDate && date < minDate) {
								return {
									isValid: false,
									error: rule.message || `Date must be after ${minDate.toLocaleDateString()}`,
								};
							}
						}
						break;
					case "max":
						if (rule.value !== undefined && rule.value !== null) {
							...
							if (maxDate && date > maxDate) {
								return {
									isValid: false,
									error: rule.message || `Date must be before ${maxDate.toLocaleDateString()}`,
								};
							}
						}
						break;
	...
	formatValue(value: unknown, formatting?: IColumnFormatting, isLocalized?: boolean): string {
		if (!value) return "";
		...
		const isLocalizedFlag =
			isLocalized === true || (typeof window !== "undefined" && localStorage.getItem("isLocalized") === "true");
