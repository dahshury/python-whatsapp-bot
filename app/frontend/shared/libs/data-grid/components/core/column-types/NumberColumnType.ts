...
export class NumberColumnType implements IColumnType {
	...
	createCell(
		...
		try {
			const isAgeWheel =
				...
				Boolean(column.metadata && (column.metadata as { useWheel?: boolean }).useWheel);
			if (isAgeWheel) {
				...
				const minRule = (column.validationRules || []).find((r) => r.type === "min");
				const maxRule = (column.validationRules || []).find((r) => r.type === "max");
	...
	validateValue(value: unknown, column: IColumnDefinition): { isValid: boolean; error?: string } {
		if (value === null || value === undefined || value === "") {
			// Missing value
			return column.isRequired ? { isValid: false, error: "" } : { isValid: true };
		}
	...
	formatValue(value: unknown, formatting?: IColumnFormatting): string {
		...
		const locale = typeof formatting?.locale === "string" ? formatting.locale : undefined;
		const options = (formatting?.options || {}) as Record<string, unknown>;
		const minFD = typeof options.minimumFractionDigits === "number" ? options.minimumFractionDigits : undefined;
		const maxFD = typeof options.maximumFractionDigits === "number" ? options.maximumFractionDigits : undefined;
		const currency = typeof options.currency === "string" ? options.currency : undefined;
		const precision = typeof options.precision === "number" ? options.precision : undefined;
			...
			editor: (props: {
				...
				try {
					const isAgeWheel =
						...
						(value as { data?: { kind?: string } }).data?.kind === "age-wheel-cell";
					...
					const min = typeof data?.min === "number" ? (data?.min as number) : 10;
					const max = typeof data?.max === "number" ? (data?.max as number) : 120;
