...
export class DropdownColumnType implements IColumnType {
	...
	createCell(
		...
		const options = (Array.isArray(column.metadata?.options) ? (column.metadata?.options as unknown[]) : []).map((v) =>
			String(v)
		);
	...
	getCellValue(cell: GridCell): unknown {
		if (
			...
			(cell as { data?: { kind?: string; value?: unknown } }).data?.kind === "dropdown-cell"
		) {
	...
	validateValue(value: unknown, column: IColumnDefinition): { isValid: boolean; error?: string } {
		if (column.isRequired && !value) {
			return {
				isValid: false,
				error: messages.validation.required(column.title || column.name || "Selection"),
			};
		}

		const options = (
			Array.isArray(column.metadata?.options) ? (column.metadata?.options as unknown[]) : []
		) as unknown[];
