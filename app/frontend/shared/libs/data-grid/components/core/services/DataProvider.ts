...
import type { IColumnDefinition, IDataProvider, IDataSource } from "../interfaces/IDataSource";
...
export class DataProvider implements IDataProvider {
	...
	constructor(dataSource: IDataSource, theme: Partial<Theme>, isDarkTheme: boolean) {
		this.dataSource = dataSource;
		this.theme = theme;
		this.isDarkTheme = isDarkTheme;
		this.columnDefinitions = dataSource.getColumnDefinitions();
		this.editingState = new EditingState(dataSource.rowCount, theme, isDarkTheme);
		this.editingState.setColumnDefinitions(this.columnDefinitions);
		this.columnTypeRegistry = ColumnTypeRegistry.getInstance();
	}

	public setOnCellDataLoaded(callback: (col: number, row: number) => void): void {
	...
	public getCell(col: number, row: number): GridCell {
		...
		const rowContext: IRowContext = {
			...
			getRowCellData: (targetCol: number) => {
				if (this.dataSource.id === "in-memory") {
					...
					if (inMemorySource.data && inMemorySource.data[row] && inMemorySource.data[row][targetCol] !== undefined) {
		...
		if (this.dataSource.id === "in-memory") {
			...
			const inMemorySource = this.dataSource as { data?: unknown[][] };
			if (inMemorySource.data && inMemorySource.data[row] && inMemorySource.data[row][col] !== undefined) {
				const value = inMemorySource.data[row][col];
				const cell = columnType.createCell(value, formattedColumn, this.theme, this.isDarkTheme, rowContext);
		...
		const cell = columnType.createCell(defaultValue, formattedColumn, this.theme, this.isDarkTheme, rowContext);

		// Asynchronously load the actual cell data and update cache
		this.dataSource
			.getCellData(col, row)
			.then((value) => {
				const updatedCell = columnType.createCell(value, formattedColumn, this.theme, this.isDarkTheme, rowContext);
	...
	public setCell(col: number, row: number, value: GridCell): void {
		...
		const updatedCell = columnType.createCell(cellValue, formattedColumn, this.theme, this.isDarkTheme, rowContext);
		...
		} else if (column.isRequired && (cellValue === null || cellValue === undefined || cellValue === "")) {
		...
		try {
			(this as unknown as { onCellDataLoaded?: (c: number, r: number) => void }).onCellDataLoaded?.(col, row);
		} catch {}
	...
	public async addRow(): Promise<number> {
		...
		this.columnDefinitions.forEach((col, idx) => {
			...
			if (columnType) {
				...
				const cell = columnType.createCell(defaultValue, col, this.theme, this.isDarkTheme, rowContext);
