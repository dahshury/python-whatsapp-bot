...
const VIEWPORT_THRESHOLD_PX = 500;
...
export type FormattingMenuProps = {
	...
	onClose: () => void;
	isDarkTheme?: boolean;
	parentTimeoutRef?: { current: NodeJS.Timeout | null };
};
	...
	const menuContent = (
		<div
			...
			style={{
				...
				transformOrigin:
					position.x < VIEWPORT_THRESHOLD_PX ? "top left" : "top right",
			}}
		>
...
function getColumnMetadata(column: GridColumn): { name: string; id: string } {
...
function checkColumnKindByMetadata(
	name: string,
	id: string
): string | undefined {
	if (
...
function determineColumnKind(column: GridColumn): string {
	...
	const { name, id } = getColumnMetadata(column);
	const metadataKind = checkColumnKindByMetadata(name, id);

	return metadataKind || "number";
}
