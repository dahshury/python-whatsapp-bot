...
export function formatDateRangeWithHijri(start?: Date | null, end?: Date | null): string {
	if (!start && !end) return "";
	if (start && !end) return `${formatHijriDate(start)} –`;
	if (!start && end) return `– ${formatHijriDate(end)}`;
