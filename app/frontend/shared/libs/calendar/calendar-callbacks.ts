...
export function createCalendarCallbacks(
	...
	return {
		...
		dateClick: (info: DateClickInfo) => {
			const clickedDate: Date = info?.date instanceof Date ? info.date : new Date(info?.date || Date.now());
			...
			const targetSlotTimes = getSlotTimes(atMidday(dateOnly || ""), freeRoam, viewType);
			...
			const currentSlotTimes = getSlotTimes(atMidday(getDateOnly(currentDateForSlots)), freeRoam, viewType);
			...
			if (isTimeGrid) {
				...
				let startStr: string = info?.dateStr || `${dateOnly}T${targetSlotTimes.slotMinTime}`;
				try {
					const timePart = startStr.split("T")[1] || targetSlotTimes.slotMinTime;
					const timeParts = timePart.split(":").map((v: string) => Number.parseInt(v || "0", 10));
				...
				if (!freeRoam && startDate.getTime() < Date.now()) {
					try {
						...
						const [ch, cm] = timePart.split(":").map((v) => Number.parseInt(String(v || "0"), 10));
						const clickedMinutes =
							(Number.isFinite(ch) ? (ch as number) : 0) * 60 + (Number.isFinite(cm) ? (cm as number) : 0);
						const [minH, minM] = targetSlotTimes.slotMinTime
							.split(":")
							.map((v: string) => Number.parseInt(String(v || "0"), 10));
						const minMinutes =
							(Number.isFinite(minH) ? (minH as number) : 0) * 60 + (Number.isFinite(minM) ? (minM as number) : 0);
