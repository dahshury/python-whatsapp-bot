...
export function getSlotTimes(date: Date, freeRoam: boolean, _view: string) {
	...
	if (isRamadan(date)) return { slotMinTime: "10:00:00", slotMaxTime: "16:00:00" };
	const day = date.getDay(); // 0=Sun..6=Sat
	if (day >= 0 && day <= 4) return { slotMinTime: "11:00:00", slotMaxTime: "17:00:00" };
...
export function isRamadan(date: Date): boolean {
	...
	try {
		const jd = gregorianToJDN(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate());
...
function subtractRamadanFromNormal(normal: BusinessHoursRule[], ramadan: BusinessHoursRule[]) {
	...
	for (const rule of normal) {
		const windowStart = rule.startRecur ? toUTCDate(rule.startRecur) : toUTCDate("2022-01-01");
		const windowEnd = rule.endRecur ? toUTCDate(rule.endRecur) : toUTCDate("2031-12-31");
