...
type PhoneGroupKey = 'selected' | 'favorites' | 'recent' | 'all'
...
export type PhoneGroupingResult<T extends { number: string }> = {
	groups: PhoneGroup<T>[]
	counts: { selected: number; favorites: number; recent: number; all: number }
	ordered: T[]
}
...
export function buildPhoneGroups<T extends IndexedPhoneOption>(
	...
	// Extract favorites (excluding selected)
	const favoriteOptions = uniqueOptions.filter(
		(option) =>
			(option as { is_favorite?: boolean }).is_favorite === true &&
			!selectedSet.has(option.number)
	)
	const favoriteOptionsSorted = sortPhoneOptionsAlphabetically(
		favoriteOptions,
		locale
	)
	const favoriteSet = new Set(
		favoriteOptionsSorted.map((option) => option.number)
	)

	// Remove selected and favorites from recent set
	const recentWithoutSelectedOrFavorites = recentOptions.filter(
		(option) =>
			!(selectedSet.has(option.number) || favoriteSet.has(option.number))
	)
	const recentSetWithoutSelectedOrFavorites = new Set(
		recentWithoutSelectedOrFavorites.map((option) => option.number)
	)
	...
	const remainingOptions = sortPhoneOptionsAlphabetically(
		uniqueOptions.filter(
			(option) =>
				!(
					selectedSet.has(option.number) ||
					favoriteSet.has(option.number) ||
					recentSetWithoutSelectedOrFavorites.has(option.number)
				)
		),
		locale
	)
	...
	orderedOptions.push(
		...favoriteOptionsSorted,
		...recentWithoutSelectedOrFavorites,
		...remainingOptions
	)
	...
	const visibleSelected =
		...
		limitedOrdered.some((opt) => opt.number === selectedOption.number)
			? [selectedOption]
			: []
	const visibleFavorites = limitedOrdered.filter((option) =>
		favoriteSet.has(option.number)
	)
	...
	if (visibleFavorites.length > 0) {
		groups.push({ key: 'favorites', items: visibleFavorites })
	}
	...
	return {
		...
		counts: {
			...
			favorites: favoriteOptionsSorted.length,
			recent: recentWithoutSelectedOrFavorites.length,
