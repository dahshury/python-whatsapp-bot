...
const POSITIVE_TREND_FACTOR = 1.05
const NEGATIVE_TREND_FACTOR = 0.95
const POSITIVE_TREND_FACTOR_SMALL = 1.02
const NEGATIVE_TREND_FACTOR_SMALL = 0.98
const TREND_MIDPOINT_OFFSET = 0.5
const TREND_VARIATION_RANGE = 0.2
const DECIMAL_PLACES_FOR_TOFIXED = 1
const RESPONSE_TIME_MIN_THRESHOLD = 0.9
const RESPONSE_TIME_MAX_THRESHOLD = 0.1
const WEEKLY_DATE_THRESHOLD = 7
const CONVERSION_RATE_MIN_FACTOR = 0.8
const CONVERSION_RATE_RANGE = 0.4
const TREND_DATA_POINTS = 7
const TREND_PROGRESS_FACTOR = 0.5
const VARIATION_AMPLITUDE = 0.05
const VARIATION_FREQUENCY = 0.5
const TREND_DECIMAL_ROUNDING = 10
const FOLLOWUP_TREND_FACTOR_POSITIVE = 1.01
const FOLLOWUP_TREND_FACTOR_NEGATIVE = 0.99
const FOLLOWUP_VOLUME_MIN = 0.9
const FOLLOWUP_VOLUME_RANGE = 0.2
const UNIQUE_CUSTOMERS_TREND_FACTOR_POSITIVE = 1.02
const UNIQUE_CUSTOMERS_TREND_FACTOR_NEGATIVE = 0.98
const UNIQUE_CUSTOMERS_POSITION_VARIATION = 0.15
const TREND_PERCENT_DIVISOR = 100

// Helper to format display value in tooltip
const formatDisplayValue = (value: unknown): string => {
	...
	return value.toFixed(DECIMAL_PLACES_FOR_TOFIXED)
}
...
export function AnalyticsDashboard({
	...
	const generateReservationsChartData = (): Array<{
		...
		if (dailyTrends && dailyTrends.length > 0) {
			return dailyTrends.map((day, index) => ({
				name:
					dailyTrends.length > WEEKLY_DATE_THRESHOLD
			...
			return dailyTrends.map((day, index) => {
				...
				if (reservationRange > 0 && totalReservations > 0) {
					...
					const trendFactor = safeStats.trends?.activeCustomers?.isPositive
						? POSITIVE_TREND_FACTOR
						: NEGATIVE_TREND_FACTOR
					const positionFactor =
						1 +
						(index / (dailyTrends.length - 1) - TREND_MIDPOINT_OFFSET) *
							TREND_VARIATION_RANGE
					...
					const trendFactor = safeStats.trends?.activeCustomers?.isPositive
						? POSITIVE_TREND_FACTOR_SMALL
						: NEGATIVE_TREND_FACTOR_SMALL
					estimatedCustomers =
	...
	const generateConversionRateChartData = (): Array<{
		...
		if (dailyTrends && dailyTrends.length > 0) {
			...
			return dailyTrends.map((day, index) => {
				...
				const estimatedRate =
					baseRate *
					(CONVERSION_RATE_MIN_FACTOR + volumeFactor * CONVERSION_RATE_RANGE) // Vary between 80%-120% of base
				return {
					name:
						dailyTrends.length > WEEKLY_DATE_THRESHOLD
							? new Date(day.date).toLocaleDateString(
	...
	const generateResponseTimeChartData = (): Array<{
		...
		if (dailyTrends && dailyTrends.length > 0) {
			...
			return dailyTrends.map((day, index) => {
				...
				const estimatedTime =
					baseResponseTime *
					(RESPONSE_TIME_MIN_THRESHOLD +
						volumeFactor * RESPONSE_TIME_MAX_THRESHOLD) // Vary between 90%-110% of base
				return {
					name:
						dailyTrends.length > WEEKLY_DATE_THRESHOLD
							? new Date(day.date).toLocaleDateString(
	...
	const generateTrendData = (
		...
		const dataPoints = TREND_DATA_POINTS
		const values: number[] = []
		const totalChange = trend ? trend.percentChange / TREND_PERCENT_DIVISOR : 0

		for (let i = 0; i < dataPoints; i++) {
			// Create a smooth trend from start to end
			const progress = i / (dataPoints - TREND_PROGRESS_FACTOR)
			...
			const variation = Math.sin(i * VARIATION_FREQUENCY) * VARIATION_AMPLITUDE // ±5% smooth variation
			values.push(
				Math.max(
					0,
					Math.round(trendValue * (1 + variation) * TREND_DECIMAL_ROUNDING) /
						TREND_DECIMAL_ROUNDING
				)
			)
		}
		...
		if (dailyTrends && dailyTrends.length > 0) {
			...
			return dailyTrends.map((day, index) => {
				...
				const estimatedUnique =
					safeStats.uniqueCustomers * reservationProportion * dailyTrends.length
				const trendFactor = safeStats.trends?.uniqueCustomers?.isPositive
					? UNIQUE_CUSTOMERS_TREND_FACTOR_POSITIVE
					: UNIQUE_CUSTOMERS_TREND_FACTOR_NEGATIVE
				const positionFactor =
					1 +
					(index / (dailyTrends.length - 1) - TREND_MIDPOINT_OFFSET) *
						UNIQUE_CUSTOMERS_POSITION_VARIATION
				return {
					name:
						dailyTrends.length > WEEKLY_DATE_THRESHOLD
							? new Date(day.date).toLocaleDateString(
		...
		if (dailyTrends && dailyTrends.length > 0) {
			...
			return dailyTrends.map((day, index) => {
				...
				const trendFactor = safeStats.trends?.avgFollowups?.isPositive
					? FOLLOWUP_TREND_FACTOR_POSITIVE
					: FOLLOWUP_TREND_FACTOR_NEGATIVE
				const estimatedFollowups =
					baseFollowups *
					(FOLLOWUP_VOLUME_MIN + volumeFactor * FOLLOWUP_VOLUME_RANGE) *
					trendFactor ** index
				return {
					name:
						dailyTrends.length > WEEKLY_DATE_THRESHOLD
