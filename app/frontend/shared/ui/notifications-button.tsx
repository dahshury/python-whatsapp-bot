...
export function NotificationsButton({
	...
	React.useEffect(() => {
		...
		const fromWs = (ev: Event) => {
			try {
				const detail = (ev as CustomEvent).detail as
					...
					| undefined;
				const list = (detail && Array.isArray(detail.items) ? detail.items : []) as Array<{
					id?: number | string;
					type?: string;
					timestamp?: string | number;
					data?: Record<string, unknown>;
					...
					.map((r) => {
						...
						return {
							...
							text: (() => {
								...
								if (r.type === "reservation_updated" || r.type === "reservation_reinstated")
									return `${i18n.getMessage("toast_reservation_modified", isLocalized)}: ${
										resolveCustomerName(d.wa_id, d.customer_name) || d.wa_id
									} ${d.date ?? ""} ${d.time_slot ?? ""}`;
								if (r.type === "reservation_cancelled")
									return `${i18n.getMessage("toast_reservation_cancelled", isLocalized)}: ${d.wa_id}`;
		...
		try {
			(window as unknown as { __notif_history_requested__?: boolean }).__notif_history_requested__ =
				(window as unknown as { __notif_history_requested__?: boolean }).__notif_history_requested__ || false;
			const already = (
			...
			if (!already) {
				...
				(window as unknown as { __notif_history_requested__?: boolean }).__notif_history_requested__ = true;
			}
		} catch {}
		return () => {
			isCancelled = true;
			window.removeEventListener("notifications:history", fromWs as EventListener);
		};
	...
	const { computedUnreadCount, renderEntries } = React.useMemo(() => {
		...
		for (const it of items) {
			if (it.type === "conversation_new_message") {
				...
				if (!waId) continue;
				const dateStr = String((it.data as { date?: string } | undefined)?.date || "");
		...
		const groupEntries: GroupEntry[] = Array.from(groups.entries()).map(([key, g]) => {
			const waId = String(key.split("|")[0] ?? "");
			const unreadCount = g.items.reduce((acc, n) => acc + (n.unread ? 1 : 0), 0);
			return {
				kind: "group",
				waId,
				date: g.date,
		...
		const nonMsgUnread = nonMessages.reduce((acc, n) => acc + (n.unread ? 1 : 0), 0);
		const groupUnread = groupEntries.reduce((acc, g) => acc + (g.unreadCount > 0 ? 1 : 0), 0);
	...
	const formatTimeAgo = React.useCallback((ts: number) => {
		const now = Date.now();
		const diffSec = Math.max(1, Math.floor((now - ts) / 1000));
		if (diffSec < 60) return i18n.messages.time.justNow();
		const diffMin = Math.floor(diffSec / 60);
		if (diffMin < 60) return i18n.messages.time.minAgo(diffMin);
		const diffHr = Math.floor(diffMin / 60);
		if (diffHr < 24) return i18n.messages.time.hoursAgo(diffHr);
	...
	React.useEffect(() => {
		const handler = (ev: Event) => {
			...
			try {
				const localOps: Set<string> | undefined = (globalThis as { __localOps?: Set<string> }).__localOps;
			...
			const text = (() => {
				if (type === "reservation_created")
					return `${i18n.getMessage("toast_reservation_created", isLocalized)}: ${resolveCustomerName(data.wa_id, data.customer_name) || data.wa_id} ${data.date ?? ""} ${
						data.time_slot ?? ""
					}`;
				if (type === "reservation_updated" || type === "reservation_reinstated")
					return `${i18n.getMessage("toast_reservation_modified", isLocalized)}: ${resolveCustomerName(data.wa_id, data.customer_name) || data.wa_id} ${data.date ?? ""} ${
						data.time_slot ?? ""
					}`;
				...
				if (type === "vacation_period_updated") return i18n.getMessage("toast_vacation_periods_updated", isLocalized);
				return String(type);
			})();
	...
	const handleNotificationClick = React.useCallback((notification: NotificationItem) => {
		...
		try {
			if (notification.type === "conversation_new_message" && notification.data) {
		...
		setItems((prev) =>
			prev.map((n) => {
				if (
					...
					getWaId(n.data) === waId &&
					String((n.data as { date?: string } | undefined)?.date || "") === String(date || "")
				) {
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className="w-80 p-1 shadow-lg" side="bottom" align="end" sideOffset={8} forceMount>
						...
						>
							...
							>
								<div className="flex items-baseline justify-between gap-4 px-3 py-2">
									<div className="text-sm font-semibold">{i18n.getMessage("notifications", isLocalized)}</div>
									{computedUnreadCount > 0 && (
								...
								>
									<motion.div variants={listVariants} initial="hidden" animate="shown">
										{renderEntries.map((entry) => {
													...
													>
														<div className="relative flex items-start pe-3">
															<div className="flex-1 space-y-1">
																...
																>
																	<span className="text-foreground font-medium">{notification.text}</span>
																</button>
															...
															{notification.unread && (
																<div className="absolute end-0 self-center">
																	<span className="sr-only">{i18n.getMessage("unread", isLocalized)}</span>
																	<Dot />
																</div>
															)}
														</div>
											...
											const group = entry;
											const label = i18n.getMessage("new_message", isLocalized);
											...
											const countToShow = group.unreadCount > 0 ? group.unreadCount : group.totalCount;
													...
													<div className="relative flex items-start pe-12">
														<div className="flex-1 space-y-1">
															<button
																type="button"
																className="text-foreground/80 text-left after:absolute after:inset-0"
																onClick={() => handleGroupClick(group.waId, group.date)}
															>
