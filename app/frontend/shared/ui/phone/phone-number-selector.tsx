...
export const PhoneNumberSelectorContent: React.FC<PhoneNumberSelectorBaseProps> = ({
	search,
	setSearch,
	visiblePhones,
	selectedPhone,
				...
				onKeyDown={(e) => {
					if (e.key === "Enter") {
						...
						try {
							...
							const active = root?.querySelector("[cmdk-item][data-selected='true']") as HTMLElement | null;
					...
					{canCreateNew && (
						<div className="p-2">
							...
							>
								...
								<span>
									{i18n.getMessage("phone_add_number_label", isLocalized).replace("{value}", addPreviewDisplay)}
								</span>
							</CommandItem>
						</div>
					)}
					<CommandEmpty>{i18n.getMessage("phone_no_phone_found", isLocalized)}</CommandEmpty>
					<CommandGroup dir="ltr">
						{visiblePhones.map((option) => (
							<CommandItem
								...
								ref={
									selectedPhone === option.number ? (selectedRef as React.RefObject<HTMLDivElement | null>) : undefined
								}
							...
							>
								<div className="flex flex-col space-y-2 min-w-0 flex-1">
									...
									<div className="flex items-center gap-1.5">
										<FlagComponent
											...
											className="opacity-60 scale-75 max-w-full overflow-hidden"
										/>
								...
								{selectedPhone === option.number && <CheckCircle2 className="ms-auto size-4 text-primary" />}
							</CommandItem>
						))}
					</CommandGroup>
				</ThemedScrollbar>
