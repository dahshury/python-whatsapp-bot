...
import { AlertCircle, Bell, ClipboardCheck, FileText, MessageSquareQuote } from "lucide-react";
...
function formatTimeAgo(isLocalized: boolean, ts: number): string {
	...
	if (diffMin < 60) return isLocalized ? `${diffMin} دقيقة` : `${diffMin} min ago`;
...
function NotificationInboxPopover() {
	...
	const filteredEntries = useMemo(
		() =>
			tab === "unread"
				? renderEntries.filter((e) => (e.kind === "item" ? e.item.unread : e.unreadCount > 0))
				: renderEntries,
		[renderEntries, tab]
	);
	...
	const uiEntries = useMemo(() => {
		return filteredEntries.map((entry) => {
			if (entry.kind === "item") {
				...
				const customer = resolveCustomerName(d.wa_id, d.customer_name) || d.wa_id || "";
				...
				const action = (() => {
					if (n.type === "conversation_new_message") return isLocalized ? "أرسل رسالة" : "sent a message";
					if (n.type === "reservation_created") return isLocalized ? "تم إنشاء حجز" : "created reservation";
					if (n.type === "reservation_updated" || n.type === "reservation_reinstated")
						return isLocalized ? "عدّل الحجز" : "updated reservation";
					if (n.type === "reservation_cancelled") return isLocalized ? "ألغى الحجز" : "cancelled reservation";
					if (n.type === "vacation_period_updated") return isLocalized ? "تحديث النظام" : "system update";
					...
					<div className="flex items-center justify-between border-b px-3 py-2">
						<TabsList className="bg-transparent">
							...
							<TabsTrigger value="unread" className="text-sm">
								...
								{computedUnreadCount > 0 && <Badge className="ml-1">{computedUnreadCount}</Badge>}
							</TabsTrigger>
						</TabsList>
						{computedUnreadCount > 0 && (
							<button
							...
							uiEntries.map((n) => {
								...
								return (
									<button
										...
										onClick={() => {
											if (n.kind === "group") {
												...
												setItems((prev) =>
													prev.map((it) => {
														if (
															...
															String((it.data as { date?: string } | undefined)?.date || "") === String(n.date || "")
														)
												...
												try {
													if (n.waId) useSidebarChatStore.getState().openConversation(n.waId);
												} catch {}
												setOpen(false);
												return;
											}
									...
									>
										...
										{n.unread && <span className="mt-1 inline-block size-2 rounded-full bg-primary" />}
									</button>
								);
							})
						)}
