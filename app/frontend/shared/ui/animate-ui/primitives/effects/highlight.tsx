	...
	return (
		<HighlightContext.Provider
			value={{
				...
				forceUpdateBounds:
					(props as ParentModeHighlightProps)?.forceUpdateBounds ?? false,
			}}
		>
			{(() => {
				if (!enabled) {
				...
				return render(
					Children.toArray(children).map((child) => {
						// Use child key if available, otherwise generate stable key from child properties
						const childElement = child as ReactElement;
						const childKey =
							childElement?.key ||
							`${childElement?.type?.toString() || "unknown"}-${Date.now()}-${Math.random()}`;
						return (
							<HighlightItem
								className={itemsClassName}
								key={`highlight-item-${id}-${childKey}`}
							>
								{child}
							</HighlightItem>
						);
...
type HighlightItemProps<T extends ElementType = "div"> =
...
function HighlightItem<T extends ElementType>({
	...
	const commonHandlers = createHighlightHandlers({
		hover,
		click,
		setActiveValue,
		childValue,
	...
	// Helper function to render the animated highlight background
	...
	if (enabled) {
		return (
			<Component
				...
				{...commonHandlers}
			>
				{mode === "children" && renderHighlightBackground()}
