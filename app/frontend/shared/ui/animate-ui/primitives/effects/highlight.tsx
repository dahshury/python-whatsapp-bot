...
function Highlight<T extends React.ElementType = "div">({ ref, ...props }: HighlightProps<T>) {
	...
	const itemsClassName = (props as UncontrolledParentModeHighlightProps | UncontrolledChildrenModeHighlightProps)
		?.itemsClassName;
	...
	return (
		<HighlightContext.Provider
			value={{
				...
				forceUpdateBounds: (props as ParentModeHighlightProps)?.forceUpdateBounds ?? false,
			}}
		>
			{enabled
				? controlledItems
					...
					: render(
							React.Children.toArray(children).map((child) => {
								...
								const childKey =
									childElement?.key || `${childElement?.type?.toString() || "unknown"}-${Date.now()}-${Math.random()}`;
								return (
									<HighlightItem key={`highlight-item-${id}-${childKey}`} className={itemsClassName}>
										{child}
									</HighlightItem>
								);
							})
