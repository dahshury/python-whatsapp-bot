...
export function FiltersMenu({
	...
	return (
		...
		>
			...
			<DropdownMenuContent
				align="start"
				className={cn('w-56', 'click-outside-ignore')}
				side="right"
				sideOffset={8}
			...
			>
				...
				<DropdownMenuSub>
					<DropdownMenuSubTrigger>
						...
						<span>
							{i18n.getMessage('phone_filter_status', isLocalized) || 'Status'}
						</span>
					</DropdownMenuSubTrigger>
					<DropdownMenuSubContent
					...
					>
						<DropdownMenuLabel>
							{i18n.getMessage(
								'phone_filter_status_section_status',
								isLocalized
							) || 'Status'}
						</DropdownMenuLabel>
						<DropdownMenuGroup>
							<DropdownMenuItem
								onSelect={() => handleStatusFilterSelect('registered')}
							>
								<span className="flex-1">
									{i18n.getMessage(
										'phone_filter_status_registered',
										isLocalized
									) || 'Registered'}
								</span>
							...
							<DropdownMenuItem
								onSelect={() => handleStatusFilterSelect('unknown')}
							>
								<span className="flex-1">
									{i18n.getMessage(
										'phone_filter_status_unknown',
										isLocalized
									) || 'Unregistered'}
								</span>
						...
						<DropdownMenuLabel>
							{i18n.getMessage(
								'phone_filter_status_section_moderation',
								isLocalized
							) || 'Moderation'}
						</DropdownMenuLabel>
						<DropdownMenuGroup>
							<DropdownMenuItem
								onSelect={() => handleStatusFilterSelect('blocked')}
							>
								<span className="flex-1">
									{i18n.getMessage(
										'phone_filter_status_blocked',
										isLocalized
									) || 'Blocked'}
								</span>
				...
				<DropdownMenuSub>
					...
					<DropdownMenuSubContent
						className={cn('w-auto p-0', 'click-outside-ignore')}
						style={{ zIndex: 'var(--z-submenu)' }}
					>
						<DropdownMenuSub>
							<DropdownMenuSubTrigger>
