...
import type { StatusFilterValue } from './useStatusFilter'
 ...
 * Supports filtering by country, contact status, and date range.
 */
	...
	const { data, isLoading, error } = useQuery({
		...
		queryFn: async () => {
			...
			if (filters?.status) {
				...
				// Backwards compatibility: legacy backends expect `registration`
				params.append('registration', filters.status)
			}
			if (filters?.dateRange) {
				params.append('date_range_type', filters.dateRange.type)
				if (filters.dateRange.range.from) {
					params.append('date_from', filters.dateRange.range.from.toISOString())
				}
