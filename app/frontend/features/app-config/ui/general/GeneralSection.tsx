...
import { THEME_OPTIONS as SETTINGS_THEME_OPTIONS } from '@/features/settings/settings/theme-data'
...
const THEME_SELECTION_OPTIONS = SETTINGS_THEME_OPTIONS.map((option) => ({
	value: option.value,
	label: option.name,
}))

const THEME_OPTION_LOOKUP = new Map(
	THEME_SELECTION_OPTIONS.map((option) => [option.value, option])
)

function formatThemeLabel(value: string): string {
	// Format theme value by capitalizing first letter and replacing hyphens with spaces
	return value
		.split('-')
...
export const GeneralSection = ({ form, className }: GeneralSectionProps) => {
	...
	return (
		<div className={cn('w-full space-y-4', className)}>
			...
			<Card className="space-y-4 border bg-background/40 p-4">
				<div className="space-y-1">
					<h3 className="font-semibold text-base">Available Themes</h3>
					<p className="text-muted-foreground text-sm">
						Control which visual themes appear inside the in-app theme picker
					</p>
				</div>
				<Controller
					control={form.control}
					...
					render={({ field }) => {
						...
						const orderedValue = THEME_SELECTION_OPTIONS.filter((option) =>
							dedupedValue.includes(option.value)
						).map((option) => option.value)
						const isInvalid = orderedValue.length === 0
						const orderThemes = (values: string[]) =>
							THEME_SELECTION_OPTIONS.filter((option) =>
								values.includes(option.value)
							).map((option) => option.value)

						const handleThemeToggle = (themeValue: string) => {
							if (orderedValue.includes(themeValue)) {
								const next = orderedValue.filter(
									(value) => value !== themeValue
								)
								field.onChange(orderThemes(next))
								return
							}
							field.onChange(orderThemes([...orderedValue, themeValue]))
						}

						return (
							<div className="space-y-2">
								<Popover onOpenChange={setIsThemesOpen} open={isThemesOpen}>
											...
											<div className="flex flex-wrap gap-1">
												{orderedValue.length > 0 ? (
													orderedValue.map((value) => {
														const theme = THEME_OPTION_LOOKUP.get(value)
														return (
															<Badge
																className="mr-1"
																key={value}
																variant="secondary"
															>
																{theme?.label ?? formatThemeLabel(value)}
									...
									<PopoverContent className="w-full p-0">
										<Command>
											...
											<CommandList>
												...
												<CommandGroup>
													{THEME_SELECTION_OPTIONS.map((theme) => {
														const isChecked = orderedValue.includes(theme.value)
														return (
															<CommandItem
																key={theme.value}
																onSelect={(selectedValue) => {
																	handleThemeToggle(selectedValue)
																}}
																value={theme.value}
															>
																<Checkbox
								...
								{isInvalid ? (
									<p className="text-destructive text-xs">
										At least one theme must remain available *
									</p>
								) : (
									<p className="text-muted-foreground text-xs">
										Users only see the themes you keep enabled here.
									</p>
								)}
							</div>
						)
										...
										<CommandList>
											...
											<CommandGroup>
												{LANGUAGE_OPTIONS.map((lang) => {
													...
													const isChecked = normalizedValue.includes(lang.value)
													return (
														<CommandItem
															key={lang.value}
															onSelect={(selectedValue) => {
