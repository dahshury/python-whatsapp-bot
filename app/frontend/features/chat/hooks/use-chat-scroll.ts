...
export function useChatScroll(
	...
	const tryScrollToTarget = React.useCallback(() => {
		...
		for (let i = 0; i < sortedMessages.length; i++) {
			...
			if (targetMsg) {
				const text =
					(m as { message?: string; text?: string }).message || (m as { message?: string; text?: string }).text || "";
		...
		if (foundIndex >= 0) {
			try {
				const el = messageListRef.current?.querySelector(`[data-message-index="${foundIndex}"]`) as HTMLElement | null;
	...
	React.useEffect(() => {
		...
		window.addEventListener("chat:scrollToMessage", onScrollRequest as EventListener);
		return () => window.removeEventListener("chat:scrollToMessage", onScrollRequest as EventListener);
	}, [tryScrollToTarget]);
	...
	React.useEffect(() => {
		...
		const conversationChanged = selectedConversationId !== lastScrolledConversationIdRef.current;
	...
	React.useEffect(() => {
		const handler = (ev: Event) => {
			try {
				...
				if (detail?.type === "conversation_new_message" && detail?.data?.wa_id === selectedConversationId) {
					setTimeout(() => {
