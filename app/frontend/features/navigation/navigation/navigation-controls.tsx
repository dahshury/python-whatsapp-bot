...
export const NavigationControls = React.memo(function NavigationControls({
	...
	const prevButton = (
		<DockIcon widthScale={1.4} paddingPx={0} className="transition-colors" {...prevHoldHandlers}>
			<Tooltip>
				<TooltipTrigger asChild>
					<Button
						...
						onClick={onPrev}
						disabled={isCalendarPage && isPrevDisabled}
						className={`${buttonHeight} w-full rounded-theme transition-all duration-200 flex items-center justify-center
	...
	const nextButton = (
		<DockIcon widthScale={1.4} paddingPx={0} className="transition-colors" {...nextHoldHandlers}>
			<Tooltip>
				<TooltipTrigger asChild>
					<Button
						variant="ghost"
