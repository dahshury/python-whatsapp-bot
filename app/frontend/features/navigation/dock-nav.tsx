...
import {
	type CalendarCoreRef,
	useCalendarViewOptionsFiltered,
...
export function DockNav({
	...
	const { options: calendarViewOptions } =
		useCalendarViewOptionsFiltered(isLocalized)
	...
	const onCtrlUp = useCallback(() => {
		try {
			if (calendarViewOptions.length === 0) {
				return
			}
			const current = nav.navigation.activeView || currentCalendarView
			const index = calendarViewOptions.findIndex((o) => o.value === current)
			const nextIndex =
				(index - 1 + calendarViewOptions.length) % calendarViewOptions.length
			nav.handlers.handleCalendarViewChange(
				calendarViewOptions[nextIndex]?.value || 'multiMonthYear'
			)
		} catch {
	...
	const onCtrlDown = useCallback(() => {
		try {
			...
			const current = nav.navigation.activeView || currentCalendarView
			const index = calendarViewOptions.findIndex((o) => o.value === current)
			const nextIndex = (index + 1) % calendarViewOptions.length
			nav.handlers.handleCalendarViewChange(
				calendarViewOptions[nextIndex]?.value || 'multiMonthYear'
			)
		} catch {
