    ...
    <div className="relative rounded-md border p-2">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center">
          ...
          <Button
            aria-label={i18n.getMessage("vacation_delete_period", isLocalized)}
            className="h-8 w-8"
            onClick={onRemove}
            size="icon"
            variant="ghost"
      ...
      <div className="grid grid-cols-2 gap-2">
        {/* Start Date */}
        <div
          className={`relative flex items-center justify-between rounded-md border p-2 ${isRecordingStart ? "ring-2 ring-destructive/40" : ""}`}
        >
          {/* Recording overlay */}
          {isRecordingStart && (
        ...
        {/* End Date */}
        <div
          className={`relative flex items-center justify-between rounded-md border p-2 ${isRecordingEnd ? "ring-2 ring-destructive/40" : ""}`}
        >
          {/* Recording overlay */}
          {isRecordingEnd && (
            <span className="absolute top-1 right-2 animate-pulse select-none font-medium text-[0.625rem] text-destructive">
    ...
    <div className="space-y-2">
      ...
      {upcoming.length === 0 ? (
        ...
        upcoming.map(({ period, originalIndex }, index) => {
          ...
          // Calculate days count: duration in milliseconds / milliseconds per day
...
export const VacationPeriods = memo(VacationPeriodsComponent);
