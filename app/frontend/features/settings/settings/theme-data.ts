...
export const getThemeNameLocalized = (
	...
	const theme = THEME_OPTIONS.find((t) => t.value === value)
	if (!theme) {
		return i18n.getMessage('theme_unknown', isLocalized)
	}
	return isLocalized ? theme.nameRTL || theme.name : theme.name
}

export const filterThemeOptions = (
	...
	const filtered = THEME_OPTIONS.filter((option) =>
		allowedSet.has(option.value)
	)
	return filtered.length > 0 ? filtered : THEME_OPTIONS
}
