	...
	return {
		generateCalendarEvents(
			...
			for (const [key, arr] of orderedGroups) {
				...
				let offsetMinutes = 0
				// Fixed duration per event = slot_duration / max_events_per_slot (2hr / 5 = 20m)
				const SLOT_DURATION_MIN = Math.max(60, (SLOT_DURATION_HOURS || 2) * 60)
				const MAX_EVENTS_PER_SLOT = 5
				const FALLBACK_MINUTES_PER_RESERVATION = 20
				const minutesPerReservation =
					Math.floor(SLOT_DURATION_MIN / MAX_EVENTS_PER_SLOT) ||
					FALLBACK_MINUTES_PER_RESERVATION
				const gapMinutes = 1
			...
			if (options.freeRoam && conversationsByUser) {
				for (const [waId, conv] of Object.entries(conversationsByUser)) {
					...
					// Calculate end time: 120 minutes divided by 6 (20 minutes per slot)
					const DURATION_MINUTES = 120
					const DIVISOR = 6
					const endTime = addMinutesToClock(
						baseTime,
						Math.floor(DURATION_MINUTES / DIVISOR)
					)
