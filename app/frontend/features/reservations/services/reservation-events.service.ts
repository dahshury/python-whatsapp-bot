  ...
  return {
    generateCalendarEvents(
      ...
      for (const [key, arr] of orderedGroups) {
        ...
        let offsetMinutes = 0;
        // Fixed duration per event = 20 minutes
        const MINUTES_PER_RESERVATION = 20;
        const minutesPerReservation = MINUTES_PER_RESERVATION;
        const gapMinutes = 1;
      ...
      if (options.freeRoam && conversationsByUser) {
        for (const [waId, conv] of Object.entries(conversationsByUser)) {
          ...
          // Calculate end time: 20 minutes per conversation event
