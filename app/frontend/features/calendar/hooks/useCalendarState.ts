...
export function useCalendarState(
	...
	// Use a ref to track the current view to avoid stale closures
	// Initialize with the current view value
	const currentViewRef = useRef<string>(currentView)

	// Track the previous initialViewFromStorage to detect when it actually changes
	const prevInitialViewRef = useRef<string>(initialViewFromStorage)

	// Only sync when initialViewFromStorage changes (e.g., config reload, localStorage external change)
	// Do NOT sync when user manually changes view
	useEffect(() => {
		if (
			initialViewFromStorage &&
			initialViewFromStorage !== prevInitialViewRef.current
		) {
			prevInitialViewRef.current = initialViewFromStorage
			currentViewRef.current = initialViewFromStorage
	 ...
	 * Uses a ref to access the latest view value without adding it to dependencies
	 * Updates the ref immediately to prevent race conditions
	 */
