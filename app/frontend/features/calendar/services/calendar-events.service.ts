...
export async function orchestrateCalendarDrag(params: {
	...
	const fallbackDate = startIso.split('T')[0] || ''
	const newDate =
		formattingService.formatDateOnly(startDate ?? startIso) || fallbackDate
	const fallbackTime =
		startIso.split('T')[1]?.slice(0, SLOT_TIME_PREFIX_LENGTH) || '00:00'
	const localTime =
		formattingService.formatHHmm(startDate ?? startIso) ||
		formattingService.to24h(fallbackTime)
	const newTime = normalizeToSlotBase(newDate, localTime)
	...
	if (info?.oldEvent?.start instanceof Date) {
		previousStart = info.oldEvent.start
	} else if (previousStartStr) {
		previousStart = new Date(previousStartStr)
	}
	const fallbackPreviousDate = previousStartStr.split('T')[0] || newDate
	const previousDate =
		formattingService.formatDateOnly(previousStart ?? previousStartStr) ||
		fallbackPreviousDate
	const fallbackPreviousTime =
		previousStartStr.split('T')[1]?.slice(0, SLOT_TIME_PREFIX_LENGTH) ||
		localTime
	const previousTimeRaw =
		formattingService.formatHHmm(previousStart ?? previousStartStr) ||
		formattingService.to24h(fallbackPreviousTime)
	const previousSlotTime = previousTimeRaw
