...
import {
  DURATION_PER_RESERVATION_FEW,
  DURATION_PER_RESERVATION_MANY,
  GAP_MINUTES,
  MANY_EVENTS_THRESHOLD,
} from "./constants";
import { getWindowProperty, setWindowProperty } from "./window-utils";

export function reflowSlot(
  api: { getEvents?: () => unknown[] },
  dateStr: string,
  baseTime: string
): void {
  try {
    ...
    const minutesPerReservation =
      inSlot.length >= MANY_EVENTS_THRESHOLD
        ? DURATION_PER_RESERVATION_MANY
        : DURATION_PER_RESERVATION_FEW;
    ...
    for (const ev of inSlot) {
      ...
      const totalStartMinutes =
        baseHours * 60 + baseMinutes + Math.floor(offset);
      ...
      try {
        (ev as unknown as EventApi).setExtendedProp("slotDate", dateStr);
      } catch {
        /* noop */
      }
      try {
        (ev as unknown as EventApi).setExtendedProp("slotTime", baseTime);
      } catch {
        /* noop */
      }
