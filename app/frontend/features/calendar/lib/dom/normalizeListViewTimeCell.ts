...
export function normalizeListViewTimeCell(rowEl: HTMLElement | null) {
  if (!rowEl) return
  const timeCell = rowEl.querySelector('.fc-list-event-time') as HTMLElement | null
  if (!timeCell) return
  ...
  const cleanupTextNodes = () => {
    for (const node of Array.from(timeCell.childNodes)) {
      if (node.nodeType === 3) {
        ...
        if (text) timeCell.removeChild(node)
      }
    }
  }
  ...
  if (!timeCell.hasAttribute('data-observer-attached')) {
    ...
    ;(timeCell as any)._timeNormalizerObserver = observer
  }
}
