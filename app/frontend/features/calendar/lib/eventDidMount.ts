...
interface EventDidMountOptions {
  eventApi: any
  el: HTMLElement
  viewType: string
  ...
  onEventDidMount?: (info: {
    event: { id: string; title: string; start: Date; end?: Date; extendedProps?: Record<string, unknown> }
    el: HTMLElement
  }) => void
}

export function eventDidMountHandler(opts: EventDidMountOptions) {
  const { eventApi, el, viewType, onEventMouseDown, onContextMenu, onEventDidMount } = opts
  ...
  try {
    if (String(viewType || '').toLowerCase().includes('list')) {
  ...
  // Call-through
  if (onEventDidMount) {
    const safeStart = eventApi.start ?? (eventApi.startStr ? new Date(eventApi.startStr) : new Date())
    onEventDidMount({
      event: {
