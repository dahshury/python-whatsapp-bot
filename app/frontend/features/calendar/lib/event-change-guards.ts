...
function ensureDedupMap(): Map<string, number> {
  let map = getWindowProperty(
    DEDUP_MAP_KEY,
    null as Map<string, number> | null
  );
  if (!map) {
...
export type EventChangeInfo =
  | EventChangeArg
  | { event: EventApi; draggedEl: HTMLElement };
...
export function getEventChangeDedupKey(info: EventChangeInfo): string {
  try {
    ...
    const startStr =
      (ev?.startStr as string | undefined) ||
      (ev?.start instanceof Date ? ev.start.toISOString() : "");
 ...
 */
export function suppressDuplicateEventChange(
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          typeof (info as { revert?: () => void }).revert === "function"
        ) {
          (info as { revert?: () => void }).revert?.();
        }
        return true;
