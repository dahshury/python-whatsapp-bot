...
describe("ReservationCacheSynchronizer", () => {
  ...
  it("updates cached reservations on reservation update events", () => {
    ...
    const updatedDay = queryClient.getQueryData<
    ...
    const updatedMonth = queryClient.getQueryData<
      Record<string, TestReservation[]>
    >(["calendar-reservations", "2025-11", false]);
  ...
  it("marks cancellations only for caches that include cancelled reservations", () => {
    ...
    queryClient.setQueryData(["calendar-reservations", "2025-12-24", false], {
      "5678": [reservation],
    });
    queryClient.setQueryData(["calendar-reservations", "2025-12-24", true], {
      "5678": [reservation],
    });
