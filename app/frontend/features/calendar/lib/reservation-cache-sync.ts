...
export class ReservationCacheSynchronizer {
	...
	handle(event: ReservationRealtimeEvent | undefined): boolean {
		if (!event || !ReservationCacheSynchronizer.SUPPORTED_TYPES.includes(event.type)) {
			return false
		}
		...
		if (isLocalOperation(event.type, payload)) {
			console.log('[ReservationCacheSync] Skipping local echo', {
				type: event.type,
				id: payload.id,
				wa_id: payload.wa_id || payload.waId,
				date: payload.date,
			})
	...
	private buildReservation(params: {
		...
		const resolvedTypeRaw = payload.type ?? (base.type as number | string | undefined)
		...
		delete (baseReservation as { id?: unknown }).id
	...
	private syncReservation(action: Extract<ReservationCacheAction, { kind: 'sync' }>): void {
		const queries = this.queryClient.getQueriesData<Record<string, Reservation[]>>({
			queryKey: ['calendar-reservations'],
		})

		console.log('[ReservationCacheSync] syncReservation called', {
		...
		for (const [key, data] of queries) {
			...
			if (changed) {
				console.log('[ReservationCacheSync] Updating cache', {
					queryKey: key,
					reservationId: action.id,
					customerId: action.waId,
				})
	...
	private purgeByReservationId(reservationId: string): void {
		...
		for (const [key, data] of queries) {
			...
			const { updated, changed } = this.removeReservationFromDataset(data, reservationId)
			if (changed) {
				this.queryClient.setQueryData(key, updated)
			}
		}
	...
	private applyReservationToDataset(params: {
		...
		const { updated: removed, removedReservation, changed: removalChanged } =
		...
		if (descriptor.kind === 'period') {
			...
			if (isCancellation) {
				if (descriptor.freeRoam) {
					changed = this.insertReservation(removed, {
						...baseline,
						cancelled: true,
					}) || changed
				}
	...
	private insertReservation(
		...
		const next = Array.isArray(data[customerId])
			? [...data[customerId]]
			: []
	...
	private resolveWaId(payload: ReservationRealtimePayload): string | null {
		const wa = payload.wa_id ?? payload.waId ?? payload.customer_id ?? payload.customerId
		return typeof wa === 'string' && wa.trim().length > 0 ? String(wa) : null
	}

	private resolveReservationId(payload: ReservationRealtimePayload): string | null {
	...
	private resolveDescriptor(key: QueryKey): PeriodDescriptor | null {
		...
		if (key[1] === 'legacy') {
			...
			const fromDate =
				typeof key[4] === 'string' && key[4] ? this.parseDateOnly(key[4]) : null
			const toDate =
	...
	private matchesReservationId(
		...
		if (typeof (reservation as { reservation_id?: number }).reservation_id === 'number') {
			return (
