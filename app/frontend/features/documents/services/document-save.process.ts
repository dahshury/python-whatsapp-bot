...
export async function saveDocumentOnce(
  ...
  try {
    ...
    return {
      ...
      normalizedBody: body, // ✅ OPTIMIZATION: Return normalized body to avoid duplicate normalization
      ...res,
    } as SaveResult;
  } catch (e) {
...
export function createIdleAutosaveController(
  ...
  async function flush(
    ...
    // ✅ FIX: Use payload.sig if provided (avoids redundant computation), otherwise compute
    const combinedSig = payload.sig || computeCombinedSignature(payload);
    if (!combinedSig) {
      return null;
    }
    ...
    if (res?.success) {
      ...
      try {
        // ✅ OPTIMIZATION: Reuse normalized body from saveDocumentOnce
        const scene =
          res.normalizedBody ||
          normalizeForPersist({
            elements: payload.elements,
