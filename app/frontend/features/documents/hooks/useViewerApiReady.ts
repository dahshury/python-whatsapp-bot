        ...
        if (pendingViewerInitRef.current) {
          ...
          const applyScene = () => {
            ...
            try {
              ...
              logViewerApiReadyWarning(
                "Failed to apply pending viewer scene",
                error
              );
            }
          };
          try {
            ...
            // Fallback when rAF is unavailable (e.g., SSR or test)
            setTimeout(applyScene, 0);
          }
        }
      } catch (error) {
        logViewerApiReadyWarning(
          "Error handling viewer API ready event",
          error
        );
      }
    },
