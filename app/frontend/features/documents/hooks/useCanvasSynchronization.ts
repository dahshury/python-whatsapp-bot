...
import { withBatchedUpdatesThrottled } from "@/shared/libs/react/batched";
...
export function useCanvasSynchronization(
  ...
  // Throttled viewer camera change handler using batched updates
  const throttledViewerCameraChangeRef = useRef(
    withBatchedUpdatesThrottled((newSig: unknown, appState: unknown) => {
      const typedSig = newSig as string;
      const typedAppState = appState as Record<string, unknown>;
      // Update refs
      viewerCameraRef.current = typedAppState;
      lastViewerCameraSigRef.current = typedSig;
  ...
  // Update the throttled function ref when dependencies change
  useEffect(() => {
    throttledViewerCameraChangeRef.current = withBatchedUpdatesThrottled(
      (newSig: unknown, appState: unknown) => {
        const typedSig = newSig as string;
        const typedAppState = appState as Record<string, unknown>;
        viewerCameraRef.current = typedAppState;
        lastViewerCameraSigRef.current = typedSig;
  ...
  const handleViewerCanvasChange = useCallback(
    (
      ...
      // Use throttled batched update to avoid excessive re-renders
      throttledViewerCameraChangeRef.current(newSig, appState);
    },
    []
  );
