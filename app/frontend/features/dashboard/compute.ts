...
export function computeFullDashboardData(
	...
	const uniqueCustomerIds = new Set<string>([...Object.keys(reservations ?? {}), ...Object.keys(conversations ?? {})]);
	...
	const totalCancellations = filteredReservationEntries.reduce(
		(sum, [, items]) =>
			sum + (Array.isArray(items) ? items.filter((r) => (r as Reservation).cancelled === true).length : 0),
		0
	);
	...
	return {
		...
		prometheusMetrics: {
			...
			...(typeof (prom as { reservations_requested_total?: number }).reservations_requested_total === "number" && {
			...
			...(typeof (prom as { reservations_successful_total?: number }).reservations_successful_total === "number" && {
							...
							(sum, [, msgs]) =>
								...
								(Array.isArray(msgs)
									? msgs.reduce(
											(s, m) =>
												s +
												((m as ConversationMessage).text || (m as ConversationMessage).message || "").toString().length,
											0
										)
									: 0),
							0
