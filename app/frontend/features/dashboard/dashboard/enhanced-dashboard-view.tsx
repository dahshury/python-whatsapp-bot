...
import { Tabs, TabsList, TabsTrigger } from "@/shared/ui/animate-ui/components/radix/tabs";
...
export function EnhancedDashboardView() {
	...
	const { dashboardData, isLoading, error, refresh: refreshDashboard } = useDashboardData();
			...
			<div className="min-h-[25rem] flex items-center justify-center">
				<Card className="w-full max-w-md">
					<CardContent className="p-6 text-center">
						...
						<h3 className="text-lg font-semibold mb-2">{i18n.getMessage("dashboard_error_title", isLocalized)}</h3>
		...
		return (
			<div className="space-y-6">
				...
				<div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6">
					{["total", "active", "pending", "completed", "cancelled", "revenue"].map((metric) => (
	...
	return (
		<div className="space-y-6">
			...
			>
				<div>
					<h1 className="text-3xl font-bold tracking-tight">{i18n.getMessage("dashboard_title", isLocalized)}</h1>
					...
					{isUsingMockData && (
						<div className="mt-2 p-3 bg-accent/10 border border-accent/20 rounded-lg">
							<p className="text-sm text-accent-foreground">
								<strong>{i18n.getMessage("dashboard_demo_mode", isLocalized)}:</strong>{" "}
						...
						{dashboardData && (
							<div className="flex items-center gap-2 self-start">
								...
								<Badge variant="outline">
									{dashboardData.stats.totalReservations} {i18n.getMessage("dashboard_reservations", isLocalized)}
								</Badge>
							</div>
						)}

						<div className="flex items-center gap-3">
							<DateRangeWithPresets value={filters.dateRange} onChange={handleDateRangeChange} />

							<Button onClick={handleExport} variant="outline" size="sm" disabled={!dashboardData}>
			...
			{isLoading && !dashboardData && (
				<div className="space-y-6">
					<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
						{["reservations", "revenue", "customers", "avg-value", "completion", "rating"].map((stat) => (
					...
					<div className="grid gap-6 md:grid-cols-2">
						{["trends", "distribution", "performance", "analytics"].map((chart) => (
			...
			{dashboardData && (
				<Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
					<TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 gap-2 lg:max-w-[37.5rem] mx-auto">
						...
						<div className="pt-4">
							...
							<div className="grid gap-6 grid-cols-1 lg:grid-cols-12">
								...
								<div className="lg:col-span-4">
									<OperationMetrics prometheusMetrics={safeDashboard.prometheusMetrics} isLocalized={isLocalized} />
								</div>
							...
							<div className="space-y-6">
								<div className="grid gap-6">
									...
									<Card>
										<CardHeader>
											<CardTitle>{i18n.getMessage("response_time_insights", isLocalized)}</CardTitle>
										</CardHeader>
										<CardContent>
											<div className="space-y-2 text-sm">
												{dashboardData.conversationAnalysis.responseTimeStats.avg <= 2 && (
													<div className="p-3 bg-chart-1/20 border border-chart-1/30 rounded-lg">
														<p className="text-chart-1">{i18n.getMessage("response_time_excellent", isLocalized)}</p>
													</div>
												)}
												{dashboardData.conversationAnalysis.responseTimeStats.avg > 2 &&
													dashboardData.conversationAnalysis.responseTimeStats.avg <= 5 && (
														<div className="p-3 bg-chart-2/20 border border-chart-2/30 rounded-lg">
															<p className="text-chart-2">{i18n.getMessage("response_time_good", isLocalized)}</p>
														</div>
													)}
												{dashboardData.conversationAnalysis.responseTimeStats.avg > 5 &&
													dashboardData.conversationAnalysis.responseTimeStats.avg <= 10 && (
														<div className="p-3 bg-chart-3/20 border border-chart-3/30 rounded-lg">
															<p className="text-chart-3">
																{i18n.getMessage("response_time_needs_improvement", isLocalized)}
															</p>
														</div>
													)}
												{dashboardData.conversationAnalysis.responseTimeStats.avg > 10 && (
													<div className="p-3 bg-destructive/20 border border-destructive/30 rounded-lg">
														<p className="text-destructive">{i18n.getMessage("response_time_poor", isLocalized)}</p>
													</div>
												)}
											</div>
										</CardContent>
									...
									<Card>
										<CardHeader>
											<CardTitle>{i18n.getMessage("conversation_insights", isLocalized)}</CardTitle>
										</CardHeader>
										<CardContent>
											<div className="space-y-2 text-sm">
												{dashboardData.conversationAnalysis.avgMessagesPerCustomer >= 20 && (
													<div className="p-3 bg-chart-1/20 border border-chart-1/30 rounded-lg">
														<p className="text-chart-1">{i18n.getMessage("conversation_insight_high", isLocalized)}</p>
													</div>
												)}
													...
													dashboardData.conversationAnalysis.avgMessagesPerCustomer < 20 && (
														<div className="p-3 bg-chart-2/20 border border-chart-2/30 rounded-lg">
															<p className="text-chart-2">
																{i18n.getMessage("conversation_insight_medium", isLocalized)}
															</p>
														</div>
													)}
												{dashboardData.conversationAnalysis.avgMessagesPerCustomer < 10 && (
													<div className="p-3 bg-chart-3/20 border border-chart-3/30 rounded-lg">
														<p className="text-chart-3">{i18n.getMessage("conversation_insight_low", isLocalized)}</p>
													</div>
												)}
											</div>
										</CardContent>
									...
									<Card>
										<CardHeader>
											<CardTitle>{i18n.getMessage("dashboard_business_insights", isLocalized)}</CardTitle>
										</CardHeader>
										<CardContent>
											<div className="space-y-4">
												...
												<div className="p-4 bg-chart-1/10 rounded-lg border-l-4 border-chart-1">
													<h4 className="font-semibold text-chart-1">
														{i18n.getMessage("dashboard_customer_retention_title", isLocalized)}
													</h4>
