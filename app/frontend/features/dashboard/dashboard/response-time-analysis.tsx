...
function ResponseTimeMetric({ title, value, unit, icon, variant = "default", description }: ResponseTimeMetricProps) {
...
export function ResponseTimeAnalysis({ conversationAnalysis, isLocalized }: ResponseTimeAnalysisProps) {
	const { responseTimeStats } = conversationAnalysis;

	// Determine performance levels based on response times
	const getPerformanceVariant = (time: number): "success" | "warning" | "danger" | "default" => {
	...
	return (
		<div className="space-y-4">
			<div className="flex items-center justify-between">
				<h2 className="text-xl font-semibold">{i18n.getMessage("response_time_analysis_title", isLocalized)}</h2>
				<Badge variant="outline" className={getScoreColor(performanceScore)}>
					{i18n.getMessage("response_time_score", isLocalized)}: {performanceScore.toFixed(0)}%
				</Badge>
			</div>
