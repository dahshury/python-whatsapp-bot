...
export function DashboardView() {
	// Warm up query cache for stats - hooks must be called unconditionally
	useDashboardStats()
...
export function _OriginalDashboardView() {
	...
	const getActivityIndicatorClass = (type: string) => {
		switch (type) {
			case 'reservation':
				return 'bg-primary'
			case 'conversation':
	...
	const getActivityStatusVariant = (
		status: string
							...
							{recentActivity.map((activity) => (
								...
								>
									<div className="flex items-center gap-3">
										<div
											className={`h-2 w-2 rounded-full ${getActivityIndicatorClass(
												activity.type
											)}`}
										/>
										<div>
