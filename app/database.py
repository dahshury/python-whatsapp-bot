...
class PostgreSQLConnection:
    ...
    async def execute(
        ...
        try:
            ...
            if sql.strip().upper().startswith(("INSERT", "UPDATE", "DELETE")):
                ...
                parts = result.split()
                changes = int(parts[-1]) if len(parts) >= 2 else 0
            ...
            raise RuntimeError("PostgreSQL operation failed") from e
    ...
    async def fetchall(
        ...
        try:
            ...
            raise RuntimeError("PostgreSQL fetchall failed") from e
        else:
    ...
    async def fetchone(
        ...
        try:
            ...
            raise RuntimeError("PostgreSQL fetchone failed") from e
        else:
    ...
    async def executemany(
        ...
        try:
            async with connection.transaction():
                ...
                return {"success": True, "changes": total_changes, "results": []}

        except Exception as e:
            raise RuntimeError("PostgreSQL executemany failed") from e
    ...
    async def batch(self, statements: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError("PostgreSQL batch operation failed") from e
...
class DatabaseManager:
    """Singleton class to manage database instance"""
    _instance: Optional[PostgreSQLDatabase] = None

    @classmethod
    async def get_instance(cls) -> PostgreSQLDatabase:
...
async def search_similar_conversations(
    query_text: str, limit: int = 10, wa_id: Optional[str] = None
) -> List[Dict[str, Any]]:
...
async def insert_conversation_with_embedding(
    wa_id: str, role: str, message: str, date: str, time: str
) -> Dict[str, Any]:
