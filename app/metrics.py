...
MODIFY_REQUESTS = Counter(
    ...
    "Total reservation modification attempts",
)
...
RETRY_LAST_TIMESTAMP = Gauge(
    "api_retry_last_timestamp_seconds",
...
ANTHROPIC_RETRY_ATTEMPTS = LLM_RETRY_ATTEMPTS.labels(
    provider="anthropic", error_type="unknown"
)
...
BACKUP_SCRIPT_FAILURES = Counter(
    "backup_script_failures_total", "Total number of backup script failures"
)
...
INVALID_HTTP_REQUESTS = Counter(
    "invalid_http_requests_total", "Total number of invalid HTTP requests received"
)
...
INBOUND_QUEUE_ENQUEUE_DUPLICATE = Counter(
    "inbound_queue_enqueue_duplicate_total",
    "Total inbound messages skipped due to duplicate message_id",
    ["source"],
)

INBOUND_QUEUE_CLAIMED = Counter(
    "inbound_queue_claimed_total", "Total queue items successfully claimed by workers"
)

INBOUND_QUEUE_PROCESSED = Counter(
    "inbound_queue_processed_total", "Total inbound queue items processed successfully"
)

INBOUND_QUEUE_PROCESSING_ERRORS = Counter(
    "inbound_queue_processing_errors_total",
    "Total inbound queue items that failed during processing",
)

INBOUND_QUEUE_CLAIM_FAILURES = Counter(
    "inbound_queue_claim_failures_total",
    "Total failures when attempting to claim a queue item",
)

INBOUND_QUEUE_LENGTH = Gauge(
    "inbound_queue_length", "Current number of pending inbound queue items"
)

INBOUND_QUEUE_OLDEST_AGE_SECONDS = Gauge(
    "inbound_queue_oldest_age_seconds",
    "Age in seconds of the oldest pending inbound queue item",
)
