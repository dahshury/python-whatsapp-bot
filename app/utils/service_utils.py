...
def format_response(
    success: bool, data=None, message: str = None, status_code: int = None
):
    """
...
def find_nearest_time_slot(target_slot, available_slots):
    ...
    for slot in available_slots:
        try:
            ...
            logging.error(
                f"Could not parse slot '{slot}' in find_nearest_time_slot: {e}"
            )
            continue
...
def get_all_reservations(future=True, include_cancelled=False):
    ...
    try:
        with get_session() as session:
            ...
            if future:
                today = (
                    datetime.datetime.now(ZoneInfo(config["TIMEZONE"]))
                    .date()
                    .isoformat()
                )
            ...
            stmt = stmt.order_by(
                ReservationModel.wa_id.asc(),
                ReservationModel.date.asc(),
                ReservationModel.time_slot.asc(),
            )
            ...
            if start_date and not wa_id:
                ...
                recent_wa_ids = [
                    ...
                    for row in session.execute(
                        recent_stmt, {"threshold": threshold}
                    ).all()
                ]
            ...
            if wa_id:
                ...
                if limit > 0:
                    stmt = base_stmt.order_by(
                        ConversationModel.date.desc(), ConversationModel.time.desc()
                    ).limit(limit)
                else:
                    stmt = base_stmt.order_by(
                        ConversationModel.date.asc(), ConversationModel.time.asc()
                    )
                rows = session.execute(stmt).all()
            elif recent_wa_ids:
                base_stmt = select(
                ...
                if limit > 0:
                    stmt = base_stmt.order_by(
                        ConversationModel.wa_id.asc(),
                        ConversationModel.date.desc(),
                        ConversationModel.time.desc(),
                    )
                else:
                    stmt = base_stmt.order_by(
                        ConversationModel.wa_id.asc(),
                        ConversationModel.date.asc(),
                        ConversationModel.time.asc(),
                    )
                rows = session.execute(stmt).all()
            else:
                base_stmt = select(
...
def retrieve_messages(wa_id):
    ...
    try:
        ...
        try:
            ...
            conv_limit = 30
        response = get_all_conversations(
            wa_id=wa_id, limit=conv_limit if conv_limit > 0 else 0
        )
        ...
        try:
            messages = sorted(
                messages, key=lambda m: (str(m.get("date", "")), str(m.get("time", "")))
            )
        except Exception:
            pass
        input_chat = []
    ...
    with get_session() as session:
        ...
        if existing is None:
            ...
            if customer_name is not None and (
                existing.customer_name is None
                or str(existing.customer_name).strip() == ""
            ):
...
def parse_time(time_str, to_24h=True):
    ...
    try:
        ...
        logging.debug(
            f"Error while parsing time with dateutil: {e}, trying manual parsing"
        )
        ...
        try:
            ...
            logging.error(
                f"Manual time parsing failed for '{time_str}': {manual_error}"
            )
    ...
    try:
        ...
        for start_day, duration in vacation_dict.items():
            try:
                ...
                logging.error(
                    f"Error checking vacation period for date {start_day}: {e}"
                )
...
def filter_past_time_slots(
    time_slots_dict, current_time=None, slot_duration_hours: int = 2
):
    """
    ...
    for time_slot, count in time_slots_dict.items():
        try:
            ...
            slot_start_dt = datetime.datetime(
                2000, 1, 1, time_obj.hour, time_obj.minute
            )
            slot_end_dt = slot_start_dt + datetime.timedelta(
                hours=max(1, int(slot_duration_hours))
            )
            ...
            logging.error(
                f"Could not parse time slot '{time_slot}' in filter_past_time_slots: {e}"
            )
            filtered_slots[time_slot] = count
    ...
    try:
        ...
        if time_str:
            ...
            if date_obj == today:
                ...
                slot_end_dt = start_dt + datetime.timedelta(
                    hours=2
...
def delete_user(wa_id):
    ...
    # Perform deletion
    with get_session() as session:
        session.query(ReservationModel).filter(ReservationModel.wa_id == wa_id).delete(
            synchronize_session=False
        )
        session.query(ConversationModel).filter(
            ConversationModel.wa_id == wa_id
        ).delete(synchronize_session=False)
        session.query(CustomerModel).filter(CustomerModel.wa_id == wa_id).delete(
            synchronize_session=False
        )
        session.commit()
