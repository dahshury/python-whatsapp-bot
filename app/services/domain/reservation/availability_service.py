...
from .reservation_models import ReservationType
...
class AvailabilityService(BaseService):
    ...
    def _get_upcoming_vacation_info(
        ...
        try:
            ...
            if vacation_start_dates and vacation_durations:
                ...
                def _safe_parse_date(date_str: str) -> Optional[datetime.date]:
                    """Parse a YYYY-MM-DD string to a date, returning None if invalid."""
                    try:
                        return datetime.datetime.strptime(date_str, "%Y-%m-%d").date()
                    except (ValueError, TypeError):
                        self.logger.exception("Error processing vacation date %s", date_str)
                        return None

                for start_date_str, duration in zip(start_dates, durations):
                    vacation_start = _safe_parse_date(start_date_str)
                    ...
                    if 0 < days_until_vacation <= 30:
                        ...
                        start_dt = datetime.datetime.combine(
                            vacation_start, datetime.time.min
                        ).replace(tzinfo=ZoneInfo(self.timezone))
                        end_dt = datetime.datetime.combine(
                            vacation_end, datetime.time.min
                        ).replace(tzinfo=ZoneInfo(self.timezone))
            ...
            self.logger.exception("Error checking vacation info")
            return None
        else:
            return None
    ...
    async def get_available_time_slots(
        ...
        try:
            ...
            try:
                ...
                self.logger.exception("Error parsing start date")
                return format_response(False, message="Invalid date format")
            ...
            try:
                ...
                for slot_12h, slot_24h in time_format_map.items():
                    try:
                        active_reservations = await repository.find_active_by_slot(
                            parsed_date_str, slot_24h
                        )
                        all_slots[slot_12h] = len(active_reservations)
                    except (ValueError, KeyError, OSError):
                        self.logger.exception("Error checking slot %s on %s", slot_24h, parsed_date_str)
                        continue
            except Exception:
                self.logger.exception("Error getting repository for slot checking")
                return format_response(False, message="Database error")

            # Build list of slots with remaining capacity
            available_slots = [
                ts for ts, count in all_slots.items() if count < max_reservations
            ]
            if not available_slots:
                return format_response(
                    False, message=get_message("all_slots_fully_booked")
                )
    ...
    async def search_available_appointments(
        ...
        try:
            ...
            if start_date:
                # Convert string start_date to date object with proper typing
                search_start_date: datetime.date
                if isinstance(start_date, str):
                    try:
                        # Use parse_date which handles both Gregorian and Hijri dates
                        parsed_date_str = parse_date(start_date, hijri=hijri)
                        search_start_date = datetime.datetime.strptime(
                            parsed_date_str, "%Y-%m-%d"
                        ).date()
                    except Exception:
                        self.logger.exception("Error parsing start date")
                        # Fall back to today if parsing fails
                        search_start_date = today.date()
                else:
                    raise ValueError(
                        "start_date must be a string (YYYY-MM-DD) format"
                    )

                # Create a datetime object at the start of the day in TIMEZONE
                today = datetime.datetime.combine(
                    search_start_date, datetime.time.min
                ).replace(tzinfo=ZoneInfo(self.timezone))
            else:
                ...
                if is_vacation_today:
                    ...
                    if vacation_end_date:
                        ...
                        search_start_date = vacation_end_date + datetime.timedelta(days=1)
                        today = datetime.datetime.combine(
                            search_start_date, datetime.time.min
                        ).replace(tzinfo=ZoneInfo(self.timezone))
            ...
            for day_offset in date_range:
                ...
                if time_slot is None:
                    result = await self.get_available_time_slots(
                    ...
                    if isinstance(result, dict) and "data" in result:
                        ...
                        if isinstance(slot_data, dict) and "time_slots" in slot_data:
                            ...
                            response_gregorian = slot_data.get(
                                "gregorian_date", gregorian_date_str
                            )
                            ...
                            available_slots = (
                                slot_data if isinstance(slot_data, list) else []
                            )
                ...
                is_exact = (
                    closest_time.hour == requested_time.hour
                ...
                try:
                    ...
                    self.logger.exception("Database error while checking slot %s on %s", closest_slot_24h, gregorian_date_str)

            # If no time_slot was provided, convert the grouped map to a list
            if time_slot is None and date_slots_map:
                available_dates.extend(date_slots_map.values())
            ...
            if vacation_info:
                ...
                if vacation_info["status"] == "current":
                    if vacation_info.get("end_date"):
                        ...
                        response_data["vacation_end_gregorian"] = end_date.strftime(
                            "%Y-%m-%d"
                        )
                    ...
                    response_data["vacation_start_gregorian"] = start_date.strftime(
                        "%Y-%m-%d"
                    )
    ...
    async def get_available_slots(self, date: str, _reservation_type: int) -> List[str]:
        """
        ...
        try:
            ...
            existing_reservations = await repository.get_reservations_by_date(
                date, include_cancelled=False
            )
            ...
            available_slots = [
                slot for slot in self.time_slots if slot not in booked_slots
            ]

        except Exception:
            self.logger.exception("Error getting available slots")
            return []
        else:
            return available_slots

    async def is_slot_available(
        self, date: str, time_slot: str, exclude_reservation_id: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.exception("Error checking slot availability")
            return False
        else:
            return True
    ...
    async def get_daily_availability_summary(self, date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            reservations = await repository.get_reservations_by_date(
                date, include_cancelled=False
            )
            ...
            type_0_count = len(
                [r for r in reservations if r.type == ReservationType.CHECK_UP]
            )
            type_1_count = len(
                [r for r in reservations if r.type == ReservationType.FOLLOW_UP]
            )
            ...
            self.logger.exception("Error getting daily availability summary")
            return {"error": "An error occurred while getting daily availability summary"}
    ...
    async def get_weekly_availability(
        ...
        try:
            ...
            weekly_summary = {
                ...
                await self.get_daily_availability_summary(
                    (start_datetime + datetime.timedelta(days=i)).strftime("%Y-%m-%d")
                )
            ...
            self.logger.exception("Error getting weekly availability")
            return {"error": "An error occurred while getting weekly availability"}
        else:
            return weekly_summary

    def get_next_available_slots(
        self, date: str, available_slots: List[str], count: int = 3
    ) -> List[str]:
        """
        ...
        try:
            ...
            if date_obj == now.date():
                ...
                available_slots = [
                    slot for slot in available_slots if slot > current_time
                ]
            ...
            self.logger.exception("Error getting next available slots")
            return []
