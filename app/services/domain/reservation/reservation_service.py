...
class ReservationService(BaseService):
    ...
    def get_customer_reservations(
        ...
        try:
            ...
            reservations = self.reservation_repository.find_by_wa_id(
                wa_id, include_past
            )
    ...
    def reserve_time_slot(
        ...
        try:
            ...
            self.logger.info(
                f"Reservation check for wa_id={wa_id}: found {len(existing_reservations)} existing, {len(future_reservations)} future"
            )
            ...
            self.logger.info(
                f"Creating new reservation: wa_id={wa_id}, type={reservation_type}, date={parsed_date_str}, time={parsed_time_str}"
            )
            ...
            try:
                ...
                if not reservation_id:
                    self.logger.error(
                        f"Repository save returned falsy ID: {reservation_id} for wa_id={wa_id}, type={reservation_type}"
                    )
                    return self._handle_error(
                        "reserve_time_slot",
                        Exception("Failed to save reservation - no ID returned"),
                        ar,
                    )

                self.logger.info(
                    f"Successfully created reservation {reservation_id} for wa_id={wa_id}, type={reservation_type}"
                )
            except Exception as save_error:
                self.logger.error(
                    f"Exception during reservation save for wa_id={wa_id}, type={reservation_type}: {save_error}",
                    exc_info=True,
                )
    ...
    def modify_reservation(
        ...
        try:
            ...
            if (
                ...
                allowed_slots_24h = get_time_slots(
                    date_str=parsed_new_date_str, to_24h=True
                )
                if (
                    isinstance(allowed_slots_24h, dict)
                    and allowed_slots_24h.get("success") is False
                ):
                    # Forward vacation/non-working day or other errors
                    return allowed_slots_24h
                if (
                    not isinstance(allowed_slots_24h, dict)
                    or parsed_new_time_str not in allowed_slots_24h
                ):
                    display_time_slot_candidate = normalize_time_format(
                        parsed_new_time_str, to_24h=False
                    )
                    allowed_slots_display = get_time_slots(
                        date_str=parsed_new_date_str, to_24h=False
                    )
                    if (
                        isinstance(allowed_slots_display, dict)
                        and allowed_slots_display.get("success") is False
                    ):
                        self._record_failure_metric(
                            "modify", "slot_invalid_or_unavailable"
                        )
                        return allowed_slots_display
                    slots_list = (
                        list(allowed_slots_display.keys())
                        if isinstance(allowed_slots_display, dict)
                        else []
                    )
                    slots_str = (
                        ", ".join(slots_list) if slots_list else "None available"
                    )
                    return format_response(
                        False,
                        message=get_message(
                            "reservation_failed_slot",
                            ar,
                            slot=display_time_slot_candidate,
                            slots=slots_str,
                        ),
                    )
                ...
                if count_other_reservations >= max_reservations:
                    if approximate:
                        ...
                        self._record_failure_metric(
                            "modify", "slot_unavailable_approx_not_impl"
                        )
                        return format_response(
                            False,
                            message=get_message("slot_unavailable_approx_not_impl", ar),
                        )
                    else:
    ...
    def cancel_reservation(
        ...
        try:
            ...
            if reservation_id_to_cancel is not None:
                ...
                success = self.reservation_repository.cancel_by_id(
                    reservation_id_to_cancel
                )
                if success:
                ...
                active_reservations_on_date = [
                    res
                    for res in self.reservation_repository.find_by_wa_id(
                ...
                if not active_reservations_on_date:
                    ...
                    return format_response(
                        False,
                        message=get_message(
                            "no_reservation_on_date", ar, date=parsed_target_date_str
                        ),
                    )
                ...
                if not future_reservations_on_date:
                    self._record_failure_metric(
                        "cancel", "no_future_reservations_on_date"
                    )
                    return format_response(
                        False,
                        message=get_message(
                            ...
                            ar,
                            date=parsed_target_date_str,
                        ),
                    )

                for res in future_reservations_on_date:
                    if self.reservation_repository.cancel_by_id(
                        res.id
                    ):  # Use cancel_by_id for individual tracking
                ...
                if (
                    ...
                    return format_response(
                        False,
                        message=get_message(
                            "cancellation_failed_date", ar, date=parsed_target_date_str
                        ),
                    )
                ...
                cancelled_count = self.reservation_repository.cancel_by_wa_id(
                    wa_id, date_str=None
                )
    ...
    def undo_cancel_reservation_by_id(
        ...
        try:
            ...
            if (
                not isinstance(allowed_slots_24h, dict)
                or reservation.time_slot not in allowed_slots_24h
            ):
                display_slot = normalize_time_format(
                    reservation.time_slot, to_24h=False
                )
                allowed_slots_display = get_time_slots(
                    date_str=reservation.date, to_24h=False
                )
                if (
                    isinstance(allowed_slots_display, dict)
                    and allowed_slots_display.get("success") is False
