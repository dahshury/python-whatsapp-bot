...
class ReservationService(BaseService):
    ...
    def _record_failure_metric(self, operation: str, reason: str) -> None:
        ...
        try:
            if operation == "modify":
                MODIFY_FAILURES_BY_REASON.labels(reason=reason, endpoint="n/a", method="n/a").inc()
            elif operation == "cancel":
                CANCELLATION_FAILURES_BY_REASON.labels(reason=reason, endpoint="n/a", method="n/a").inc()
        except Exception:
            pass
        ...
        try:
            ...
            if display_time_slot not in available_slots:
                ...
                return format_response(
                    False, message=get_message("reservation_failed_slot", ar, slot=display_time_slot, slots=slots_str)
                )

            # Check for existing active reservations
            existing_reservations = self.reservation_repository.find_by_wa_id(wa_id)
            future_reservations = [
                res for res in existing_reservations if res.is_future(datetime.datetime.now(tz=ZoneInfo(self.timezone)))
            ]
            ...
            cancelled_reservation = self.reservation_repository.find_cancelled_reservation(
            ...
            try:
                ...
                if not reservation_id:
                    ...
                    return self._handle_error(
                        "reserve_time_slot", Exception("Failed to save reservation - no ID returned"), ar
                    )
        ...
        try:
            ...
            if reservation_id_to_modify:
                ...
                if not reservation_to_modify:
                    ...
                    return format_response(
                        False, message=get_message("reservation_not_found_id", ar, id=reservation_id_to_modify)
                    )
            ...
            if (
                ...
                if not isinstance(allowed_slots_24h, dict) or parsed_new_time_str not in allowed_slots_24h:
                    ...
                    return format_response(
                        False,
                        message=get_message(
                            "reservation_failed_slot", ar, slot=display_time_slot_candidate, slots=slots_str
                        ),
                    )
            ...
            hijri_date_str_new = (
                f"{hijri_date_obj_new.year}-{hijri_date_obj_new.month:02d}-{hijri_date_obj_new.day:02d}"
            )
        ...
        try:
            ...
            if reservation_id_to_cancel is not None:
                ...
                if not reservation:
                    ...
                    return format_response(
                        False, message=get_message("reservation_not_found_id", ar, id=reservation_id_to_cancel)
                    )
                ...
                if not reservation.is_future(now):
                    ...
                    return format_response(
                        False, message=get_message("cannot_cancel_past_reservation", ar, id=reservation_id_to_cancel)
                    )
                ...
                if success:
                    ...
                    return format_response(
                        False, message=get_message("cancellation_failed_specific", ar, id=reservation_id_to_cancel)
                    )
                ...
                if not active_reservations_on_date:
                    ...
                    return format_response(
                        False, message=get_message("no_reservation_on_date", ar, date=parsed_target_date_str)
                    )

                # Filter to only future reservations
                future_reservations_on_date = [res for res in active_reservations_on_date if res.is_future(now)]

                if not future_reservations_on_date:
                    self._record_failure_metric("cancel", "no_future_reservations_on_date")
                    return format_response(
                        False, message=get_message("no_future_reservations_on_date", ar, date=parsed_target_date_str)
                    )
                ...
                for res in future_reservations_on_date:
                    if self.reservation_repository.cancel_by_id(res.id):  # Use cancel_by_id for individual tracking
                        ...
                        try:
                            enqueue_broadcast(
                                "reservation_cancelled",
                                {"id": res.id, "wa_id": res.wa_id, "date": res.date, "time_slot": res.time_slot},
                ...
                if cancelled_count == 0 and future_reservations_on_date:  # Should not happen if logic is correct
                    ...
                    return format_response(
                        False, message=get_message("cancellation_failed_date", ar, date=parsed_target_date_str)
                    )
                ...
                if cancelled_count > 0:
                    try:
                        enqueue_broadcast(
                            "reservation_cancelled", {"wa_id": wa_id}, affected_entities=[wa_id], source=_call_source
                        )
                    except Exception:
                        pass
            ...
            if cancelled_count > 0:
                result = format_response(
                    True,
                    message=get_message("reservations_cancelled_successfully", ar, count=cancelled_count),
    ...
    def undo_cancel_reservation_by_id(
        ...
        try:
            ...
            if reservation.status == "active":
                return format_response(
                    True,
                    message=get_message("reservation_already_active", ar, id=reservation_id),
            ...
            if not isinstance(allowed_slots_24h, dict) or reservation.time_slot not in allowed_slots_24h:
                ...
                slots_str = ", ".join(slots_list) if slots_list else "None available"
                return format_response(
                    False, message=get_message("reservation_failed_slot", ar, slot=display_slot, slots=slots_str)
                )
            ...
            if success:
                ...
                if reinstated_reservation:
                    ...
                    return format_response(
                        True,
                        message=get_message("reservation_reinstated", ar, id=reservation_id),
                        data={
                    ...
                    return self._handle_error(
                        "undo_cancel_reservation_by_id", Exception("Failed to fetch reinstated reservation"), ar
                    )
            else:
                # This means it was not in 'cancelled' state or db error
                return format_response(
                    False, message=get_message("reservation_reinstate_failed", ar, id=reservation_id)
                )
        except Exception as e:
    ...
    def undo_reserve_time_slot_by_id(self, reservation_id: int, ar: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                return format_response(
                    True,
                    message=get_message("reservation_cancelled_for_undo", ar, id=reservation_id),
                    data={"cancelled_ids": [reservation_id]},
                )
            else:
                # This implies it was not 'active' or a DB error.
                return format_response(
                    False, message=get_message("cancellation_failed_specific", ar, id=reservation_id)
                )
        except Exception as e:
