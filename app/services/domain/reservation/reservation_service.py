...
class ReservationService(BaseService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.reservation_repository = (
            reservation_repository or get_reservation_repository()
        )
    ...
    def _broadcast_reservation_update(
        ...
        try:
            ...
            def run_broadcast():
                try:
                    ...
                    loop.run_until_complete(
                        websocket_manager.broadcast_reservation_update(
                            action, reservation_data
                        )
                    )
                    loop.close()
                except Exception:
                    self.logger.exception("Error broadcasting reservation update")
            ...
            self.logger.exception("Failed to start WebSocket broadcast thread")
    ...
    def get_customer_reservations(
        ...
        try:
            ...
            reservations = self.reservation_repository.get_reservations_by_customer(
                wa_id, include_past
            )
        ...
        try:
            ...
            if not customer_name:
                return format_response(
                    False, message=get_message("customer_name_required", ar)
                )
            ...
            if display_time_slot not in available_slots:
                slots_str = (
                    ", ".join(available_slots) if available_slots else "None available"
                )
                return format_response(
                    False,
                    message=get_message(
            ...
            existing_reservations = (
                self.reservation_repository.get_reservations_by_customer(wa_id)
            )
            future_reservations = [
                res
                for res in existing_reservations
                if res.is_future(datetime.datetime.now(tz=ZoneInfo(self.timezone)))
            ]
            ...
            cancelled_reservation = (
                self.reservation_repository.find_cancelled_reservation(
            ...
            if cancelled_reservation:
                ...
                success = self.reservation_repository.update_reservation(
                ...
                if not success:
                    return self._handle_error(
                        "reserve_time_slot",
                        Exception("Failed to reinstate reservation"),
                        ar,
                    )
            ...
            reservation_id = self.reservation_repository.create_reservation(
                wa_id, parsed_date_str, parsed_time_str, reservation_type
            )
            if not reservation_id:
                return self._handle_error(
                    "reserve_time_slot", Exception("Failed to save reservation"), ar
                )
    ...
    def modify_reservation(
        ...
        try:
            ...
            if reservation_id_to_modify:
                reservation_to_modify = self.reservation_repository.find_by_id(
                ...
                if not reservation_to_modify:
                    return format_response(
                        False,
                        message=get_message(
                            "reservation_not_found_id", ar, id=reservation_id_to_modify
                        ),
                    )
                ...
                if reservation_to_modify.wa_id != wa_id:
                    ...
                    self.logger.warning(
                        f"Attempt to modify reservation ID {reservation_id_to_modify} with mismatched wa_id {wa_id} (owner is {reservation_to_modify.wa_id})"
                    )
                    return format_response(
                        False,
                        message=get_message(
                ...
                reservations = self.reservation_repository.get_reservations_by_customer(
                    wa_id, include_past=False
                )
                future_reservations = [
                    res for res in reservations if res.is_future(now)
                ]
                ...
                reservation_to_modify = sorted(
                    future_reservations, key=lambda r: (r.date, r.time_slot)
                )[0]
            ...
            if customer:
                ...
                self.logger.error(
                    f"Customer not found for wa_id {wa_id} during modification of reservation {reservation_to_modify.id}"
                )
            ...
            if new_name and new_name != original_data["customer_name"]:
                name_update_result = self.customer_service.update_customer_name(
                    wa_id, new_name, ar
                )
                if not name_update_result.get("success"):
                    return name_update_result  # Propagate error
                reservation_to_modify.customer_name = (
                    new_name  # Update in-memory object
                )
                changes_made = True

            # Update reservation type if provided
            if new_type is not None:
                is_valid_type, type_error_result, parsed_new_type = (
                    validate_reservation_type(new_type, ar)
                )
            ...
            if new_date or new_time_slot:
                ...
                valid_dt, err_msg_dt, temp_parsed_date, temp_parsed_time = (
                    is_valid_date_time(date_to_parse, time_to_parse, hijri)
                )
                if not valid_dt:
                ...
                if new_time_slot:  # Only validate if time_slot was actually provided
                    valid_slots_result = get_time_slots(
                        parsed_new_date_str, check_vacation=True, to_24h=False
                    )

                    # Check if get_time_slots returned an error (vacation, non-working day, etc.)
                    if isinstance(
                        valid_slots_result, dict
                    ) and not valid_slots_result.get("success", True):
                        return format_response(
                            False,
                            message=valid_slots_result.get(
                                "message", "Invalid date for reservations"
                            ),
                        )
                    ...
                    if not isinstance(valid_slots_result, dict):
                        return format_response(
                            ...
                            message=get_message(
                                ...
                                ar,
                                error="Failed to get valid time slots",
                            ),
                        )
                    ...
                    requested_time_12h = normalize_time_format(
                        parsed_new_time_str, to_24h=False
                    )

                    if requested_time_12h not in valid_time_slots:
                        slots_str = (
                            ", ".join(valid_time_slots)
                            if valid_time_slots
                            else "None available"
                        )
                        return format_response(
                ...
                if (
                    parsed_new_date_str != reservation_to_modify.date
                    or parsed_new_time_str != reservation_to_modify.time_slot
                ):
                    changes_made = True

            if (
                not changes_made and new_name is None and new_type is None
            ):  # If only date/time considered and they are same
                return format_response(
                    True,
                    message=get_message("no_actual_change_made", ar),
                    data={
            ...
            if (
                ...
                current_reservations_in_new_slot = (
                    self.reservation_repository.find_active_by_slot(
                ...
                count_other_reservations = len(
                    [
                        res
                        for res in current_reservations_in_new_slot
                        if res.id != reservation_to_modify.id
                    ]
                )

                # Enhanced logging for capacity debugging
                self.logger.info(
                    f"🎯 CAPACITY CHECK: slot={normalize_time_format(parsed_new_time_str, to_24h=False)} date={parsed_new_date_str}"
                )
                self.logger.info(
                    f"🎯 Current reservations in slot: {count_other_reservations}/{max_reservations} (excluding moving reservation {reservation_to_modify.id})"
                )
                ...
                if count_other_reservations >= max_reservations:
                    if approximate:
                        ...
                        self.logger.info(
                            f"🔄 APPROXIMATE: Slot {parsed_new_time_str} on {parsed_new_date_str} is full ({count_other_reservations}/{max_reservations}), searching for nearest available slot"
                        )

                        # Get available slots for the target date
                        availability_result = (
                            self.availability_service.get_available_time_slots(
                                parsed_new_date_str, max_reservations, hijri=False
                            )
                        )
                        ...
                        nearest_slot = find_nearest_time_slot(
                            requested_time_12h, available_slots
                        )
                        ...
                        nearest_slot_24h = normalize_time_format(
                            nearest_slot, to_24h=True
                        )

                        self.logger.info(
                            f"🔄 APPROXIMATE: Found nearest available slot: {nearest_slot} (24h: {nearest_slot_24h}) for original request {requested_time_12h}"
                        )
                        ...
                        self.logger.info(
                            f"🔄 APPROXIMATE: Automatically adjusted reservation from {requested_time_12h} to {nearest_slot}"
                        )

                    else:
                        self.logger.info(
                            f"❌ EXACT MODE: Slot {normalize_time_format(parsed_new_time_str, to_24h=False)} is full ({count_other_reservations}/{max_reservations}), rejecting modification"
                        )
                        return format_response(
                            False, message=get_message("slot_fully_booked", ar)
                        )
                    ...
                    self.logger.info(
                        f"✅ CAPACITY OK: Slot has space ({count_other_reservations}/{max_reservations}), proceeding with modification"
                    )
            ...
            update_success = self.reservation_repository.update_reservation(
                reservation_to_modify.id,
                date=parsed_new_date_str,
                time_slot=parsed_new_time_str,
                type=reservation_to_modify.type.value,
            )
            ...
            if not update_success:
                return self._handle_error(
                    "modify_reservation",
                    Exception("Failed to update reservation in DB"),
                    ar,
                )

            # Prepare response data
            hijri_date_obj_new = convert.Gregorian(
                *map(int, reservation_to_modify.date.split("-"))
            ).to_hijri()
            hijri_date_str_new = f"{hijri_date_obj_new.year}-{hijri_date_obj_new.month:02d}-{hijri_date_obj_new.day:02d}"
            display_time_slot_new = normalize_time_format(
                reservation_to_modify.time_slot, to_24h=False
            )
            ...
            if reservation_id_to_cancel is not None:
                ...
                if not reservation:
                    return format_response(
                        False,
                        message=get_message(
                            "reservation_not_found_id", ar, id=reservation_id_to_cancel
                        ),
                    )
                if reservation.wa_id != wa_id:
                    self.logger.warning(
                        f"Attempt to cancel reservation ID {reservation_id_to_cancel} with mismatched wa_id {wa_id} (owner is {reservation.wa_id})"
                    )
                    return format_response(
                        False,
                        message=get_message(
                ...
                if reservation.status == "cancelled":
                    return format_response(
                        True,
                        message=get_message("reservation_already_cancelled", ar),
                        data={"cancelled_ids": [reservation_id_to_cancel]},
                    )
                ...
                if not reservation.is_future(now):
                    return format_response(
                        ...
                        message=get_message(
                            ...
                            ar,
                            id=reservation_id_to_cancel,
                        ),
                    )

                success = self.reservation_repository.cancel_reservation(
                    reservation_id_to_cancel
                )
                if success:
                ...
                if not parsed_target_date:
                    return format_response(
                        False, message=get_message("invalid_date_format", ar)
                    )
                ...
                active_reservations_on_date = [
                    res
                    for res in self.reservation_repository.get_reservations_by_customer(
                        wa_id, include_past=True
                    )  # Get all to find by date
                ...
                if not active_reservations_on_date:
                    return format_response(
                        False,
                        message=get_message(
                            "no_reservation_on_date", ar, date=parsed_target_date
                        ),
                    )

                # Filter to only future reservations
                future_reservations_on_date = [
                    res for res in active_reservations_on_date if res.is_future(now)
                ]
                ...
                if not future_reservations_on_date:
                    return format_response(
                        False,
                        message=get_message(
                            "no_future_reservations_on_date",
                            ar,
                ...
                for res in future_reservations_on_date:
                    if self.reservation_repository.cancel_reservation(
                        res.id
                    ):  # Use cancel_reservation for individual tracking
                        cancelled_ids.append(res.id)
                        cancelled_count += 1

                if (
                    cancelled_count == 0 and future_reservations_on_date
                ):  # Should not happen if logic is correct
                    return format_response(
                        False,
                        message=get_message(
                            "cancellation_failed_date", ar, date=parsed_target_date
                        ),
                    )
                ...
                cancelled_ids.extend(
                    res.id for res in active_future_reservations
                )  # Store IDs before batch operation (or rely on repo return)

                # The repository method cancel_reservation (with date_str=None) handles this.
                # It returns the count of affected rows.
                cancelled_count = self.reservation_repository.cancel_reservation(
                    wa_id, date_str=None
                )
            ...
            if cancelled_count > 0:
                ...
                for reservation_id in cancelled_ids:
                    try:
                        ...
                        cancelled_reservation = self.reservation_repository.find_by_id(
                            reservation_id
                        )
                        if cancelled_reservation:
                            reservation_data = {
                                ...
                                "time_slot": normalize_time_format(
                                    cancelled_reservation.time_slot, to_24h=False
                                ),
                                "type": cancelled_reservation.type.value,
                                "status": "cancelled",
                            }
                            self._broadcast_reservation_update(
                                "cancelled", reservation_data
                            )
                    except Exception as e:
                        broadcast_errors.append((reservation_id, str(e)))

                # Log all broadcast errors at once
                if broadcast_errors:
                    self.logger.exception(
                        "Errors broadcasting cancellations for reservations: %s", broadcast_errors
                    )

                return format_response(
                    True,
                    message=get_message(
                        "reservations_cancelled_successfully", ar, count=cancelled_count
                    ),
                    data={"cancelled_ids": cancelled_ids},
                )
            else:
                ...
                return format_response(
                    False, message=get_message("no_reservations_to_cancel", ar)
                )
        ...
        try:
            ...
            if not reservation:
                return format_response(
                    False,
                    message=get_message(
                        "reservation_not_found_id", ar, id=reservation_id
                    ),
                )
            ...
            if reservation.status == "active":
                return format_response(
                    True,
                    message=get_message(
                        "reservation_already_active", ar, id=reservation_id
                    ),
            ...
            if success:
                ...
                reinstated_reservation = self.reservation_repository.find_by_id(
                    reservation_id
                )
                if reinstated_reservation:
                    hijri_date_obj = convert.Gregorian(
                        *map(int, reinstated_reservation.date.split("-"))
                    ).to_hijri()
                    hijri_date_str = f"{hijri_date_obj.year}-{hijri_date_obj.month:02d}-{hijri_date_obj.day:02d}"
                    display_time_slot = normalize_time_format(
                        reinstated_reservation.time_slot, to_24h=False
                    )
                    ...
                    return format_response(
                        True,
                        message=get_message(
                            "reservation_reinstated", ar, id=reservation_id
                        ),
                        data={
                    ...
                    return self._handle_error(
                        "undo_cancel_reservation_by_id",
                        Exception("Failed to fetch reinstated reservation"),
                        ar,
                    )
            else:
                ...
                return format_response(
                    False,
                    message=get_message(
                        "reservation_reinstate_failed", ar, id=reservation_id
                    ),
                )
        except Exception as e:
            return self._handle_error("undo_cancel_reservation_by_id", e, ar)

    def undo_reserve_time_slot_by_id(
        self, reservation_id: int, ar: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if reservation.status == "cancelled":
                return format_response(
                    True,
                    message=get_message("reservation_already_cancelled", ar),
                    data={"cancelled_ids": [reservation_id]},
                )
            ...
            if success:
                ...
                try:
                    ...
                    self.logger.exception(
                        "Error broadcasting undo reserve for reservation %s", reservation_id
                    )

                return format_response(
                    True,
                    message=get_message(
                        "reservation_cancelled_for_undo", ar, id=reservation_id
                    ),
                    data={"cancelled_ids": [reservation_id]},
                )
            else:
                ...
                return format_response(
                    False,
                    message=get_message(
                        "cancellation_failed_specific", ar, id=reservation_id
                    ),
                )
        except Exception as e:
