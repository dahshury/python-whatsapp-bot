...
class BaseService(abc.ABC):
    ...
    def _handle_error(
        self, operation: str, error: Exception, ar: bool = False
    ) -> dict[str, Any]:
        """
        ...
        if os.environ.get("ENVIRONMENT", "").lower() in ("development", "dev", "debug"):
            ...
            return format_response(
                False, message=f"Operation '{operation}' failed: {error_details}"
            )
    ...
    def _validate_wa_id(self, wa_id: str, ar: bool = False) -> dict[str, Any] | None:
        """
