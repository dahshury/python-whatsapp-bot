        ...
        try:
            ...
            if vacation_start_dates and vacation_durations:
                ...
                for start_date_str, duration in zip(start_dates, durations):
                    try:
                        vacation_start = datetime.datetime.strptime(
                            start_date_str, "%Y-%m-%d"
                        ).date()
                        vacation_end = vacation_start + datetime.timedelta(
                            days=duration - 1
                        )
                ...
                if parsing_errors:
                    self.logger.exception("Error processing vacation dates: %s", parsing_errors)
