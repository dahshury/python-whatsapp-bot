...
from app.database import get_connection, vector_similarity_search, insert_with_vector
...
class ConversationRepository:
    ...
    def _generate_simple_embedding(self, text: str, dimension: int = 384) -> List[float]:
        """
    ...
    async def add_conversation(self, wa_id: str, role: str, message: str, 
        ...
        try:
            ...
            await self._notify_update("conversation_added", {
                "wa_id": wa_id,
                "role": role,
    ...
    async def get_conversations(self, wa_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        """
    ...
    async def search_similar_conversations(self, query_text: str, 
                                         limit: int = 10,
                                         wa_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    async def find_intent_patterns(self, intent_keywords: List[str], 
        ...
        try:
            ...
            for conv in similar_conversations:
                ...
                conv['customer_name'] = customer_result['customer_name'] if customer_result else 'Unknown'
                enriched_results.append(conv)
            
            return enriched_results
            
        except Exception as e:
            print(f"Error finding intent patterns: {e}")
            return []
    
    async def analyze_conversation_sentiment(self, wa_id: str, 
        ...
        try:
            ...
            # Word count analysis
            total_words = sum(len(c['message'].split()) for c in conversations if c['message'])
            avg_words_per_message = round(total_words / total_messages, 2) if total_messages > 0 else 0
            
            # Look for common patterns
            reservation_keywords = ['reservation', 'book', 'table', 'reserve', 'appointment']
            inquiry_keywords = ['what', 'how', 'when', 'where', 'can you', 'help']
            complaint_keywords = ['problem', 'issue', 'wrong', 'error', 'cancel', 'disappointed']
            ...
            return {
                ...
                "engagement_score": round((user_messages / total_messages * 100) if total_messages > 0 else 0, 2)
            }
    ...
    async def get_conversation_insights(self, limit_customers: int = 100) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            total_conversations = await connection.fetchone("SELECT COUNT(*) as total FROM conversation")
            total_customers = await connection.fetchone("SELECT COUNT(DISTINCT wa_id) as total FROM conversation")
            
            return {
                "total_conversations": total_conversations['total'] if total_conversations else 0,
                "total_active_customers": total_customers['total'] if total_customers else 0,
                "top_active_customers": active_customers[:10],
                "trending_keywords": [{"word": word, "frequency": freq} for word, freq in top_keywords],
    ...
    async def search_conversations_by_keyword(self, keyword: str, 
        ...
        try:
            ...
            print(f"Error searching conversations by keyword: {e}")
            return []
    ...
    async def delete_conversation(self, conversation_id: int) -> bool:
        ...
        try:
            ...
            if result.get('changes', 0) > 0:
                # Send real-time notification
                await self._notify_update("conversation_deleted", {
