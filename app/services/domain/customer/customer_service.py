...
class CustomerService(BaseService):
    ...
    def __init__(self, repository: CustomerRepository | None = None, **kwargs):
        """
    ...
    def update_customer_name(
        self, wa_id: str, new_name: str, ar: bool = False
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not customer:
                ...
                return format_response(
                    False,
                    message=get_message(
                        "customer_not_found_for_update", ar, wa_id=wa_id
                    ),
                )
            ...
            save_success = self.repository.save(
                customer
            )  # Assuming save handles insert or update logic
            ...
            if save_success:
                ...
                return format_response(
                    False, message=get_message("customer_name_update_failed", ar)
                )
    ...
    def update_customer_age(
        self, wa_id: str, age: int | None, ar: bool = False
    ) -> dict[str, Any]:
            ...
            return (
                format_response(
                    ...
                    data={
                        "wa_id": wa_id,
                        "age": customer.age,
                        "age_recorded_at": customer.age_recorded_at.isoformat()
                        if customer.age_recorded_at
                        else None,
                    },
                )
                if ok
                else format_response(
                    False, message=get_message("customer_age_update_failed", ar)
                )
            )
    ...
    def get_or_create_customer(
        self, wa_id: str, customer_name: str | None = None
    ) -> Customer:
        """
